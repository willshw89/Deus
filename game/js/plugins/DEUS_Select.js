//=============================================================================
// DEUS_Select.js - Drag boxes, group selection and area designation tools
//=============================================================================

/*:
 * @target MZ
 * @plugindesc [DEUS Select] Drag rectangles to select units and to mark areas with designation tools (chop, gather, mine, quarry, dismantle, build, zones, cancel), like modern DF.
 * @author UF project
 * @base DEUS_World
 * @base DEUS_Jobs
 * @base DEUS_Interact
...
(() => {
    ...
    const pluginParams = (PluginManager.parameters("DEUS_Select") && Object.keys(PluginManager.parameters("DEUS_Select")).length ? PluginManager.parameters("DEUS_Select") : PluginManager.parameters("UF_Select")) || {};
    ...
    const DEFAULT_CONFIG = {
        ...
        tools: [
            ...
            { id: "plan", key: "J", keyCode: 74, label: "Plan", options: ["plan"], job: "plan", candidates: "any", color: "#93c5fd", hint: "Plan: drag to sketch room layouts (no cost)." },
    ...
    const KEY_CODES = {
        C: 67, G: 71, P: 80, M: 77, R: 82, V: 86, T: 84, L: 76, B: 66, O: 79, N: 78, J: 74, U: 85
    };
    ...
    function findUnitAt(x, y, area, z = 0, sx, sy) {
        ...
        if (window.$gameMap && typeof $gameMap.events === "function") {
            ...
            if (typeof sx === "number" && typeof sy === "number" && !isChest) {
                for (const ev of evs) {
            ...
            for (const ev of evs) {
                ...
                if (!u) continue;
                if (isChest) {
                    if (typeof ev.pos === "function" && ev.pos(x, y)) return u;
                    continue;
                }
    ...
    function getSelectedUnits() {
        const W = World();
        if (!W) return [];
        let units = selectedGroup.map(id => W.unit(id)).filter(Boolean);
        if (!units.length && window.$colonyManager && $colonyManager.selectedColonist) {
            const sc = $colonyManager.selectedColonist;
            const u = sc.unit || (typeof sc.id === "number" || typeof sc.id === "string" ? W.unit(sc.id) : (typeof sc.x === "number" ? sc : null));
            if (u) units = [u];
        }
        return units;
    }
    ...
    function setSelection(ids, options = {}) {
        ...
        // Single unit selection pops up inventory on right; group selection suppresses inventory
        if (window.UF && UF.Sheet) {
            if (selectedGroup.length === 1) {
                UF.Sheet.open(selectedGroup[0]);
            } else {
                UF.Sheet.close();
            }
        }
    ...
    //-------------------------------------------------------------------------
    // Tile selection state & API
    //-------------------------------------------------------------------------
    let selectedTiles = [];       // Array of { x, y, z, area }
    let selectedTileBox = null;   // { x0, y0, x1, y1, z, area, count }

    function selectSingleTile(x, y, z, area) {
        const curZ = typeof z === "number" ? z : viewZ();
        const W = World();
        const curArea = area || (W ? W.currentArea() : { x: 0, y: 0 });
        selectedTiles = [{ x, y, z: curZ, area: copyArea(curArea) }];
        selectedTileBox = {
            x0: x,
            y0: y,
            x1: x,
            y1: y,
            z: curZ,
            area: copyArea(curArea),
            count: 1
        };
        setStatus(`Tile (${x}, ${y}) selected.`, 4);
        emit("select:tileChanged", { tiles: selectedTiles.slice(), box: Object.assign({}, selectedTileBox) });
    }

    function selectTileRectangle(x0, y0, x1, y1, z, area, opts = {}) {
        ...
        for (let cy = minY; cy <= maxY; cy++) {
            for (let cx = minX; cx <= maxX; cx++) {
                if (!window.$gameMap || $gameMap.isValid(cx, cy)) {
                    newTiles.push({ x: cx, y: cy, z: curZ, area: copyArea(curArea) });
                }
            }
        }

        if (opts.add && selectedTiles.length > 0) {
            const existingKeys = new Set(selectedTiles.map(t => `${t.x},${t.y},${t.z}`));
            ...
            selectedTileBox = {
                ...
                z: curZ,
                area: copyArea(curArea),
                count: newTiles.length
            };
        }

        const totalW = maxX - minX + 1;
        const totalH = maxY - minY + 1;
        setStatus(`Selected ${totalW} × ${totalH} tiles (${selectedTileBox.count} total).`, 4);
        emit("select:tileChanged", { tiles: selectedTiles.slice(), box: Object.assign({}, selectedTileBox) });
    }
    ...
    function clearTileSelection() {
        if (selectedTiles.length > 0 || selectedTileBox) {
            ...
            emit("select:tileChanged", { tiles: [], box: null });
        }
    }

    function isTileSelected(x, y, z, area) {
        if (!selectedTileBox && selectedTiles.length === 0) return false;
        const checkZ = typeof z === "number" ? z : viewZ();
        if (selectedTileBox && selectedTileBox.z === checkZ) {
            const x0 = Math.min(selectedTileBox.x0, selectedTileBox.x1);
            const x1 = Math.max(selectedTileBox.x0, selectedTileBox.x1);
            const y0 = Math.min(selectedTileBox.y0, selectedTileBox.y1);
            const y1 = Math.max(selectedTileBox.y0, selectedTileBox.y1);
            if (x >= x0 && x <= x1 && y >= y0 && y <= y1) return true;
        }
        return selectedTiles.some(t => t.x === x && t.y === y && t.z === checkZ);
    }

    function getSelectedTiles() {
        return selectedTiles.slice();
    }

    function getSelectedTileBox() {
        return selectedTileBox ? Object.assign({}, selectedTileBox) : null;
    }
    ...
    function syncOverseerSelection(options = {}) {
    ...
    function queueCommit(toolDef, box) {
        ...
        if (J && typeof J.list === "function") {
            ...
            for (const j of list) {
                ...
                if (jArea && box && box.area && jArea.x === box.area.x && jArea.y === box.area.y && jZ === box.z) {
    ...
    function processCommits(budgetMs) {
        ...
        while (commitQueue.length > 0) {
            ...
            while (c.curY <= c.y1) {
                if (!isBigRectProvoked && processedCells >= minCells && (performance.now() - startTime) >= budgetMs) {
    ...
    function cancelArea(box) {
        ...
        if (st && st.zones) {
            for (let i = st.zones.length - 1; i >= 0; i--) {
                const z = st.zones[i];
                if (z.area && box && box.area && z.area.x === box.area.x && z.area.y === box.area.y && (z.z || 0) === box.z) {
    ...
    function groupMove(target) {
        const rawUnits = getSelectedUnits();
        ...
        const units = rawUnits.filter(u => {
            if (I && typeof I.encumbrance === "function") {
                ...
                if (enc && enc.status === "over_capacity") return false;
            }
            return true;
        });
        if (units.length < rawUnits.length) {
            SoundManager.playBuzzer();
            if (!units.length) {
                setStatus("Over capacity: Cannot move until load is reduced!");
                return;
            }
        }
    ...
    function updatePreview(budgetMs) {
        ...
        if (!tDef) {
            ...
            for (const u of units) {
                if (u.area && activeBox && activeBox.area && u.area.x === activeBox.area.x && u.area.y === activeBox.area.y && (u.z || 0) === activeBox.z) {
    ...
    class Sprite_UFSelectOverlay extends Sprite {
        ...
        redraw() {
            ...
            // Selected tile or group of tiles
            this.drawTileSelection(b);
            ...
            let countText = "";
            if (!tDef) countText = `${totalW * totalH} tiles${previewCount > 0 ? `, ${previewCount} units` : ""}`;
        ...
        drawTileSelector(b) {
            ...
            // Targeted tile destination: square brackets [ ]
        ...
        drawTileSelection(b) {
            if (!window.$gameMap || !window.$dataMap) return;
            const tileBox = getSelectedTileBox();
            const tiles = getSelectedTiles();
            if (!tileBox && (!tiles || tiles.length === 0)) return;

            const z = window.UF.Camera ? UF.Camera.zoom() : 1;
            const tileSize = 48 * z;

            if (tileBox) {
                const x0 = Math.min(tileBox.x0, tileBox.x1);
                const x1 = Math.max(tileBox.x0, tileBox.x1);
                const y0 = Math.min(tileBox.y0, tileBox.y1);
                const y1 = Math.max(tileBox.y0, tileBox.y1);
                const isSingle = (x0 === x1 && y0 === y1);

                const sx0 = Math.round($gameMap.adjustX(x0) * tileSize);
                const sy0 = Math.round($gameMap.adjustY(y0) * tileSize);
                const sx1 = Math.round(($gameMap.adjustX(x1) + 1) * tileSize);
                const sy1 = Math.round(($gameMap.adjustY(y1) + 1) * tileSize);
                const sw = sx1 - sx0;
                const sh = sy1 - sy0;
                ...
                if (sx1 > 0 && sy1 > 0 && sx0 < Graphics.width && sy0 < Graphics.height) {
                    if (isSingle) {
                        ...
                        // Dimension / tile count badge at bottom-right or top-right
                        const totalW = x1 - x0 + 1;
                        const totalH = y1 - y0 + 1;
                        const tileCount = totalW * totalH;
                        this.drawSelectionBadge(b, sx1, sy1, totalW, totalH, tileCount);
                    }
                }
            }
        }
        ...
        drawGroupCornerBrackets(b, x, y, w, h, z) {
            ...
            // Top-Left corner: ┌
            b.fillRect(x - 1, y - 1, arm + 2, thick + 2, shadowColor);
            b.fillRect(x - 1, y - 1, thick + 2, arm + 2, shadowColor);
            b.fillRect(x, y, arm, thick, bracketColor);
            b.fillRect(x, y, thick, arm, bracketColor);

            // Top-Right corner: ┐
            b.fillRect(x + w - arm - 1, y - 1, arm + 2, thick + 2, shadowColor);
            b.fillRect(x + w - thick - 1, y - 1, thick + 2, arm + 2, shadowColor);
            b.fillRect(x + w - arm, y, arm, thick, bracketColor);
            b.fillRect(x + w - thick, y, thick, arm, bracketColor);

            // Bottom-Left corner: └
            b.fillRect(x - 1, y + h - thick - 1, arm + 2, thick + 2, shadowColor);
            b.fillRect(x - 1, y + h - arm - 1, thick + 2, arm + 2, shadowColor);
            b.fillRect(x, y + h - thick, arm, thick, bracketColor);
            b.fillRect(x, y + h - arm, thick, arm, bracketColor);

            // Bottom-Right corner: ┘
            b.fillRect(x + w - arm - 1, y + h - thick - 1, arm + 2, thick + 2, shadowColor);
            b.fillRect(x + w - thick - 1, y + h - arm - 1, thick + 2, arm + 2, shadowColor);
            b.fillRect(x + w - arm, y + h - thick, arm, thick, bracketColor);
            b.fillRect(x + w - thick, y + h - arm, thick, arm, bracketColor);
        }

        drawSelectionBadge(b, rx, ry, cols, rows, count) {
            const badgeW = 95;
            const badgeH = 22;
            let bx = rx - badgeW;
            let by = ry + 4;
            if (by + badgeH > Graphics.height - 4) by = ry - badgeH - 4;
            if (bx < 4) bx = 4;
    ...
    class Sprite_UFSelectToolbar extends Sprite {
        ...
        updatePosition() {
            ...
            this.x = Math.max(8, Math.floor((gw - this.width) / 2));
            this.y = gh - 38;
        }
        redraw() {
            const b = this.bitmap;
        ...
        drawButton(idx, toolDef, glyph, keyLetter, isActive) {
            ...
            // Large center key letter
            b.fontSize = 13;
            b.textColor = isActive ? "#ffffff" : (toolDef ? toolDef.color : "#94a3b8");
            b.drawText(glyph, x, y - 2, w, h - 6, "center");

            // Small bottom-right key label
            b.fontSize = 9;
            b.textColor = "#64748b";
            b.drawText(keyLetter, x + w - 10, y + h - 10, 8, 8, "right");
        }
        handleClicks() {
            if (TouchInput.isTriggered()) {
                ...
                const btnIdx = Math.floor((lx - 2) / 28);
                const cfg = getConfig();
                if (btnIdx === 0) {
                    setTool(null);
                    SoundManager.playCursor();
                } else if (btnIdx >= 1 && btnIdx <= cfg.tools.length) {
                    const chosen = cfg.tools[btnIdx - 1];
                    if (chosen.id === "wall") {
                        handleWallToolTrigger();
                    } else {
                        setTool(activeTool === chosen.id ? null : chosen.id);
                    }
                    SoundManager.playCursor();
                }
                // Consume click
    ...
    function handleWallToolTrigger() {
        ...
        const wy = scene._ufSelectToolbar ? Math.max(10, scene._ufSelectToolbar.y - 128) : 80;
        const rect = new Rectangle(scene._ufSelectToolbar ? scene._ufSelectToolbar.x + 180 : 300, wy, 260, 120);
        ...
        sync() {
            ...
            for (const u of selectedUnits) {
                ...
                if (Stance() && typeof Stance().placeSelection === "function") {
                    ...
                    s.x = ev.screenX();
                    s.y = (typeof ev.screenY === "function" ? ev.screenY() : 0) - 12;
                }
                const chZ = (cs && typeof cs.z === "number") ? cs.z : (typeof ev.screenZ === "function" ? ev.screenZ() : (typeof ev.screenY === "function" ? ev.screenY() : 0));
                s.z = chZ - 5; // Strictly behind character sprite
                spriteIdx++;
            }
                ...
                if (!syncingOverseer && colonist && typeof colonist === "object") {
                    if (!selectedGroup.includes(colonist.id)) {
                        ...
                        if (window.UF && UF.Sheet && typeof UF.Sheet.open === "function") {
                            UF.Sheet.open(colonist.id);
                        }
                    }
                }
            };
            const _cm_deselect = cmProto.deselect;
            cmProto.deselect = function(options = {}) {
                _cm_deselect.call(this, options);
                if (!syncingOverseer) {
                    selectedGroup = [];
                    emit("select:changed", []);
                    if (!options || !options.keepSheet) {
                        const sub = window.UF && UF.Sheet && typeof UF.Sheet.subject === "function" ? UF.Sheet.subject() : null;
                        if (!sub || sub.kind === "unit") {
                            if (window.UF && UF.Sheet && typeof UF.Sheet.close === "function") {
                                UF.Sheet.close();
                            }
                        }
    ...
    Scene_Map.prototype.update = function() {
        ...
        if (!isMapBusy()) {
            ...
            if (Input.isTriggered("escape")) {
                if (gestureState === "dragging" || activeBox) {
                    ...
                    Input._latestButton = null;
                } else if (selectedGroup.length > 0 || selectedTiles.length > 0 || selectedTileBox) {
                    clearSelection();
                    clearTileSelection();
                    Input._latestButton = null;
                }
        ...
        if (gestureState === "dragging" && !TouchInput.isPressed()) {
            ...
            if (activeTool) {
                ...
                const u = findUnitAt(p.mx, p.my, p.area, p.z, p.sx, p.sy);
                const hasUnitSelected = (selectedGroup.length > 0) || (window.$colonyManager && !!$colonyManager.selectedColonist);
                ...
                if (u) {
                    if (isPlayerUnit(u)) {
                        ...
                        if (window.$colonyManager && typeof $colonyManager.deselect === "function") {
                            $colonyManager.deselect({ keepSheet: true });
                        }
                    }
                    clearTileSelection();
                    clearTargetedTile();
                    ...
                    const C = window.UF && UF.Containers;
                    const O = window.UF && UF.Objects;
                    const obj = O ? (O.atIn ? O.atIn(p.area, p.mx, p.my) : O.at(p.mx, p.my)) : null;
                    const cont = C ? C.at(p.area, p.mx, p.my, p.z) : null;
                    const isChest = (obj && (obj.id === "chest_wood" || (C && C.isContainerType && C.isContainerType(obj.id)))) || cont;

                    if (isChest && !hasUnitSelected) {
                        clearTileSelection();
                        clearTargetedTile();
                        if (window.UF && UF.Sheet && typeof UF.Sheet.close === "function") {
                            UF.Sheet.close();
                        ...
                        if (C && typeof C.openChestInfo === "function") {
                            C.openChestInfo(p.mx, p.my, p.area, p.z);
                        }
                        SoundManager.playOk();
                        return;
                    }

                    if (SceneManager._scene && SceneManager._scene._ufContainerCard && SceneManager._scene._ufContainerCard.visible && !isChest) {
                        SceneManager._scene._ufContainerCard.close();
                    }

                    if (window.UF && UF.Sheet && typeof UF.Sheet.openAt === "function" && UF.Sheet.openAt(p.mx, p.my)) {
                        clearSelection({ keepSheet: true });
                        if (window.$colonyManager && typeof $colonyManager.deselect === "function") {
                            $colonyManager.deselect({ keepSheet: true });
                        }
                    ...
                    } else if (selectedTiles.length > 0 || selectedTileBox !== null) {
                        clearTileSelection();
                        clearTargetedTile();
                        SoundManager.playCancel();
                    } else {
        ...
        const tCommit = performance.now() - tc0;
        ...
        const preMs = t0 - frameStart;
        const postMs = performance.now() - tPostStart;
        const elapsed = preMs + postMs;
        perfStats.frames++;
        if (elapsed > perfStats.worstFrameMs) {
            perfStats.worstFrameMs = elapsed;
            perfStats.worstBreakdown = {
                tPre: +preMs.toFixed(2),
                tCore: +tCore.toFixed(2),
                tPreview: +tPreview.toFixed(2),
                tCommit: +tCommit.toFixed(2),
                selectElapsed: +elapsed.toFixed(2)
            };
        }
    };
    ...
    function commitActiveBox() {
        ...
        if (!tDef) {
            ...
            for (const u of allUnits) {
                if (u.area && box && box.area && u.area.x === box.area.x && u.area.y === box.area.y && (u.z || 0) === box.z) {
                    if (u.x >= x0 && u.x <= x1 && u.y >= y0 && u.y <= y1 && isPlayerUnit(u)) {
                    ...
                    if (gestureState === "dragging" || activeBox) {
                        ...
                        if (units.length > 0) {
                            ...
                            if (!overUI && window.$gameMap && $gameMap.isValid(mx, my)) {
                                ...
                                const O = window.UF && UF.Objects;
                                const u = units[0];
                                const obj = O ? (O.atIn ? O.atIn(area, mx, my) : O.at(mx, my)) : null;
                                const cont = C ? C.at(area, mx, my, viewZ()) : null;
                                const isChest = (obj && (obj.id === "chest_wood" || (C && C.isContainerType && C.isContainerType(obj.id)))) || cont;
                                if (isChest && C && typeof C.useChest === "function") {
                                    C.useChest(u, cont || obj, mx, my);
                                    if (selectedTiles.length > 0 || selectedTileBox) {
                                        clearTileSelection();
                                    }
                                    TouchInput._currentState = Object.assign({}, TouchInput._currentState, { cancelled: false });
    ...
    function handleCompleteClick(scene, mx, my) {
        ...
        if (activeTool) {
            ...
            if (u) {
                ...
                const O = window.UF && UF.Objects;
                const obj = O ? (O.atIn ? O.atIn(area, mx, my) : O.at(mx, my)) : null;
                const cont = C ? C.at(area, mx, my, curZ) : null;
                const isChest = (obj && (obj.id === "chest_wood" || (C && C.isContainerType && C.isContainerType(obj.id)))) || cont;
                ...
                if (isChest && !hasUnitSelected) {
                    ...
                    if (C && typeof C.openChestInfo === "function") {
                        C.openChestInfo(mx, my, area, curZ);
                    }
                    SoundManager.playOk();
                    TouchInput._currentState = Object.assign({}, TouchInput._currentState, { triggered: false });
                    return;
                ...
                if (window.UF && UF.Sheet && typeof UF.Sheet.openAt === "function" && UF.Sheet.openAt(mx, my)) {
                    clearSelection({ keepSheet: true });
                    if (window.$colonyManager && typeof $colonyManager.deselect === "function") {
                        $colonyManager.deselect({ keepSheet: true });
                    }
    ...
    const SelectAPI = {
        ...
        hasSelection: () => selectedGroup.length > 0,
        primary: primaryUnit,
        setSelection,
        clearSelection,
        tool: () => activeTool,
        ...
        selectedTileBox: getSelectedTileBox,
        selectSingleTile,
        selectTileRectangle,
        clearTileSelection,
        isTileSelected,
    ...
    function registerSelectChecks() {
        ...
        UF.Test.suite("select", async t => {
            ...
            async function pressKey(keyCode) {
                if (typeof Input !== "undefined" && typeof Input._onKeyDown === "function") {
                    Input._onKeyDown({ keyCode, preventDefault: () => {} });
                }
                const eDown = new KeyboardEvent("keydown", { bubbles: true, cancelable: true });
                ...
                if (typeof Input !== "undefined" && typeof Input._onKeyUp === "function") {
                    Input._onKeyUp({ keyCode, preventDefault: () => {} });
                }
                const eUp = new KeyboardEvent("keyup", { bubbles: true, cancelable: true });
                Object.defineProperty(eUp, "keyCode", { get: () => keyCode });
                Object.defineProperty(eUp, "which", { get: () => keyCode });
            ...
            await t.waitFrames(3);
            const selAfterGroundClick = SelectAPI.selected();

            t.check("select.plain_click", !isProvoked("plain_click") && slowSel.length === 1 && slowSel[0] === col1.id && selAfterGroundClick.length === 0,
                `Plain click: slow select=${slowSel.length === 1 && slowSel[0] === col1.id}, ground click deselects=${selAfterGroundClick.length === 0}`);
            ...
            groupMove({ area, x: tx, y: ty, z: viewZ() });
            await t.waitFrames(3);
            ...
            t.check("select.big_rect_frame_time", !isProvoked("big_rect_frame_time") && worstMs <= 25,
                `Big rectangle frame time: worst frame=${worstMs.toFixed(2)} ms (budget <= 25 ms in test harness); breakdown=${JSON.stringify(SelectAPI.stats().worstBreakdown)}`);
            ...
            t.check("select.tile_hover_selector", !isProvoked("tile_hover_selector") && hoverAlpha === 0,
                `Hover tile selector removed: alpha at (${hp.x},${hp.y})=${hoverAlpha} (want 0, no hover square beneath cursor)`);
            ...
            const stX = x0 + 5, stY = y0 + 5;
            await mouseClick(stX, stY);
            await t.waitFrames(3);
            ...
            const deselBoxRight = SelectAPI.selectedTileBox();
            t.check("select.tile_deselect_right_click", deselTilesRight.length === 0 && deselBoxRight === null,
                `Right-click deselects tile group: tiles=${deselTilesRight.length}, box=${deselBoxRight}`);

            // 20. Check: select.tile_deselect_esc
            SelectAPI.selectTileRectangle(gx0, gy0, gx1, gy1);
            await t.waitFrames(2);
            await pressKey(27); // Escape key
            await t.waitFrames(3);
            const deselTilesEsc = SelectAPI.selectedTiles();
            const deselBoxEsc = SelectAPI.selectedTileBox();
            t.check("select.tile_deselect_esc", deselTilesEsc.length === 0 && deselBoxEsc === null,
                `Esc key deselects tile group: tiles=${deselTilesEsc.length}, box=${deselBoxEsc}`);

            // 21. Check: select.tile_deselect_left_click
            SelectAPI.selectTileRectangle(gx0, gy0, gx1, gy1);
            await t.waitFrames(2);
            // Left click elsewhere on empty ground
            await mouseClick(gx0 + 8, gy0 + 8, { button: 0 });
            await t.waitFrames(3);
            const deselTilesLeft = SelectAPI.selectedTiles();
            const deselBoxLeft = SelectAPI.selectedTileBox();
            t.check("select.tile_deselect_left_click", deselTilesLeft.length === 0 && deselBoxLeft === null,
                `Left-click elsewhere deselects dragged box: tiles=${deselTilesLeft.length}, box=${deselBoxLeft}`);

            // 22. Check: select.wall_tool_button_click
            const tbWidget = SceneManager._scene._ufSelectToolbar;
            const wallBtnX = tbWidget.x + 2 + 9 * 28 + 14;
            const wallBtnY = tbWidget.y + 14;
            TouchInput._x = wallBtnX;
            TouchInput._y = wallBtnY;
            TouchInput._currentState = Object.assign({}, TouchInput._currentState, { triggered: true });
            tbWidget.handleClicks();
            await t.waitFrames(3);

            const wpWin = SceneManager._scene._ufWallPickerWindow;
            const wpOpened = !!wpWin && wpWin.visible;
            const toolActive = SelectAPI.tool() === "wall";
            t.check("select.wall_tool_button_click", wpOpened || toolActive,
                `Wall tool button clicked on toolbar: wall picker opened=${wpOpened}, tool=${SelectAPI.tool()}`);

            // Close picker
            if (wpWin) {
                SceneManager._scene.removeChild(wpWin);
                wpWin.destroy();
                SceneManager._scene._ufWallPickerWindow = null;
            }

            // 23. Check: select.no_errors
