//=============================================================================
// UF_Interact.js - Right-click context menu: everything that can be done on a cell becomes a designation
//=============================================================================

/*:
 * @target MZ
 * @plugindesc [UF Interact] Right-click any cell for a menu of what can be done there; each choice is an open job (a designation) with a marker on the cell. Adds the dismantle, dig and fish job types.
 * @author UF project
 * @base UF_World
 * @base UF_Jobs
 * @orderAfter UF_Look
...
(() => {
    ...
    function optionsFor(x, y) {
        ...
        if (onCell.length) {
            ...
            const food = onCell.find(it => I.type(it.type) && I.type(it.type).food);
            if (food) add("eat", `Eat ${lower(I.type(food.type).name)}`, () => personalJob("eat", target, { itemId: food.id }));
        }
        ...
        if (water) {
            ...
            add("drink", "Drink here", () => personalJob("drink", target));
        }
