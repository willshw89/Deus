...
| **WG.00.04** | Biome Identity Standard | Gemini | 5 canonical biomes (`TEMP`, `WET`, `ARID`, `HIGH`, `VOLC`), physical-Z hooks, 10 transitions. (Note: superseded by DEC-030) | `DONE` (DW.01.04) | |
...
| **WG.00.17** | Z-Range Configurable Setting & Expansion to 32 Layers | Claude | Make Z-range a single configurable setting in engine core, then expand to 32 layers (-16..+15: surface 0, 16 underground -1..-16, 15 upper +1..+15, 320 ft total height, 10 ft layers, 2 ft strata). Refactor hardcoded spots (DEUS_Levels.js L1137 fixed maps, L1805 +2 offset/slice cap 24, all +2/5/24 assumptions). Mandatory sparse storage: memory and save size scale with occupied cells, not 32 × area (empty sky and solid rock cost near zero). Must maintain support for running at 9 layers in automated tests. Depends on Lanes K and N merging; inputs: ADR-003. Notes: natural terrain is capped at +11; +12..+15 (40 ft) is reserved open build air. Engine range stays -16..+15. Lane AA is live, so do not change its scope. Do NOT open code lane yet. | `PLANNED` | |
| **WG.00.18** | Layer-View Presentation (Owner-Led) | Owner / Claude | Depth presentation (lighting, fog, atmosphere, colour grading of lower layers, parallax or none) designed directly with Owner after Lanes K, N, and WG.00.17 land. Governed by visual goal "looking down layers must be beautiful and breathtaking". DEC-011 governs until opened with Owner. Explicit requirement: visible lower layers show all effects, HP bars, status indicators, and combat/spell overlays at 1:1 scale with zero filters (no blur, tint, fog, desaturation, or scaling; Owner 2026-09-26 11:31 CT; DEC-011). The camera follows the controlled unit's layer while it walks across slopes; no fade, pause, or load (Owner requirement 2026-09-26 12:12 CT; DEC-020). | `PLANNED` | |
| **WG.00.19** | In-Layer Height Presentation & Multi-Strata Movement | Claude / Grok | Characters/objects drawn raised by fixed pixel offset per stratum of ground height (straight shift, no scale; DEC-011/DEC-016 compliant). Movement: 1 stratum (2 ft) normal step, 2 strata (4 ft) climb/jump, full layer (10 ft) requires stairs/ladder/ramp. Falling damage, melee reach, LOS use real height differences. Cross-reference WG.00.20: multi-layer hills and continuous slopes span several Z layers with units walking straight up without stairs/loading (DEC-020). | `PLANNED` | |
| **WG.00.20** | Seamless Inter-Layer Ramps & Camera-Follow Connector | Claude / Grok | Run of cells rising one stratum per cell (5 cells = one 10 ft layer); unit's Z becomes Z+1 at top stratum with zero transfer/fade/pause. Camera follows player automatically (default Owner decision). Non-player units transfer layer memberships. Multi-Z pathfinding traversable connector. Colonist construction. A hill can span several Z layers, and units walk straight up it with no stairs, no transfer, and no loading. Terrain rises continuously across layer boundaries via slope and ramp tiles between Z layers (multi-layer hills, not only single 5-cell ramps). Pathfinding across Z treats walkable slopes as ordinary path edges between layers. Camera follows the controlled unit's layer while it walks across slopes (no fade, pause, or load). Worldgen terrain generation must produce multi-layer hills and slopes. (DEC-020; Owner requirement 2026-09-26 12:12 CT; dep: WG.00.17, WG.00.19, SIM.00.00). | `PLANNED` | |
...
| **WG.00.22** | Multi-Depth Presentation Catalogue Slots & Asset Placeholders | Gemini / Codex | Catalogue entries and blank tile/sprite slots for depth cues 1–6: visible inner side walls of openings, rim shadows cast onto lower layers, darker baked tile palettes, height edges and ramps, hanging/falling props (roots, vines, stalactites, waterfalls, dust, light shafts), deep light sources against darkness. Slope and ramp slots between Z layers flagged per terrain/material family (up/down/corners/edges; slots only, no art under DEC-007; Directive 0090-CM). Data manifests and blank slots only; zero art generation (DEC-007; DEC-011 amendment). | `PLANNED` | |
| **WG.00.23** | Overlook Zoom-Out Multi-Layer View (Owner-Led Placeholder) | Owner / Claude | Future overlook zoom-out view where units on multiple layers are visibly moving. Sequenced post-K, post-N, post-32-layer refactor, and post-benchmark headroom proof. Simplified/low-detail sprites and capped animation rates at far zoom. DEC-011 1:1 scale lock governs now. Explicit requirement: visible lower layers show overlays, effects, HP bars, and status indicators at 1:1 scale with zero filters (Owner 2026-09-26 11:31 CT; DEC-011). | `PLANNED` | |
| **WG.00.24** | Custom Multi-Layer PixiJS Map Renderer (Fallback) | Owner / Claude | Benchmark-gated fallback map renderer inside RMMZ Scene_Map. Replaces stock Spriteset_Map/Tilemap if stock renderer cannot meet 32 layers, occlusion culling, and 1:1 flat rendering within frame budget. Menus, dialogue, save, database, and battle remain stock RMMZ. Gated on Lane K normal + 0019-T stress baselines, §18 occlusion benchmark, and Owner go/no-go. Zero code opened before benchmarks. (DEC-017; Directive 0028-AC §4). Explicit requirement: must render lower-layer effects, HP bars, status indicators, and combat/spell overlays at 1:1 with zero filters (Owner 2026-09-26 11:31 CT; DEC-011). Camera follows controlled unit's layer across multi-layer slopes without fade, pause, or load (DEC-020). | `PLANNED` | |
| **WG.00.25** | Coarse Whole-World Generation | Claude / Grok | 3x3 grid of 256x256 maps (approx 768x768 tiles). Deterministic per seed. Grid size is data-driven (extensible to 4x4 later). No art. Dep: WG.00.17, WG.00.15. | `PLANNED` | |
| **WG.00.26** | Map Wrap Stitching & Seam Test | Claude / Grok | 3x3 wrap stitching on all edges and corners (round world). Headless multi-seed seam test ensures consistency. No art. Dep: WG.00.25. | `PLANNED` | |
| **WG.00.27** | Seamless Map-Edge Transitions | Claude / Grok | Transitions between map grids reuse Lane N's in-place swap, zero load screen. No art. Dep: WG.00.26, Lane N. | `PLANNED` | |
| **WG.00.28** | Per-Map LOD Integration | Claude / Grok | Current map runs at full detail, the other 8 run at ADR-003 LOD summary level. No art. Dep: WG.00.25, SIM.00.01 (ADR-003). | `PLANNED` | |
| **WG.00.29** | Depth Band Physical Rules Doc | Owner / PM | Doc defining physical rules for 5 depth bands (Deep Earth, Caverns, Lowlands, Uplands, Highlands). Owner review required. | `PLANNED` | |
| **WG.00.30** | Geology-First Worldgen Layer | Claude / Grok | Geology-first worldgen layer on the coarse pass. Dep: WG.00.17. | `PLANNED` | |
| **WG.00.31** | Biome Placement Rule Engine | Claude / Grok | Rule engine for placing 6 biomes across 4 vertical link types. Dep: WG.00.29, WG.00.30. | `PLANNED` | |
| **WG.00.32** | Six Biome Stack Sheets | Owner / Gemini | Text-only sheets for the 6 biome stacks (VOLCANIC, WET, ARID, TEMPERATE, COLD, WILD) across 5 depth bands. Awaiting Owner approval. | `PLANNED` | |
| **WG.00.33** | Headless Multi-Seed Worldgen Test | Claude / Grok | All 30 combinations reachable, zero natural terrain at +12..+15, seams consistent. Dep: WG.00.30, WG.00.31. | `PLANNED` | |
| **WG.00.34** | Catalogue Regeneration (30 Biomes + 15 Transitions) | Gemini / Grok | Regenerate catalogue data for 6x5 biome/depths plus 15 transitions. Data only. Dep: WG.00.32 (Owner approval). | `PLANNED` | |
| **WG.00.35** | Lower-Layer Overlay Rendering (Effects/HP/Status/Combat & Spell Overlays at 1:1) | Claude / Grok | When the player looks down through layers, every visible lower layer renders its effects, HP bars, status indicators, and combat/spell overlays at 1:1 scale with zero filters (no blur, tint, fog, desaturation, or scaling applied; DEC-011; Owner requirement 2026-09-26 11:31 CT). Follow-up to Lane AA. Dep: WG.00.17. | `PLANNED` | |
| **WG.00.36** | Cross-Layer Multi-Unit Selection & Group Orders | Claude / Grok | Multi-unit selection and group orders operate across layers: player can select units on several layers at once and give them orders together (box-select through visible lower layers, modifier-click/box multi-layer selection). Selected units and group orders keep working while units cross layers on slopes (selection is not dropped at a layer change; Owner requirement 2026-09-26 12:12 CT; DEC-020). Dep: WG.00.17, WG.00.35. (Owner requirement 2026-09-26 11:32 CT; DEC-011). | `PLANNED` | |
| **WG.00.37** | Recruitment of Non-Core Humanoids (Persuade/Recruit & Shared Paper-Doll Templates) | Claude / Grok | Non-core humanoids (11 extra face cultures beyond the 9 SRD races: goblin, orc, kobold, etc.) can be persuaded or recruited into the party. They receive full paper-doll bodies on shared body templates (same layer and anchor rules as the 9 races). Monstrous types (undead, elementals, etc.) stay fixed-sprite (no paper-doll). Dep: WG.00.17, WG.20.01. (Owner requirement 2026-09-26 12:59 CT; DEC-033). | `PLANNED` | |
| **WG.00.38** | Capture, Prisoners & Domestication (Pets, Mounts, Livestock & Work Animals) | Claude / Grok | Enemies and wildlife can be captured and tamed into pets, mounts, livestock, and work animals. Captured humanoids become prisoners or recruits. Dep: WG.00.17, SIM.40.10. (Owner requirement 2026-09-26 12:59 CT; DEC-033). | `PLANNED` | |
| **WG.00.39** | Tamed Creatures in Party Combat (SRD 5.1 Stat Blocks & Natural Attacks Only; No Creature Gear) | Claude / Grok | Tamed creatures fight in the party using only their basic SRD 5.1 stat blocks and natural attacks and defenses. There are NO creature armor or equipment slots, no barding, and no crafted creature gear. A riding saddle is a visual marker only (no equipment slot, no mechanical stats). Dep: WG.00.17, SIM.60.05. (Owner requirement 2026-09-26 13:01 CT; DEC-033). | `PLANNED` | |
...
| **WG.22.01–25**| 25 Biome/Z Environment Catalogues | Gemini | Explicit catalogue for each of the 25 combinations (5 biomes $\times$ 5 macro-Z); incorporates biome-specific natural states (VOLC geothermal, HIGH snowpack, WET saturated soil, ZM karst). FLAG FOR REBUILD: 30 catalogues (6x5). | `PLANNED` |
...
| **WG.24.01–10**| 10 Horizontal Transition Catalogues | Gemini | Explicit blend assets for all 10 pairwise biome transitions; accounts for natural boundaries (wet $\rightarrow$ dry soil, burned $\rightarrow$ regrowing forest, river $\rightarrow$ floodplain, snow $\rightarrow$ bare). FLAG FOR REBUILD: 15 pairwise transitions (6 choose 2). | `PLANNED` |
| **WG.25.01** | Vertical Cliff & Ramp Transition Catalogue | Gemini | Step-down ramps, natural stair formations, vertical cliff faces, cliff seepages, wet rock, karst sinkholes, and cave mouths across all 5 strata. Includes slope and ramp slots between Z layers per terrain/material family (up/down/corners/edges; slots only, no art under DEC-007; Directive 0090-CM). | `PLANNED` |
...
| WBS Leaf | Title | Owner | Scope & Deliverables | Status |
| :--- | :--- | :---: | :--- | :---: |
| **WG.50.01** | Temperate Z0 Ground & Cliff Golden Pack | Gemini | Core loam, fertile grass, limestone cliffs, dirt road, fieldstone. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.50.02** | Temperate Z0 Flora & Silviculture Golden Pack | Gemini | Common Oak, White Birch, shrubs, reeds, wildflowers, stumps, logs. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.50.03** | Temperate Z0 Geology & Scatter Golden Pack | Gemini | Limestone boulders, fieldstone clusters, flint pebbles, loose rubble. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.51.01–04**| Remaining Z0 Biome Packs (WET, ARID, HIGH, VOLC)| Gemini | Complete visual packs for Wetland, Arid, Highland, and Volcanic surface Z0. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.52.01–10**| 10 Horizontal Pairwise Transition Packs | Gemini | Boundary tiles and transitional scatter across all 10 biome borders. FLAG FOR REBUILD: rebuilt to 15 pairwise transitions (6 choose 2). Art stays Owner-gated. | `PLANNED` |
| **WG.53.01–02**| Positive-Z Alpine & Canopy Packs (Z+1, Z+2) | Gemini | Dwarf pines, scree, alpine meadows, jagged granite crags, cloud margins. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.54.01–02**| Negative-Z Cavern & Deep Packs (Z-1, Z-2) | Gemini | Subterranean stalagmites, damp slate, basalt floors, magma hazard shores. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.55.01** | All-Z Physical Cave & Rupture Pack | Gemini | Cave entrances, physical rock arches, rock breaches, vertical shafts. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.56.01** | Partial-Height Terrain & Micro-Relief Pack | Gemini | 1/5, 2/5, 3/5, 4/5 step-down ledges, eroded gullies, drainage ditches. Flagged for slope and ramp slot definitions between Z layers (slots only, no art under DEC-007; Directive 0090-CM). FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
| **WG.57.01** | Fluid Depth & Animated Hazard Pack | Gemini | Multi-depth water, shoreline foam, sulfur vents, boiling mud, flowing lava. FLAG FOR REBUILD: rebuilt to 6x5 plus 15 pairs. Art stays Owner-gated. | `PLANNED` |
...
| **WG.62.02** | Race Home-Layer Assignment in WorldGen | Fable | Procedural spawn placement and native habitat generation for 9 races across 32 Z layers. Must lie within -16..+11. 5 vertical biome bands (Deep Earth -16..-11, Caverns -10..-5, Lowlands -4..+1, Uplands +2..+6, Highlands +7..+11). Soft home layer ranges with cross-layer travel/trade/war. Depends on WG.00.17 and race design doc. | `PLANNED` |
...
| **WG.68.02** | Biome-Specific Succession Pathways | Fable | Distinct ecological succession sequences tailored for Temperate, Wetland, Arid, Highland, and Volcanic biomes. (Note: superseded by DEC-030) | `PLANNED` |
...
| OPS.10.04 | **merge_gate LF clones** (update `tools/governance/merge_gate.js` to clone repositories using `-c core.autocrlf=false` with regression test on autocrlf=true) | PLANNED | Lane Y escalation; PM ruling 10:40 | — | Claude → Grok | `merge_gate.js` successfully clones using core.autocrlf=false, and regression tests pass on an autocrlf=true environment | S | — |

#### M0.3 Worker pipeline (OPS.20)

| ID | Title | Status | Source | Depends on | Writer → Reviewer | Definition of done | Size | Gate |
|---|---|---|---|---|---|---|---|---|
| OPS.20.01 | **Standard worker launcher** `tools/ops/launch_worker.ps1` (planned Lane J) | PLANNED (Merged `62067d1c`; review `tasks/WG.00.12/lane-j/review_grok_5a1ec388.md` by grok) | [OPS] R8, §4 Flow step 2 | — | Claude → Grok | The launcher: refuses an empty or missing BRIEF; writes a non-empty `logs/<lane>/<runid>.jsonl` (stream-json); writes an exit-code file; enforces a hard timeout on a `sleep` fixture; heartbeats on log mtime; writes the `active_workers.json` entry at start and at exit; states the headless bypass flag explicitly. Grok mutants (timeout removed, registry not written) are caught | M | — |
...
| OPS.30.06 | **Catalogue test repair on main** (repair `test_catalogue.js` 46/47 and `build_catalogue.js --check` failures on main due to pre-existing catalogue pin) | PLANNED | fb5391fa, 176bffe4 | — | Grok → Gemini | Both tools run and exit 0; NO ART generated; catalogue data/tooling only | S | — |
...
| OPS.70.02 | Secrets scanner and dependency checker | IN_REVIEW | Phases 11E and 11F; `docs/SECURITY_AND_SECRETS.md`; `docs/DEPENDENCY_POLICY.md` | — | Claude → Grok | A fake key planted in a fixture is detected; the clean tree exits 0 | S | — |
...
| SIM.40.00 | **Material and constructed-strata model** (mass, volume, structural support load limits per material, interaction matrix for stone, wood, soil, metals) | IN PROGRESS | Directive 0062-BK; DEC-028 | — | Claude → Grok | Data-driven material properties catalog in `game/data/` or sim core; mass/support values defined; Grok review artifact approves | M | — |
| SIM.40.01 | **Support model design & blast propagation** (materials, vertical propagation, span limits, cross-layer blast attenuation, V128 natural rock, V133 change-driven). Load-bearing rules, span capacities, and vertical blast propagation | PLANNED (Design Merged `d655c122`; review `tasks/SIM.40.01/lane-q/review_grok_fd256fc5.md` by grok) | Directive 0021-V §6, §12; V137; ADR-003 | WG.00.17, dep: SIM.00.01 | Claude → Grok (attack) | Support model spec in docs/systems/: which materials bear load, downward propagation through strata and Z layers, span limits for floors/bridges/roofs, cross-layer blast damage and material attenuation (applyVolumeDamage), natural rock support (V128). Recomputes only near mutations (V133; zero per-tick full-world scans). Grok review artifact approves | M | — |
...
| SIM.40.05 | **Decay model** (unmaintained structure HP loss by material/exposure, roofs fail first, walls fail, feeds collapse) | PLANNED (Design Merged `7f07ee48`; review `tasks/SIM.40.05/lane-r/review_grok_9a2908fb.md` by grok) | Directive 0021-V §7; V138; LIFE-001 | SIM.40.01, dep: SIM.00.01 | Claude → Grok | Decay simulation: unmaintained structures lose HP based on material (wood fast, stone slow, metal rusts) and exposure. Roofs fail before walls, feeding into SIM.40.02 collapse. Change-driven, LOD-aware, scheduled in slow ticks. Grok review artifact approves | M | — |
...
| SIM.50.11 | **People-side gap audit** (`tasks/SIM.50.11/gap-audit-people/PEOPLE_GAP_AUDIT.md`). Audit people/colonist systems against living world, needs, and reproduction | DONE | Directive 0035-AJ | dep: SIM.50.01 | Claude → Grok | Audit report mapping people-side mechanics; zero code modifications; Grok review artifact approves | M | — |
| SIM.50.12 | **Living-world rule-breach fix package** (fix LWGA findings F-01 through F-05: rule breach enforcement across 32 layers) | PLANNED | Directive 0062-BK; LWGA | WG.00.17, SIM.50.13, SIM.40.11 | Claude → Grok | Post-merge regression suite and fixes for LWGA findings F-01 through F-05 across 32 layers; automated tests verify compliance; Grok review artifact approves | M | — |
| SIM.50.13 | **Ore sprouting & fluid solver attachment bugfixes** (fix LWGA F-03 loose stones turning into ore, fix F-05/D-4 fluid solver not attaching and flood fill creating water) | PLANNED | Directive 0062-BK; DEC-023, DEC-024 | dep: WG.00.17 | Claude → Grok | Ore never sprouts from loose stone; fluid solver attaches cleanly to DEUS_Fluid; no water creation via flood fill; dropped dep on SIM.50.02 as bug fixes do not require cross-layer water; Grok review artifact approves | M | — |

#### M3.4 Hyper-realistic SRD spell effects (SIM.60). Owner-ordered 2026-09-26 (DEC-018, Directive 0028-AC §5)

| ID | Title | Status (evidence) | Source | Depends on | Writer → Reviewer | Definition of done | Size | Gate |
|---|---|---|---|---|---|---|---|---|
| SIM.60.01 | **SRD spell-effect audit** (`docs/audits/SRD_SPELL_EFFECT_AUDIT.md`, `docs/audits/srd_spell_effect_audit.json`). Classify all 327 spells in `game/data/srd51/spells.json` by world systems needed: heat, force, water, cold, earth, light, life, NONE. Quotes SRD baseline fields, names WBS deps, drafts primitive list. Docs only; no dependencies; Lane P (PM) | DONE (Merged `195f3f26`; review `tasks/SIM.60.01/lane-p/review_grok_c9d1ed86.md` by grok) | Directive 0028-AC §5; DEC-018 | — | Claude → Grok | Exhaustive audit report and 100% JSON classification of all 327 spells; drafts primitive list; zero code modifications; Grok review artifact approves | M | — |
| SIM.60.02 | **Spell-effect schema** (data-driven JSON Schema of reusable primitives: ignite, heat flux, impulse/blast via SIM.40.01, fluid source/sink, temperature/freeze, mass-conserving terrain edit, light, growth/decay) | DONE | Directive 0028-AC §5; DEC-018 | dep: SIM.60.01 | Claude → Grok | Validated JSON Schema in docs/schemas/spells/ and data fixtures; zero per-spell code; Grok review artifact approves | M | — |
...
| SIM.60.07 | **UF.Rules routing for wildlife damage** (route the second damage rule in `DEUS_Wildlife.js:1117-1124` through `UF.Rules`) | PLANNED | Lane AB review MINOR-9; DEC-027 | SIM.60.05, WG.00.17 | Claude → Grok | Wildlife damage subtraction near C.engage is processed via UF.Rules and combat rules engine; verified by automated tests | S | — |
...
| GP.07.01 | Slice 7: Creatures and combat (`DEUS_Combat.js`, Wildlife) | DRAFT | `docs/SLICES.md` §Slice 7 | GP.06.01, WG.68.07–14 | Claude → Grok; Owner approves | as GP.03.01; creature HP bars and combat overlays on visible lower layers render at 1:1 scale with zero filters (DEC-011; Owner requirement 2026-09-26 11:31 CT); selected units and group orders keep working while units cross layers on slopes (selection is not dropped at a layer change; DEC-020). | L | **OWNER-GATED** (Owner slice approval) |
| GP.07.02 | **Cross-layer 3D targeting, ballistics & volume damage** (spells, arrows, thrown items through multi-Z openings, true 3D Euclidean range, vertical impact bonus / upward range penalty, cross-layer volume blast damage) | PLANNED | Directive 0021-V Addendum §20; DEC-019; V148 | dep: WG.00.17, SIM.00.00 (Lane N), SIM.40.01 | Claude → Grok | Spells/ranged attacks target visible cells on lower/upper layers with clear 3D LOS. Range uses 3D Euclidean distance (5 ft squares, 10 ft layers). Impact damage increases with height fallen; shooting upward incurs range penalty. Area blast damage propagates downward through floor materials. Cross-layer targeting reticles, trajectories, and overlays render at 1:1 scale with zero filters (DEC-011; Owner requirement 2026-09-26 11:31 CT). Exits 0 | M | — |
...
| Rev | Date | Change |
|:---:|:---:|:---|
| 32 | 2026-09-26 | Directive 0096-CS: Minted WG.00.37 (Recruitment of non-core humanoids; shared paper-doll templates), WG.00.38 (Capture, prisoners & domestication: pets, mounts, livestock, work animals), and WG.00.39 (Tamed creatures in party combat: SRD 5.1 stat blocks & natural attacks only; no creature gear). Recorded DEC-033. Next free WG.00 is WG.00.40. |
| 31 | 2026-09-26 | Directive 0090-CM: Recorded seamless multi-layer slopes and hills requirement (Owner 12:12 CT; DEC-020). Updated WG.00.18, WG.00.19, WG.00.20, WG.00.22, WG.00.24, WG.00.36, WG.25.01, WG.56.01, SIM.00.04, GP.07.01. Corrected WG.00.20 DEC citation to DEC-020. |
| 30 | 2026-09-26 | Directive 0084-CG: Minted WG.00.36 (Cross-layer multi-unit selection & group orders; Owner requirement 11:32 CT). Scope correction to 894e9a61: removed 1:1 overlay presentation clause from headless leaves SIM.60.03 and live-lane SIM.60.05 (stays on presentation leaves WG.00.18, WG.00.23, WG.00.24, GP.07.01, GP.07.02, and WG.00.35). |
| 29 | 2026-09-26 | Directive 0083-CF: Added Owner requirement (11:31 CT, DEC-011) that visible lower layers render effects, HP bars, status indicators, and combat/spell overlays at 1:1 with zero filters to WG.00.18, WG.00.23, WG.00.24, SIM.60.03, SIM.60.05, GP.07.01, GP.07.02, and minted WG.00.35 (Lower-layer overlay rendering follow-up to WG.00.17). |
