...
| Frame box | People: 16×24 native = **48×72** exported (adults; children smaller); **all animals and monsters 48×48 or 48×72, one cell, whatever their real size** (Shining Force rule, V44); 2-cell trees 96×96; objects: their footprint width × their height, anchor at the bottom-centre of the footprint |
...
- **Grid (FF6-style, revised 2026-09-18 evening):** one PNG per layer, rows = facings in RPG Maker order (S, W, E, N), columns = the animation frames in this fixed order: `stand` (1), `walk` (3: step, stand, step), `work` (3: a swing cycle used for mine/chop/gather/build), `carry` (3: walk with the arms forward/up), `attack` (3: wind-up, strike, recover), `cast` (2), `sleep` (1). 16 columns. Frame size: people **48×72** exported (16×24 native); **every creature and monster 48×48 or 48×72, never bigger** (Shining Force rule, VISION V44: a horse and a troll are one map sprite tall). The anchor (feet at the bottom-centre of the 1×1 footprint) is the same pixel in every frame of every layer. The `attack` and `cast` frames are the combat animation on the map (VISION V45: no battle screen).
