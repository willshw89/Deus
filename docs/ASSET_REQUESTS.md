...
| AR-400 | People: 7 species × male/female, one-tile 48×48 sheets on the AR-600 grid (8 facings for every action: stand, walk, work, carry, attack, cast, hurt, death, idle), plus the age stages of AR-601 | Factions, colonists, arrivals, test units | High | REQUESTED (`$U7_Townsman`/`$U7_Ranger`/`$U7_Guard`/`$U7_Goblin`/`$U7_Skeleton` with per-species tints, and stock `People1` for test units, in use; the placeholder swap moves everyone to stock `People1–4`/`Actor1–3` sheets) |
