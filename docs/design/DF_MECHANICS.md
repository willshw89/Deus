...
- **On the map, no battle screen** (VISION V45): the attack plays as the sprite's own `attack`/`cast` frames (AR-600), one action per beat (V46).

**Resolution: d20 mechanics (user decision 2026-09-18, VISION V47; SRD 5.1, CC-BY-4.0, credited in the game).** Every person and creature has the six ability scores (STR, DEX, CON, INT, WIS, CHA; catalog `stats` per species with seeded variation per unit), hit points (hit dice by species and stage), armor class (10 + DEX modifier + armor + shield), proficiency by skill (fighting skills from the DF-style skill list feed the proficiency bonus), and saving throws. An attack: **d20 + ability modifier + proficiency vs AC**, natural 20 hits and doubles the damage dice, natural 1 misses; damage = the weapon's dice + modifier (unarmed 1 + STR); advantage/disadvantage from flanking, prone, unseen, drafted-mode; ranged attacks use DEX and ammunition; casts use a spell list later (V45 `cast` frames). Q12 answered "yes to all": **DF-style injuries are what the damage does**: each hit lands on a body part (head, torso, arms, legs, hands, feet, weighted by size), and damage above thresholds causes bleeding (HP loss per beat until bandaged), broken bones (the part can't be used: a broken leg halves speed, a broken arm drops the weapon), severed parts (limbs gone for good), and infections later if untreated; HP 0 = unconscious, HP ≤ −max = dead. The earlier draft `UF_DFCombat.js` only generates DF-flavored combat text and is not used by this design.
Medicine (later): rest, bandaging, doctors; setting bones and treating infections.
