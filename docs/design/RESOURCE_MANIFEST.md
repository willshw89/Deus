...
**Status:** phase-1 design input; not loaded by the game
...
| Source set | Mode | Checked rows | What phase 1 covers |
|---|---:|---:|---|
...
The 490 DF rows are parsed from the read-only local raws every time the validator runs. They are not copied into the manifest or the game. Each record must match one mapping rule, the source count must stay exact and the target must be a declared canonical resource.
...
The phase-1 rates are provisional counts per 4,096 compatible cells, not final balance:
...
Those cases must respectively reject an unresolved source target, renewable finite ore and a four-level world.
...
- The DF import presently covers inorganic and plant raws. Creature materials, extracts, fish, eggs, vermin products and other creature-derived roles are represented by canonical families but do not yet have a row-for-row raw audit.
