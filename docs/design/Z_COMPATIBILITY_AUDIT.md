...
New worlds must contain a guaranteed dwarven faction with **starting settlements on both z=-1 and z=-2** (user choice, 2026-09-19). Both settlements share the same faction identity; generating two separate factions or randomly choosing only one of these layers does not satisfy this requirement. Dwarven founding locations never use Ground or the upper layers. Other peoples retain their current policy until directed otherwise. This does not forbid later travel above ground.

Place one inhabited starting settlement on each underground layer at generation, with suitable locations chosen deterministically. Do not invent extra faction identities, lore or established fortresses. This decision specifies two locations, not a new total population; founder allocation must stay consistent with the existing configured faction population. Both settlements need layer-appropriate safe floor, food/water and construction resources. Keep distinct site IDs for both settlements with one shared faction ID, retain both settlement references, and keep one valid primary faction.home for existing callers rather than repeatedly overwriting it. Each history site, campfire, assigned founder, spawn reservation and resource kit must carry its settlement's actual z. The initial view, if the player is dwarven, must reference that primary home without suppressing the other settlement.
...
Add checks: across fixed seeds the guaranteed dwarf faction has populated settlements on **both** z=-1 and z=-2 with the same faction ID; every dwarf founding site/unit/camp/kit is on an allowed map and valid walkable cell; each settlement has its own usable local resources; founders are not duplicated, each belongs to exactly one starting site, and the configured total faction population is conserved; no dwarf founding sites, founders, campfires or kits appear on Ground/upper maps (inspect actual generated map content as well as diffs); identical x/y on other maps remains independent; save/load preserves both site IDs, shared faction identity, primary home, memberships and correct z without overwriting or duplicate spawning; both settlements continue operating while another map is viewed. A fixture missing either settlement must fail. No claim of implementation until those tests run on the actual five-map core.
