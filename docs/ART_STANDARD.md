...
| F6 | **Characters:** 8 facings (S, SW, W, NW, N, NE, E, SE; VISION V3) for **every action**, not only walking: stand, idle, walk, work, attack, cast, hurt and death each have all eight (carry is not drawn: F7). Diagonals are true three-quarter turns, never copies of a straight facing. The east-side facings may mirror the west side. Bodies are bare-handed in plain clothes; clothing, armour, shields, tools and weapons are layer sheets on the same frames (V61). |
| F7 | **Animation is frames in the art** (V58, V60): idle, walk, work, attack, cast, hurt, death; sway, lit, open and work loops for objects. The game draws no motion of its own. Combat happens on the map (V45, V64). **Carrying is not drawn** (V89, user 2026-09-19: "Whatever they are carrying can be documented in their profile as opposed to animated"): a person or creature hauling something looks exactly as it does walking, with no load in its hands; what it carries is written in its profile (the character sheet and the colonist card). Generators make no carry image; the sheet keeps column 7 for compatibility and the tools fill it with the stand frame. |
