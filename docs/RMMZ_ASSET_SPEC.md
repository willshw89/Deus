...
Binding specification for standard RPG Maker MZ asset formats (VISION Rule V70). All assets generated by Google Nano Banana (`generate_image`) and processed for the project must conform to these standard dimensions and layouts.
...
| **Canvas Generation** | **4× Scale (192 × 192 px)** | Nano Banana generates at 4× (192×192 per cell) for clean pixel clustering, then tools reduce by 4 to 48×48 native. |
