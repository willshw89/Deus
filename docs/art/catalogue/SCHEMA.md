...
**`scaleRow`** names a `scale_chart.json` row or a `size_classes.json` race row. Character entries cite a `RACE_*` row (or `GEOM_FRAME_*` for peoples with no SRD race, `CHARACTER_CHILD` for children); creature entries cite `GEOM_FRAME_*`; items and effects cite `RMMZ_TILE_48` (the 48x48 icon frame; the drawn size inside is not charted).
