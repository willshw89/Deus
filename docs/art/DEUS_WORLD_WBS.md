...
| **DW.01.05** | **Freeze Palette Architecture & Family Material Ramps** | Two-tier palette architecture: 254-color master (`art/palette/deus_master_world_palette_v1.hex`) + 58 cohesive family material ramps; container sheet policy in art_check.js; VFX exception. | **COMPLETED / FROZEN** | (Current commit) |
| **DW.01.06** | **Freeze Seamless Autotile & Terrain Assembly Standard** | Standardize 48×48 autotile bitmasking, border blending, corner logic, and multi-layer elevation edges to eliminate repetitive grid notches and hard tile seams. | **APPROVED NEXT LEAF** | Pending owner authorization |
