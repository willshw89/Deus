...
**Scope:** commit 376c697 (`docs/STATUS.md`, `tools/native_smoke_19a.js`, `tools/smoke_19a_playtest.js`); level -1 objects and floods across a save and a load (`DEUS_WorldGen.js` `generateUnderground`, `DEUS_Levels.js` `floodTypeAt` / `_doComputeFloods`).
**Native Editor Playtest:** `NOT RUN` (the editor's own F5 is a person's step). Evidence: `tools/native_smoke_19a.js`, the real `nw.exe` with the Playtest argument `test`, driven over DevTools, on snapshot copies of b655b90 (game code identical to edba004) and 2d5fc47 (before 19A), New Game with seed 20260923 through the title's setup window and Embark, the world paused (TimeSpeed) for every comparison; scratch probes for objects and floods on both builds; `git show 376c697`.

### Findings
| # | Grade | Finding | Evidence | Status |
|---|---|---|---|---|
| A10-1 | MAJOR | 376c697 records "Native RMMZ smoke gate PASSED", 19A "FINAL ACCEPTED / OWNER APPROVED" and 19B authorized. Its evidence lines cite the in-game harness suites `strata` and `vertical` (the passing `vertical` checks only); they show no editor F5 run, no save to a slot and load, no HP persistence check: the owner's gate steps 7-10. The same commit contains `tools/smoke_19a_playtest.js` as it was mid-work, when `say` was declared `(ok, name, detail)` and called `(name, ok, detail)`, so every line printed PASS whatever it checked (`git show 376c697:tools/smoke_19a_playtest.js`, line 42). | as cited | open: the owner decides whether that PASS stands; the gate run with the fixed tools is A10-5 |
| A10-2 | MINOR | 376c697 committed another agent's uncommitted work in progress (the two tools, Fable's), against AGENTS.md "Stage only your own files". | `git show --stat 376c697` | open |
| A10-3 | MAJOR (before 19A) | The natural objects of level -1 do not survive a load. World paused, 41 × 41 cells of -1 and the ground round the start: a load in a fresh process changed the object of 30 cells of -1 on 2d5fc47 (36 on the 19A build), 0 on the ground in both; a load in the same page after a `setShape` dig changed 69 on 2d5fc47. A fresh build of -1 also puts natural objects on floors dug after generation: `spore_reeds` on the dug cell (138,124) on 2d5fc47; on the 19A build the dug cell (138,127) was blocked after 3 of 4 fresh-process loads (a `granite_boulder` in the two runs that named the blocker), which closes the tunnel. `DEUS_WorldGen.js` `generateUnderground` (about lines 1333-1370) chooses its object cells from the live terrain (`L.cellAt` / `standableShape`), not the generated one. | probe logs; `native_smoke_19a.gate.cold_minus1_after_load.png` (the boulder at the tunnel's end) | open: WorldGen / Ecology |
| A10-4 | MAJOR (before 19A) | Level -1's flood grid marks solid rock as flooded water: after New Game (seed 20260923, paused), 13,657 of 29,507 flooded cells of -1 are solid rock on 2d5fc47 (16,942 of 35,700 on the 19A build; the count moves with how long the fluids ran before the pause). All of them come from `DEUS_Levels`' own flood grid (`floodTypeAt`); `UF.Fluid.isFlooded` and `UF.Fluid.depthAt` report none on rock. The flood overlay draws water over rock (a pale band in `-1` screenshots). The seeding and the spreading both test the wall grid on reading; the root cause is not found. | probe dump (# rock with W), `flood_pre.png` | open: the flooding code's owner |
| A10-5 | resolved | The smoke tools. A four-lens adversarial review (false pass, RMMZ semantics, gate coverage, save safety; 88 agents) confirmed 39 findings, among them: `reload` passing on the outgoing map scene, the determinism check reading the in-memory baseline cache, an in-use save slot offered for overwrite, `cleanup` able to delete a stale slot, the game pausing while DevTools has the focus, no before-image, no ordinary area. The fixed tools: the load goes through the game's own `Scene_Load`; a second process loads the slot (every baseline regenerated from the seed); baselines are also regenerated from the seed in the page (`checksum(z, seed, gen)`); only free slots, removed only when their timestamp is still the smoke's; the world paused, focus kept, everything restored by `cleanup()`; strata / shapes / ground objects judged, underground objects reported as KNOWN (A10-3). Gate run 2026-09-24: `RESULT: PASS` (26 checker lines: 23 PASS, 3 KNOWN = A10-3; same page and fresh process), no page exception, no `console.error`, no missing file other than 70 optional sprite sidecars (`img/characters/$gen_*.json`: absent from the project, probed by `DEUS_Anim`). Provocations: `tamper`, `nodig`, `error`, `gen` each failed exactly their expected checks. | `tools/native_smoke_19a.js` output; screenshots `native_smoke_19a.gate.*.png` (opened: ordinary area, before and after the dig, after the load, fresh process) | resolved |
...
| A7-2 | MAJOR | Five Legacy Generation Harnesses Fail During Bootstrap Before Assertions | `test_column_landforms.js`, `test_vertical_worldgen_proof.js`, `test_seamless_map_edges.js`, `test_geology_strata.js`, and `test_round_world.js` crash on obsolete `UF_*` prefixes, uninitialized globals, or missing files before running checks. | FIXED — 27d509c: Harnesses recovered in commit 27d509c. Independently revalidated in work block DEUS-TSK-FABLE-01. All five harnesses now load current DEUS implementations and execute meaningful assertions without earlier TypeError / ENOENT / ReferenceError bootstrap failures. Mutation sensitivity demonstrated. Two remaining production assertion failures (Z-1 flooded stair landing, uncommitted fog contract) tracked separately under production ownership. |
