...
```text
IMAGE MODEL: Google Nano Banana Pro (gemini-3-pro-image, Gemini 3 Pro Image model) is mandatory for all generation tasks (AGENTS.md Rule 11, VISION V69/V70/V79/V109). Nano Banana Pro utilizes advanced reasoning ("Thinking") to follow complex instructions, maintain brand consistency, and render high-fidelity text and pixel details. No other model allowed. All animation must happen through sprite frames—NO AFTER-EFFECT ANIMATIONS (Rule 12, V108). Draw on a 4x canvas where one RPG Maker grid cell of 48x48 px is 192x192, even 4x4 pixel blocks, on a flat #FF00FF magenta background.
