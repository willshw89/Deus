...
| V44 | **Uniform map sprites, like Shining Force:** every person and creature is drawn at a similar size on the map (one cell; people 48×72, animals and monsters 48×48 or 48×72 exported) whatever its real size; size and bulk show in the portrait and the description, not in the map sprite. |
...
- 2026-09-18 (evening): **The look changes to 2D in the style of Final Fantasy VI** (user). V2 and V9 revised; the 2.5D guide, the U7 stand-in pipeline and K3 are dropped; RPG Maker's stock art becomes the placeholder set; Q5 (4 facings) and Q6 (48 px cells) are settled by the style. The engine is unaffected (cells, objects, sprites at the feet); the catalog's images switch from U7 rips to stock sheets and tiles. Then V44 (uniform map sprites, Shining Force style) and V45 (overworld combat with attack animations, U7 style) added by the user, then V46 (the world moves on a beat, 1 per second at ×1), then V47 (d20 mechanics under the U7-style combat, with DF injuries), then V48 (every living thing decides or follows a decision every beat).
