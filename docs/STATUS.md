...
- **Status**: `STOPPED AT THE 19A GATE — AWAITING GEMINI COORDINATOR REVIEW` (`19A APPROVED / PROCEED TO 19B` needed; no editor Playtest yet).
...
- **Checks observed** (2026-09-24, snapshot copies of 2d5fc47 plus these files; no RPG Maker process in the task list when checked): `tools/test_strata_foundation.js` `RESULT: 26 passed, 0 failed` and all 23 mutants exit 1; `tools/test_volumetric_terrain_column.js` 34/34 (its moved mutant `hills_walkable` exits 1); the older terrain harnesses (column_landforms, geology_strata, upper_elevation_terrain, vertical_worldgen_proof, natural_connections, survival_regressions) print the same PASS/FAIL lines as on 2d5fc47, including their failures from before this task; the gate suites palette 503/503, biome 334/334, scale 26/26, native resolution 23/23, `art_check.js --selftest` 80/80. In game (nw.exe harness): new suite `strata` `RESULT: 6 passed, 0 failed` (seed 20260923), five provocations seen failing in two runs; regression suites give the same results as 2d5fc47 (smoke, vertical 8/11, natural_walls 6/6, flooding 8/8, floors, environment, minimap, ground, natural_connections; with seed 20260923 in both builds: world 23/30 and jobs 19/19 identical, depth 27/28 with `treatment_cost` failing on both).
- **Cost** (same seed in both builds, 600 frames of normal play, about 1,230 units): the work the strata add is 0.012 ms a frame; all shape-query time is 0.41 ms a frame (8,676 reads at 46 ns) against 5.40 ms before (8,289 reads at 652 ns); `world.path_budget` ran the same searches at 1.22 ms a plan against 6.06 ms.
- **Screenshots opened** (`strata.tunnel_before.png`, `strata.tunnel_dug.png`, level -1): before, a cave floor above solid rock; after, a one-cell floor corridor running four cells south from the cave floor (the last one under the black cap of the rock beyond it), flanked by black cap bands.
- **Found**: 23 walking animals stand on ground cells inside hills, on the pre-strata build too (same seed): a Wildlife placement matter. The depth suite's terrace-hole fixture now reads as a floor (setShape "open" over solid ground is a floor on its top under the strata).
