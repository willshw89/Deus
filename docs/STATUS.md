...
- **Side effect to decide on**: `tools/test_survival_needs_loop.js` (Gemini's) goes from 18/19 to **14/19** with the new order, deterministically: `drinks_a_gallon_a_day` and `eats_a_pound_a_day` (its calendar stays at 10:00 for the whole scene, so no meal hour ever comes and the founders work the open shelter project before drinking, as rung 8 says); `labor_resumes_after_supper` (the hauler is at the evening fire-gathering when the check looks, the timeline having shifted); `long_rest_rules` (founder #8 drank at the pond's east side and its straight-line walk back through the water fails `can't reach it` in that harness's World double). The order is the packet's; the harness encodes the earlier "needs first at any hour". Either the harness advances its clock through a meal hour, or the coordinator restores needs above work.
