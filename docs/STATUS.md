...
- **Scope** (`game/js/plugins/DEUS_Levels.js`, a roof hook in `game/js/plugins/DEUS_Floors.js`, new `tools/test_strata_foundation.js`, one moved mutant target in `tools/test_volumetric_terrain_column.js`, `docs/systems/UF_Levels.md` section Strata; no cuts, caves, fluid rewrite, depth rewrite, art, `plugins.js` or `game/data` change): every cell of the five levels is five 1 ft strata (material byte + HP byte), the one mutable authority; the shape codes everything else reads are derived from them (`derivePacked`) and cached per level as read-only grids refreshed only by the strata writer; `setStrata` / `setShape` / damage all go through one writer that emits `levels:strataChanged`, `levels:shapeChanged`, `levels:cellChanged`, `levels:strataDamaged`, `levels:strataDestroyed`; `applyStrataDamage`, `applyVolumeDamage` (box and sphere, across levels), material resist and damage hooks; adapters `surfaceHeightAt`, `worldStrataElevationAt`, `heightStateAt`, `fluidStateAt`, `isSolid`, `solidFraction`, `dominantMaterial`, `remainingStructuralHP`, `effectiveSupport`, `hasOpaqueOverburden` (also `UF.Floors.hasOpaqueOverburden`; `isRoofed` uses it), `getStrataFluidPassage` (DEUS_Fluid keeps 0..7); sparse saves (`levels[z].strata[area][cell]` = 22 hex digits, `strataSchemaVersion: 1`) and `migrateSaveToFiveStrata` for older saves.
- **Memory**: 1,802,240 B of strata + connectors for one 256 × 256 area's five levels, 2,129,920 B (2.03 MiB) with the cached shape grids (budget 3.5 MB). The baseline HP plane is implicit (unchanged strata are at full HP); changed cells carry both bytes.
...
- **Native smoke (Fable, 2026-09-24, after 376c697)**: the checker committed in 376c697 could not print FAIL (AUDIT_LOG A10-1); fixed and reviewed (A10-5). `node tools/native_smoke_19a.js` on a snapshot (real `nw.exe` in Playtest mode, DevTools-driven, seed 20260923, New Game through Embark): `RESULT: PASS`: Z0, an ordinary ground area, a tunnel dug at -1 with `applyVolumeDamage` (the map repainted, walkable, a 4-step path), partial HP, a save to a free slot, a load through the Load screen and a load in a fresh process: strata, HP, the cells round the tunnel and a far block unchanged, the baselines regenerated from the seed equal, the game running, no page exception or `console.error`. Four provocations each failed their own checks. KNOWN, measured on 2d5fc47 too: level -1's natural objects change across a load and can land on dug floors (a boulder in the tunnel after a fresh-process load; A10-3); level -1's flood grid marks solid rock flooded (A10-4). The editor's literal F5 run (the checker pasted into F8, steps in its header) is still a person's step.

## CANONICAL DESIGN UPDATE: Standard New Game Starts at World Year 0 (2026-09-25)
- **Status**: `FROZEN SYSTEM DIRECTIVE (VISION V134)`
- **Rule**: Every standard DEUS New Game begins at **World Year 0**. The player does not normally choose to begin at Year 100, 250, 500, or a pre-aged historical state.
- **WorldGen Responsibility**: Worldgen creates the initial viable Year-0 physical world: terrain, five macro-Z levels, five-strata geometry, geology, caves/cuts, hydrology, soils/fertility, finite geological resources, vegetation, food ecology, wildlife populations, natural landmarks, story anchors, and nine faction starting populations (9 × 8 = 72 founders).
- **No Pre-Materialized History**: No pre-generation of centuries of roads, old kingdoms, abandoned towns, historical mines, exhausted veins, battlefield debris, or ancient coins. All history develops dynamically through live simulation.
- **Conservation Consequence**: At World Year 0, the overwhelming majority of finite STONE, IRON, COPPER, SILVER, GOLD, and PLATINUM exists in natural geological deposits. Any conserved material already present in founder equipment, starting stockpiles, structures, tools, weapons, or other manufactured forms is deducted from the same finite world-generation material total. Standard New Game contains no pre-aged historical circulation, historical scrap economy, abandoned industrial stockpiles, or fictional centuries of prior resource extraction.
- **Simulation Capability Preserved**: Long-run simulation (100, 250, 500 years without player input) remains fully supported as a developer, balance-testing, and scenario capability, but is not an ordinary New Game starting option.
- **WBS Alignment**: WG.90 completion gate requires a viable living Year-0 world ready for simulation, rather than pre-aged historical materialization.
...
- **Scope**:
  ...
  - Curated the **Global Master World Palette V1** with exactly 226 active colors (cleanly within 220–240 target range) and 30 reserved headroom slots under the 256 ceiling (`art/palette/deus_master_world_palette_v1.hex`).
  - Consolidated near-duplicates through shared cross-family deep shadow baselines (`#142216`, `#111F1B`, `#312820`, `#2F271C`), shared midtones, and shared warm highlights; achieved 0 near-duplicates (<4.0 RGB distance).
  ...
  - Implemented variable ramp lengths (3, 4, and 5 tones) reflecting natural material properties (e.g. 4-tone compact ramps for sulfur, charred wood, anaerobic mud, caliche bone, rapids foam, healing aura).
  - Purged forbidden terminology across canonical art standards and resolvers; canonical language audit passes with 0 violations.
  ...
  - Built compiler/query tool `tools/palette_resolver.js` (`--biome`, `--material`, `--pair`, `<RAMP_ID>`, `--budget`, `--master`) for future Nano Banana Pro prompts.
  ...
  - Created dedicated automated test suite `tools/test_palette_standard.js` (503/503 checks passing).
  - Documented controlled palette versioning policy with explicit 8-step addition protocol in `docs/art/DEUS_PALETTE_ARCHITECTURE_STANDARD.md`.
...
- **Scope**:
  ...
  - Created machine-readable scale registry: `game/data/DEUS_ScaleRegistry.json` (and `docs/art/DEUS_ScaleRegistry.json`) covering 39 canonical classes across 6 domains (Characters, Micro Vegetation, Shrubs, Stone, Wood Debris, Trees, Architecture Reference Anchors).
  ...
  - Applied owner review corrections:
    1. Corrected ordinary doorway class (`ARCH_DOORWAY`) from oversized 72 px to grounded 56–60 px (target ~58 px, width 40–48 px, target 44 px), comfortably clearing ~42 px Human with headroom while separating monumental gate architecture (`ARCH_GATE_LARGE`, 84–96 px).
    2. Removed style-specific "DF Black-Cap" language from scale registry notes/fields in favor of pure geometry metadata (anchor type, footprint, overhang allowance, upper frame extension, wall-base contact).
  - Rendered deterministic technical scale strip graphic: `art/reference/DEUS_HUMAN_SCALE_STRIP_V1.png` (1340×224 px) comparing 18 objects on continuous baseline y=180 with 48px tile guides, 58 px doorway milestone, and height rulers.
  ...
  - Added automated test harness `tools/test_scale_standard.js` (26/26 checks passing).
  - Stopped at gate per AGENTS.md Rule 6.
...
## Depth compositing visual tuning: recession + fade/colour + light blur on the whole plane (Fable, 2026-09-24)
- **Status**: `STOPPED FOR OWNER YEA / NAY` on the A/B/C/D comparison (owner direction "DEUS — DEPTH COMPOSITING VISUAL TUNING", baseline fc3b358; no editor Playtest yet).
- **Scope** (`game/js/plugins/DEUS_Depth.js`, `docs/systems/DEUS_Depth.md`, `docs/VISION.md`, one parameter line in `game/js/plugins.js`; no renderer redesign, no simulation, worldgen, wall or camera change): new default preset `deus` = three progressive cues on each lower plane as one container treatment (tiles and entities share it, no per-sprite filter): eye height 140 ft (depth 1 scale 0.959, depth 2 0.921), fade as colour not alpha (brightness 0.92 / 0.82, saturation about 93 % / 85 %, contrast about 95.5 % / 90 %), blur 0.6 / 1.2 px at `blurQuality` 1. `deus_scale` and `deus_color` are its comparison steps; A (190 ft, the fc3b358 look), B, C, D, E and the 190/120/60/30 ft bracket stay for development; F7 cycles. Black caps unchanged (configurable, `config.entities.walls`).
- **Checks observed** (suite `depth`, 28 checks, snapshot run `depth14` alone, editor open): `RESULT: 28 passed, 0 failed (exit 0)`. New: `depth_transform_progressive` (provocation seen failing in `depthP10`), `entities_inherit_treatment`, `blur_off_no_blur`, `color_off_baseline`, `visual_settings_no_physics`, `config_deterministic`, `treatment_cost`.
- **Cost** (median engine tick, simulation paused, RTX 4060 through ANGLE/D3D11, editor open): planes off 45.7 ms, recession only +16.9, colour −1.2 (free within noise), blur quality 1 +14.7, blur quality 2 +34.4. The baseline itself swung 18–50 ms between runs with the editor open, so the blur's true cost must be read in Playtest (F2) before acceptance.
- **Screenshots opened** (`%TEMP%\uf_snapshots\depth13\test_output\depth.tune_A_baseline / tune_B_scale / tune_C_scale_color / tune_D_full / plus1_tune_D_full.png`, same camera): described in `docs/systems/DEUS_Depth.md` §7. Fable recommends D (full) at these values; C if blur is refused.
- **Caveat:** the RMMZ editor was open when the plugins.js parameter line (Preset deus, EyeHeightFt 140) was last written; if the project is saved from the editor, set those two parameters in the Plugin Manager or ask Fable to re-apply them with the editor closed.
...
- **Scope** (new files only; `DEUS_Levels.js`, `DEUS_Camera.js` untouched):
  - `game/js/plugins/DEUS_Depth.js` (`UF.Depth`): while `+1` or `+2` is on screen, the level below and, through its open cells, the level two below are drawn beneath the tile layer of the map on screen, each a stock `Tilemap` painting into canvas Bitmaps (nearest sampling) from `UF.World.peekArea`, scaled about the viewport centre by a pinhole-camera model: `scale = eye / (eye + 6 ft × depth)`, eye height 190 ft by default (0.969 / 0.941, which is what the addendum's 0.97 / 0.94 amount to), tunable (`EyeHeightFt`, `UF.Depth.setEyeHeight`). The viewed level's own opaque tiles are the exposure mask (its `open_air` look is transparent); beyond the last drawn level lies the near-black void (`#08080C`), never the sky. Presets: `A` (default, crisp zoom), `B` (+ value step), `C` (+ subtle blur), `D` (one level below, blurred 1.5 px; with `MaxDepth` 1 this is the intermediate direction), `E` (D + darker), `off`; F7 cycles them in play. Save data: none.
  - `docs/systems/DEUS_Depth.md` (six sections, evidence in §7); `docs/VISION.md` decision log (the three directions and the scope of the exception to V60/V108); `docs/AUDIT_LOG.md` A9; one entry in `game/js/plugins.js` before `DEUS_Test` (editor closed: no RPGMZ process, checked three times on 2026-09-24).
- **Checks observed** (suite `depth`, 20 checks, snapshot copy `depth5`, 2026-09-24): `RESULT: 20 passed, 0 failed (exit 0)`. Nine provocations seen failing in their own runs (`projection_origin`, `exposure_by_upper_geometry`, `depth2_through_depth1`, `crisp_nearest`, `planes_present`, `mask_order`, `parallax_bounded`, `void_beyond`, `blur_by_default`; the FAIL lines are quoted in the system doc §7).
- **Screenshots opened** (`%TEMP%\uf_snapshots\depth5\test_output\depth.*.png`): `plus2_off`, `plus2_A_eye190`, `plus2_A_eye120`, `plus2_A_eye60`, `plus2_A_eye30`, `plus2_B`, `plus2_C`, `plus2_D_oneLevelBlur`, `plus1_off`, `plus1_A_eye190`, `plus1_D_oneLevelBlur`, plus the planes' own renders. What is in them is described in `docs/systems/DEUS_Depth.md` §7.
- **Fable's reading for the review:** 190 ft (the addendum's numbers) keeps the lower terrain readable and the openings aligned; 120 ft is a visibly stronger but still coherent step; at 60 and 30 ft the lower levels read as miniatures and the `+1` wall face under the summit edge shows as a band. The step reads weakly at any height because the `+2` and `+1` floors share one look and no hanging faces are painted; the value step of B helps more than the zoom. Decide the eye height in Playtest with F7 and `UF.Depth.setEyeHeight(ft)` in the console (F8).
- **Found on the way:** AUDIT_LOG A9 (MAJOR, Gemini): the live `Outside_A2` sheet paints kinds 0–3 only, so `peak_rock` (kind 14) and every other ground kind draw nothing; under a terrace hole the `+2` view now shows the void instead of rock.
...
## DEUS-TSK-ASTRA-15 — HIST-10 world age and living materialization (2026-09-23) — CLOSED / PASS

- **Closure authority:** `DEUS-TSK-ASTRA-16`; independently reverified against combined HEAD `28b0e5b2672af72191dc81022f84e325fa841a14`. The ownership/dependency blocker is resolved by Gemini `e0f4e25`. This closure changes only `docs/systems/UF_History.md` and this status record.
- **Observed checks:** `node tools/test_history_materialization_and_world_age.js` **29 PASS, exit 0**; all 12 cases for seeds 0/424242/20260919 and ages 0/100/250/500; 3 complete demographic/event prefix equalities; 6 disk save -> saver exit -> fresh-process reload comparisons with exact `worldSha256` equality. Stable IDs, age-1 alias, explicit seed zero, and Colonists initialization at ages 0/250/500 also pass.
- **Negative controls:** All three requested commands produced **expected FAIL, exit 1**: `target_year_alters_early_history` -> `PREFIX_INVARIANCE`; `materialization_drops_species` -> `MATERIALIZED_CENSUS`; `save_drops_historical_records` -> `ROUNDTRIP_HISTORICAL_RECORDS` (1,582 ancestors became 0; the fresh loader restored 0).
- **Measurements:** Age-500 living counts for seeds 0 / 424242 / 20260919: **1,080 / 1,063 / 1,093**; ancestors **4,178 / 4,199 / 4,224**; serialized worlds **4,991,801 / 4,958,372 / 5,037,835 bytes**. Generation/materialization plus worker assertions: **10.294 / 11.640 / 13.465 s**. Total integration wall time **104.5186285 s**. All nine species survive at every age-250/500 boundary. Full 12-row measurements, demographic/event hashes and six exact reload-world hashes are in [UF_History.md](systems/UF_History.md).
- **Provenance:** All 20 loaded input files match the committed baseline; source-set SHA-256 `eeca632bcf0da5cc25a59f39fc856522e7dd1e049f3a6d05a8103eb56515c1de`. History `a5eb0ec691b38877dd4a441e73ee8be70d86421f83480030cdbd1fa164ee4d17`; harness `b2cd04303573392b10656b2366f6a64dd43d044a5345904ba11bfdf91b1b6453`; frozen demographics `d0a09bfda8ab63ff3eec5b362c63896ea0c3dbcb3aeeeb574c48c3113c995e4e`. No production, harness or catalog edits in ASTRA-16.
- **Evidence:** `C:/Users/snewt/AppData/Local/Temp/deus-astra16-integration-ee162c08-c4da-4d5b-950b-e2db45b28329.json` and `C:/Users/snewt/AppData/Local/Temp/deus-astra16-mutants-1790207093689.json`. These supersede the pre-integration ASTRA-15 measurements. HIST-09 capacity/self-test commands were not rerun in this closure; their prior PASS evidence remains recorded.
- **Scope limits:** Native F5/F8, screenshots and the legacy in-game history suite are **NOT RUN**. The closure is the authorized headless contract verification, not a native gameplay claim. Existing display-name/appearance-stage behavior, 400-event retention and historical snapshot/live-state separation remain documented.
- **Roadmap:** HIST-11, `Archaeological Lore & In-Game Genealogical Verification`, has concrete requirements and acceptance cases in [UF_History.md](systems/UF_History.md#hist-11-requirements--archaeological-lore--in-game-genealogical-verification-2026-09-23). Cover historical/deceased Sheet subjects, family navigation, traceable Look/inscription text, explicit ID namespaces, read-only/save invariance, bounded indexes and native UI evidence. Physical grave anchors require an approved schema/owner; no burial placement or new lore is implied. **Requirements prepared; implementation NOT STARTED.**
...
- **Result — Astra (2026-09-23): PASS.** All three required commands exited 0: direct capacity contracts, benchmark `--selftest`, and the full default benchmark. Verification claim released. Dispatch baseline `c49a142cd087b6b95cb7d7acef07dbf97d5698c1`; frozen candidate `4af58ddd486b8e5d97d24877fd1b826131724c1d`, 52,586 bytes, SHA-256 `d0a09bfda8ab63ff3eec5b362c63896ea0c3dbcb3aeeeb574c48c3113c995e4e`, unchanged before/after verification.
- **Contracts:** 23/23 common behavioral/boundary checks, 5/5 reconciled metadata checks, 8/8 capacity mutants detected with intended child exit 1; benchmark self-test 29/29 including its four negative controls. Both prior metadata failures resolve: unsupported caller capacity IDs/versions and contradictory root/config IDs reject, and genuine custom v6 migration normalizes the demographic tag to `custom` while preserving records/biology. Three supplemental in-memory boundary reversions each fail exactly `PACKET_CAPACITY_MODEL_IDENTITY`, child exit 1. No production repair by Astra.
- **Canonical matrix:** Seeds 0/424242/20260919, two fresh-process 500-year runs each; all 18 checkpoints at 100/250/500 complete. Parent process seconds (repeat 1 / repeat 2): 10.8229190 / 12.2414065; 12.5811280 / 12.6483508; 12.0479071 / 11.9582089, all <30 s. All five biological/population/serialization/timing gates PASS. Nine species survive every annual census and retain fertile partnerships at 250/500 years. Year-500 living totals 1080/1063/1093; half-elves 145/126/118; canonical annual species peak 177 (<1500); largest state 4,003,167 bytes (<15 MiB).
- **Determinism and sweep:** Three exact state/event/curve repeat comparisons PASS; three actual disk-save/process-exit/restart continuations match continuous year 250. Twenty seeds (1–20) complete 250 years with no annual species extinctions; population and serialized-checkpoint gates PASS. Living min/median/max 900/980.5/1029, annual site peak 187, largest sweep state 1,920,192 bytes, slowest worker 2.7836599 s. Canonical checkpoint hashes and all 23 unique-seed annual curves match the retained ASTRA-12 fingerprints. First default run passed in 165.7087084 s; final default run passed in 187.0362076 s after correcting only its provenance constant, without forced GC or fastest-sample selection; final measurements on Node v24.19.0 / Ryzen 7 8845HS.
- **Changes and evidence:** Corrected stale task labels and dispatch provenance in the two authorized harnesses; strengthened the capacity metadata check for explicit supported inputs, invalid caller versions and contradictory config IDs. [UF_History.md](systems/UF_History.md#deus-tsk-astra-14-independent-hist-09-candidate-revalidation--2026-09-23) records full timings, populations, hashes, source audit, controls and limitations. Final local `game/test_output/bench_species_biology.json`: task ASTRA-14, schema 5, PASS; 37,997,150 bytes; SHA-256 `afabf8695dc2cc71be5bfcf0347f7077167f4aa489860d6dee72cd86491dea19`; created `2026-09-23T22:46:57.392Z`. It replaces the old report at the reused, gitignored path. Aggregate report size is not the per-state size gate.
- **Baseline discrepancy:** The packet supplied `c49a14250269389255a0ea738d2f5926ec0ea429`, which is absent from local Git. HEAD, the shared short prefix/commit subject and reflog resolve the alignment baseline to `c49a142cd087b6b95cb7d7acef07dbf97d5698c1`. The pre-commit check caught the discrepancy before staging; the final report/harness record the observed baseline. Frozen candidate identity was correct throughout.
- **Limits and handoff:** The benchmark executed frozen snapshot inputs; concurrent working `DEUS_World.js` differed and was not loaded. Eight nongating metadata/hash-validation observations remain disclosed; passing fingerprint generation does not establish production hash-integrity validation. Older regression suite, native F5/F8, NW.js, screenshots, gameplay and real-save integration: NOT RUN. This is headless candidate verification, not production promotion or a performance optimization. Gemini retains integration authority; no next implementation task started.

## DEUS-TSK-ASTRA-12

- **Result — Astra (2026-09-23): FAIL, exit 1; PASS on all behavioral, biological, migration-preservation, and timing gates.** ASTRA-12 verification commit `62631babc86e407eaa0c0e9d939648850ce6ae20`; ASTRA-13 consolidates its retained evidence against documentation baseline `87766842ba742b40bb942da2b241a107e44ee0c0`. Verification claim released. Frozen production candidate `ebf2455af610e7a29f5393182e2b75d4eac35769` remains 51,780 bytes, SHA-256 `9efb6c69faee25c44ed3f97e067089d209126c3d3fde30e397ecc26850dedbdf`.
- **Metadata contracts: 3 PASS / 2 FAIL.** PASS: `PACKET_DEFAULT_PROFILE_TAG` (`v1` / `1.0.0-provisional-astra08`), `PACKET_DEMOGRAPHIC_MODEL` (schema 7, model 1), and `PACKET_CUSTOM_PROFILE_FINGERPRINT` (independently checked deterministic SHA-256). FAIL: `PACKET_CAPACITY_MODEL_IDENTITY` accepts caller `capacityModel.id: "TEST_UNSUPPORTED_CAPACITY"` (also null/1), creating inconsistent root/config identities. FAIL: `PACKET_CUSTOM_CANNOT_CLAIM_V1` preserves `legacy-v6` on genuine custom v6 migration instead of normalizing to `custom`; creation normalization and explicit spoof rejection pass. The ASTRA-13 packet's four-pass/one-failure summary omits the latter failed subpath; the ledger preserves the observed 3/2 result.
- **Passing checks:** Benchmark self-test 29/29 (including corruption and four mutant controls); common contracts 23/23 (fixed [60, 350] capacity bounds, density pressure, site locality, immutable validation, migration record/biology preservation, idempotence, rejection and atomic failure); all eight capacity mutants detected with intended child exit 1. Passing migration preservation does not negate the migrated metadata failure. The old regression suite is NOT RUN for ASTRA-12 because it still targets candidate `8a40d2e` and is outside the dispatch's required suites.
- **Canonical matrix:** Seeds 0/424242/20260919, two fresh-process 500-year runs each, all 18 checkpoints at 100/250/500 complete. State/event bytes, SHA-256 hashes and annual curves repeat exactly; three real disk-save/process-exit/restart continuations match their continuous 250-year reference. Parent process seconds (repeat 1 / repeat 2): 10.5635031 / 10.8362765; 10.7032454 / 10.8562837; 10.5660253 / 11.3990842, all below 30 s. Year-500 living totals 1080/1063/1093; half-elves 145/126/118. All nine species survive with fertile partnerships at 250/500 years; largest canonical annual species population 177 (<1500). Largest state 4,003,167 bytes (<15 MiB).
- **Sweep and time:** Seeds 1–20 completed 250 years; extinction/population/state-size gates PASS. Living min/median/max 900/980.5/1029, largest annual site population 187, largest state 1,920,192 bytes, slowest worker trajectory 1.9394427 s. Recorded complete suite/matrix/restart/sweep wall 141.3306056 s on Node v24.19.0 / Ryzen 7 8845HS. No timing retries or forced GC. These are retained ASTRA-12 measurements, not an ASTRA-13 rerun.
- **Evidence:** [UF_History.md](systems/UF_History.md#deus-tsk-astra-12-independent-hist-09-verification-ledger--2026-09-23) records the exact candidate and harness identities, contract breakdown, timings, populations, checkpoint hashes, restart evidence and limitations. Local `game/test_output/bench_species_biology.json` remains the final schema-5 ASTRA-12 report: 37,996,246 bytes, SHA-256 `9e2a91d98759eccdb4093def6c456b223675a55058c23080a65316d2391e2d8f`, created `2026-09-23T16:22:35.857Z`. ASTRA-13 rechecked artifact/source hashes and document consistency without changing the report, harnesses, or production. Earlier ASTRA entries describe their historical candidates; their reused artifact paths no longer identify current JSON contents.
- **Handoff and limits:** Gemini retains integration/production authority; the two metadata failures require its repair or explicit contract decision. RMMZ F5/F8, NW.js, screenshots, native gameplay and real-save integration: NOT RUN; no gameplay or production-acceptance claim. No production repair or next implementation task started.
...
- **Remaining failures (5):** Promoted default tag is `1.0.0-provisional-astra08`, not required `v1`; existing `historyModelVersion` is 1, not packet model 7 (schema is correctly 7); capacity metadata has numeric version 1 and no `local_density_v1` identifier; modified profiles can claim `v1`; no advertised deterministic custom-profile SHA-256 fingerprint. Expected/observed metadata and field mapping are recorded separately from common behavior checks. Coordinator must reconcile the packet with implementation or issue a production repair task.
- **Passing checks:** Common contracts 23/23, including read-only validation, absolute capacity bounds, density semantics and atomic/idempotent migration; eight capacity mutants detected. Regression 34/34 with four intended mutant failures. Benchmark self-test 29/29, including four further negative controls, snapshot corruption and partial-sweep failure checks. The full default command ran once, without timing retries or forced GC.
- **Canonical matrix:** Seeds 0/424242/20260919, two fresh processes each, all 18 checkpoints at 100/250/500 measured. Exact state/event bytes, SHA-256 hashes and annual curves repeat; all three dedicated disk-save/process-exit/restart continuations match the continuous 250-year run. End-to-end process times by seed (repeat 1/repeat 2): 11.566/10.058 s; 11.017/11.340 s; 10.875/10.969 s, all <30 s. Year 500 living totals 1080/1063/1093; half-elves 145/126/118. All nine species survive with fertile partnerships; every annual species population <1500. Largest canonical state 4,002,785 bytes (<15 MiB).
- **Sweep:** Seeds 1–20 each completed 250 years; extinction/population/state-size gates PASS. Total living min/median/max 900/980.5/1029, largest recorded site population 187, largest state 1,919,810 bytes, slowest worker trajectory 1.913 s. Default suite/matrix/restart/sweep wall 146.355 s on Node 24.19.0 / Ryzen 7 8845HS.
- **Evidence:** [UF_History.md](systems/UF_History.md) records measurements, hashes, methods and limitations. Local `game/test_output/bench_species_biology.json` schema 4 is the final consolidated report (37,992,993 bytes; SHA-256 `837a3442312548d3e9ceede440d128f4df6072f7e210f9a831f61a1603d3ef61`); the <15 MiB gate concerns individual simulation states, not this multi-run report. RMMZ F5/F8, screenshots, native gameplay and real-save migration: NOT RUN; no gameplay claim.
...
- **Verifier scope**: `tools/bench_species_biology.js`, `tools/test_historical_carrying_capacity.js`, `tools/test_production_history_demographics.js`, `docs/systems/UF_History.md`, this Astra entry, and local `game/test_output/bench_species_biology.json`. Production plugins, catalogs, registration, real saves, and Fable's paths were not edited by Astra.
- **Contract results**: 18 PASS / 4 FAIL. Explicit custom biology can claim the promoted default profile tag. Widening caller-configured bounds to 1..1000 admits illegal capacity 59 in both `create` and `validate`. `capacityModel: 42` is accepted. These are observed rejection/provenance failures, not speculative findings.
- **PASS evidence**: Pure validation; supported schema/model checks; default/undefined profiles; ordinary malformed-capacity rejection; independent start-of-year density oracle including deaths, earlier births, and base birth chance 1; two-site same-species locality; deterministic capacity derivation; actual populated v6 migration preserving legacy records/profiles; and migration idempotence. All eight required production mutant controls exited 1 for their intended assertions. Regression self-test passed 34 checks and its four mutants; benchmark self-tests passed 26 checks and their four existing negative controls. Syntax and scoped whitespace checks passed.
- **Canonical matrix**: Three seeds, two independent processes per seed, continuous 100/250/500-year checkpoints: six complete trajectories and 18 measured checkpoints. All repeat state/event bytes, SHA-256 hashes, and annual curves matched. Each seed's 100-year checkpoint resumed in a different process for 150 years and matched its continuous 250-year state/event bytes and hashes.
- **Biology / size**: No extinctions at any recorded annual census; all nine species had active fertile partnerships at 250 and 500 years. Seed order 0 / 424242 / 20260919: year-500 world populations 1,080 / 1,063 / 1,093, half-elves 145 / 126 / 118, serialized state 3,966,484 / 3,989,088 / 4,002,785 bytes. All remained below the 1,500-per-species and 15 MiB gates. Maximum sampled heap delta was 94.29 MiB; no forced GC or leak claim.
- **Timing FAIL**: Whole-trajectory seconds by repeat: seed 0 32.247 / 35.060; seed 424242 24.011 / 25.143; seed 20260919 23.537 / 28.865. Both seed-0 runs exceeded the strict 30-second gate. Parent-measured sums of both workers were 68.144 / 49.928 / 53.209 seconds. Complete dispatch wall time, including suites/restarts/sweep/reporting, was 256.751 seconds. Failures were retained without rerunning for a faster result.
- **Broader sweep**: All seeds 1..20 completed 250 years. Observed extinction frequency was 0/20 for each species; total living population min/median/max was 900 / 980.5 / 1,029. Largest site population observed was 187; slowest trajectory was 2.301 seconds. This is descriptive finite-seed evidence, not a general viability guarantee.
- **Additional diagnostic**: Malformed v6 migration rejects an invalid capacity after changing the input schema from 6 to 7. Failed-migration atomicity is reported separately from the packet's explicit read-only-validation requirement. Existing regression fixtures now use a documented fixed TEST seed with birth draws checked under the actual 0.10 density floor; canonical benchmark seeds are unchanged.
- **Evidence / handoff**: The consolidated JSON now contains ASTRA-10 results; earlier ASTRA-08 measurements remain archived in the documentation and commit `8e17376`. Full capacities, per-species populations, timings, nine checkpoint hash pairs, sweep rows, mutant diagnostics, and sixteen evidence fields are in [UF_History.md](systems/UF_History.md). The packet refers to a sixteen-point coordinator checklist but does not supply it; the handoff uses explicit task requirements. Native RMMZ/NW.js gameplay verification and production activation were NOT RUN. Return the observed contract failures and timing evidence to Gemini/coordinator for a bounded implementation decision.
...
- **Scope**: `tools/bench_species_biology.js`, `docs/systems/UF_History.md`, this Astra status entry, and local `game/test_output/bench_species_biology.json` only. Production plugins, catalogs, gameplay, and saves were read-only. Executed dependencies are frozen at `931b993e60545b24bddaa71ab433ebac8e967eb8`; engine SHA-256 remains `647592fc4a65b474f5f12835cee80a461d1f0c86ba847559b3ec835791471c2e`.
- **Calibration**: Before each annual call, supply `baseBirthChance * max(0.1, 1 - startPopulation / 160)` through caller-owned profile data, then restore base probabilities. Exactly one site per species is asserted. Capacity 160 is below the packet's illustrative 200–500 range and is explicitly an experimental soft fertility input chosen to reduce archival pressure; it is not a hard cap or simulated resource ecology. Seven reproductive windows/rates were calibrated; all nine lifespan bounds remain unchanged. No synthetic rescue births or settlers were added.
- **Annual work**: Calls the unchanged public `step`, retaining one full validation per year instead of `simulate(state, 1)`'s duplicate validation. Self-tests prove exact state/event equality and JSON continuation. Checkpoint indexes group species and prospective mates by site/age for diagnostics only; production pairing already covers the full configured window and remains unchanged.
- **PASS checks observed**: Final `--selftest` passed 25 checks, including negative controls for density/coverage assumptions, extinction, missing fertile couples, saturation, serialized size, timing, randomness, invalid profiles, parentage, and timeout recovery. Syntax and allowed-file diff checks passed. The unchanged default command completed all three seeds, two independent repeats each, and 18 checkpoints at 100/250/500 years with exit 0. State bytes, complete event bytes, SHA-256 hashes, and annual curves matched exactly across repeats.
- **Biology**: Zero extinctions at every recorded annual census. All nine species had active fertile couples at years 250 and 500. Final world populations were 1,086 / 1,065 / 1,090 for seeds 0 / 424242 / 20260919; annual peak for any individual species was 167 / 151 / 169. Final half-elf populations were 140 / 119 / 122. These are finite-seed observations, not general equilibrium or ecological validation.
- **Runtime and size**: Full default wall time 169.631 s. Two worker processes per seed totaled 55.841 / 57.241 / 56.225 s; trajectory wall times ranged 25.893–29.121 s. Mean cumulative simulation at 100/250/500 years was 150.348 / 1,898.056 / 27,024.517 ms; worst annual call was 373.459 ms. Serialized state ranges were 546,062–614,376 / 1,829,225–1,865,608 / 4,033,764–4,066,082 bytes, below 15 MiB. Sampled heap delta peaked at 101.66 MiB; no forced GC or leak claim.
- **500-year hashes** (state / complete events, each matched its repeat): seed 0 `e25d4d8482cc8fd616f3e808a24fedccf11c4b89c7bd95a92be46ce7cce7fb56` / `e0c47f82b56255a631ec24db5e9ffe8b61dac37ae6186e6d2a3d97de8b3ee733`; seed 424242 `6f97047ad9d5e4d471c4363792cb1458a309b41fb2546b8fdc05d7dfd8d3e521` / `eb236ce4401717c58ac22ff90fce465901f6601d4b1468397e35f512b6d0745e`; seed 20260919 `172dcb490f0abff23de8a2cea77a61dbdd326213c27e974b8db7c71ef0826e3e` / `b8c371f610a3256e08c3a9390721c01e4849bf47ab783b3dce420e951b98a70a`.
- **Failed attempts retained**: Candidate 1 failed biological and trajectory-time gates. Candidate 2's one-repeat screening passed biology but seed 424242 took 31.589 s. Repairs stopped under Rule 10; the already-required default two-repeat verification then passed without changing the harness (same SHA-256 `5d8554948d57134d0ca2155ab7a4fd29efcbc922959b14390e1d59c4130dc4af`). No third repair or repeated timing search was performed. This observed timing variation remains a limitation.
- **Evidence and remaining scope**: The complete JSON artifact includes the passing default report and separately labeled session diagnostics preserving both prior screens. Full per-seed/horizon measurements, profiles, and hashes are in [UF_History.md](systems/UF_History.md). CPU profiling attributed about 90% of sampled time to historical validation; a tested alternate VM loader was slower and was not adopted. NW.js, RMMZ F5/F8, screenshots, broader seed sampling, and production integration were **NOT RUN**. Runtime/catalog adoption requires a separate task; no further optimization is included.

## DEUS-TSK-ASTRA-07 — Species biology measurements; 500-year acceptance incomplete (2026-09-23)
- **Status**: `INCOMPLETE — frozen-engine long-run limit`. Harness implementation and shorter matrices are committed for handoff; full 500-year acceptance is not claimed.
- **Scope**: New `tools/bench_species_biology.js`, `docs/systems/UF_History.md`, and this entry only. Production plugins, catalog values, seeds, registration, saves, and gameplay remain unchanged by Astra.
- **Inputs**: Reads immutable source/catalog/plugin-order inputs from commit `a97f606fdc689961a33030131fe2dc2d6b14862c`; workers verify source and harness hashes. Supplied profiles are measured without tuning. The packet's engine SHA differs from that commit; both are disclosed in the reports.
- **PASS**: Final `--selftest` 22 checks; all three required seeds at 100 and 250 years, two repeats per seed/horizon (12 trajectories), with exact state/event bytes, hashes, and annual curves matching. Custom seed/years/runs/JSON CLI also passed. Four deliberate invalid-input controls exit 1 for their intended assertions; immortal-survivor and birth-spacing controls are rejected. A real subprocess timeout preserves a validated checkpoint; an end-to-end in-memory 1 ms harness timeout produces exit 1 and explicit FAIL JSON. No production method was patched.
- **FAIL / incomplete default**: The default attempt exited 1 when seed 424242 repeat 1 reached the 30-minute worker limit (ETIMEDOUT); last progress was year 400, 46,235 historical records, 2,120 living half-elves. Seed 0's two 500-year runs had already matched, with 69,927 records and 25,451 living half-elves. Total default wall time was approximately 3,542 seconds. That driver lost its in-memory checkpoint metrics on failure and never reached seed 20260919; the reporting fix preserves validated checkpoints for subsequent failures. Revised full 500-year matrix: **NOT RUN**.
- **Biological findings**: At 250 years, humans and half-orcs were extinct in 2/3 unique-seed worlds each; dragonborn and tieflings in 1/3 each. The seed-0 500-year half-elf census is far above the disclosed 100x-founder growth diagnostic. Carrying capacity is not configured, replacement equilibrium is not established, and profiles are not approved for integration. Early disease/infant/exposure deaths are valid; only old-age deaths must meet the lower lifespan bound.
- **Final shorter measurements**: 100-year matrix wall 5.243 s; mean simulation total 105.692 ms, worst annual call 3.566 ms, state 157,987–244,741 bytes, sampled heap delta peak 7.76 MiB. 250-year matrix wall 13.803 s; mean simulation total 1,504.096 ms, worst annual call 56.303 ms, state 589,315–1,735,144 bytes, sampled heap delta peak 31.09 MiB. Setup/verification/I/O are outside simulation timing; heap samples include ordinary GC and retained measurement evidence, not a leak claim.
- **Artifacts**: `game/test_output/bench_species_biology.json` is an explicitly INCOMPLETE session handoff assembled from the inspected `bench_species_biology_100.json` / `bench_species_biology_250.json` reports and observed timeout diagnostics; it is not a completed default matrix. Self-test, CLI, and timeout-control JSON files use the same prefix. Ignored generated artifacts are not committed.
- **NOT RUN / decision needed**: Full three-seed 500-year lifespan/state-size/timing matrix after the reporting fix; native NW.js/RMMZ F5/F8 and screenshots. Choose an extended long-run resource budget or a separate frozen-engine performance task before further 500-year attempts. No optimization, profile calibration, HIST-03, or New Game integration was started.
...
## DEUS-TSK-FABLE-13 Closed — Autonomous Settlement Expansion & Multi-Dwelling Logistics (2026-09-24)
- **Status**: `DELIVERED — PASS on every acceptance gate (headless)`.
- **Commit**: `[fable] DEUS-TSK-FABLE-13 autonomous settlement expansion, parcel tidy, bed claims for newcomers`.
- **Ownership check**: written in `game/js/plugins/DEUS_Projects.js`, `game/js/plugins/DEUS_Colonists.js` (my hunks only: Gemini's concurrent uncommitted edits to the same file in this working copy were left unstaged, the commit blob being HEAD plus my two patches), `tools/test_settlement_expansion_multi_dwelling.js` (new), `tools/test_autonomous_work_recovery.js` (outside the packet's paths: Fable's own FABLE-09 harness, its walker only, reason below), `docs/systems/DEUS_Projects.md` and this section. `DEUS_History.js`, `DEUS_Fluid.js`, `DEUS_Callings.js`, `DEUS_Items.js`, the catalog and `plugins.js` untouched. Baseline `a10c596`.
- **What the plugins already did**: `shelter` needs `ceil(population / perShelter)`, `bed` needs `population`, `storage` needs `population × slotsPerColonist`, so nine to sixteen colonists read a second shelter as unmet and the brain opens another `communal_shelter`, then a `communal_stockpile` when slots run short; `siteValid` refuses any footprint whose square or margin ring holds a constructed object or a reserved square, so a second shelter never covers or touches the first, the camp ring or a stockpile; `harvestSources` indexes only unconstructed types and `sources` never lifts a stack off an active project's reserved square, so nothing is ever taken out of a standing building. (The packet's slot figures, 96 needed and 72 a stockpile, are the pre-GEMINI-07 model; the configuration is authoritative: one slot a colonist, one a stockpile cell, nine a 3×3 stockpile.)
- **What changed**: Projects: `structures()` (finished footprint projects and their ground); `tidyParcel` (DEUS-TSK-FABLE-13): once the site phase has cleared the parcel, loose material the clearing left on it is hauled to a registered stockpile cell with room (`stockpileCellFor`: `UF.Stockpiles.findDestination` when loaded, else the nearest registered cell whose `stores` take the item, holding nothing but the same kind under its stack limit, counting what is already on its way) and the walls phase waits for those hauls; a stack is offered at most `TIDY_TRIES` 3 times (a carrier may lift part of it). Colonists: bed claims (`data.bed` the single record; `bedClaims()` indexed once per update; `claimedBed`, `claimBed`, `allocateBeds` at a sweep after a bed is built, a project finishes or someone arrives; the sleep job uses the claim and never a bed another living colonist holds; a household's planned, unbuilt bed is left alone; beds are looked for within `max(radius + 6, 24)` of the hearth, so a second shelter beyond the camp ring is found).
- **Suite** (`node tools/test_settlement_expansion_multi_dwelling.js`; real Objects/Items/Jobs/Projects/Colonists, the calendar at 600 updates an hour, a walker that steps straight and paths round water only when blocked, four newcomers through `UF.Colonists.spawnImmigrants`): **12/12, exit 0** (2026-09-24). Shelter #1 and Stockpile #1 stand within a day with no `assign` or `order` call; the founders hold eight distinct claims on Shelter #1's beds; four arrivals read population 12, shelter 1/2, beds 8/12, storage 10/12 and the brain picks the shelter; Shelter #2 opens at the origin `chooseSite` predicted, the oak planted on it is felled by the project's own job, its logs go to Stockpile #1 by tidy hauls, and the walls phase begins with nothing loose on the parcel; Shelter #2 and Stockpile #2 finish with every deficit closed; four structures, 0 overlapping cells, a smallest gap of 2 (margin 1), none on the camp ring or the larder, doors walled at the sides with free ground outside, siting over Shelter #1's east wall and flush against it refused by its walls; no clearing job ever targeted a standing building, two oaks felled for the second shelter's wood, Shelter #1 and the camp ring intact, the material ledger balanced; founders keep their beds, the newcomers hold Shelter #2's, twelve distinct standing claims; two more days: 12/12 alive, everyone in their own bed, nobody hungry; the save round trip keeps projects, claims and structures; 0 console errors.
- **Rule 4 mutants** (each exit 1): `overlapping_footprint_allowed` (`siteValid` ignores standing buildings: the probes over and against Shelter #1 are accepted), `cannibalizes_existing_building` (walls count as a wood source: seven chops posted on the camp ring, 23/30 walls left), `bed_allocation_displaces_founders` (newcomers claim held beds: four checks fail).
- **Regressions** (2026-09-24, all exit 0): `test_autonomous_work_recovery.js` 13/13 with its four mutants still failing (2/1/2/1), `test_multi_deficit_settlement.js` 10/10, `test_settlement_projects.js` 14/14, `test_autonomous_project_dispatch.js` 14/14, `test_survival_needs_loop.js` 19/19, `test_hazard_reflex.js` 10/10, `test_hazard_torture_live.js` 10/10, `test_autonomous_settlement_closure.js` 22/22, `test_conditions_native_closure.js` 18/18, `test_stabilization.js` 12/12, `test_stockpiles_designation.js` 18/18, `test_project_construction_loop.js` 14/14, `test_starter_kit_and_stockpile.js` passed.
- **The recovery harness's walker** (the out-of-scope edit): under bed claims its founders slept on the floor fifteen times (three before) and its scene-D food cache never finished in six days. The cause was that harness's straight-line walker: founders who drank on the far side of the pond could never walk back, so their one claimed bed was out of reach every night (before, they cycled through beds until one happened to be reachable). With a walker that paths round water, as the game's does, the same cache finishes in 0.07 days and the suite is 13/13, so the walker was given that rule (a straight step when dry, breadth-first round water when blocked). A sidestep-only walker was tried and rejected: it oscillates at the pond's edge and two founders never slept at all.
- **Not done / known problems**: headless only, no F5 playtest or screenshot. Only the primary colony record is planned. A douse or tidy haul into a container needs `UF.Stockpiles`/`UF.Containers` loaded (the headless sandboxes use registered cells). `Projects.structures()` is not yet consulted by `UF_Households`' plot finder. Gemini is editing `DEUS_Colonists.js` in this working copy at the same time (survival avoid windows, water search, forensics); those edits were verified alongside mine in the tree runs and excluded from this commit.

## DEUS-TSK-FABLE-12 Closed — Environmental Torture Verification, Combat Threat Response & Idle Scan Bounding (2026-09-24)
- **Status**: `DELIVERED — PASS on every acceptance gate (headless)`.
- **Commit**: `[fable] DEUS-TSK-FABLE-12 environmental and combat torture suite, bounded idle scans, physical storage accounting`.
- **Ownership check**: written in `game/js/plugins/DEUS_Colonists.js`, `game/js/plugins/DEUS_Jobs.js`, `tools/test_hazard_torture_live.js` (new), `tools/test_settlement_projects.js`, `tools/test_multi_deficit_settlement.js`, `docs/systems/DEUS_Colonists.md` and this section. `DEUS_Callings.js`, `DEUS_Fluid.js`, `DEUS_History.js`, `DEUS_BuildingGrammar.md`, the catalog and `plugins.js` untouched. Baseline `28b0e5b`.
- **Torture suite** (`node tools/test_hazard_torture_live.js`, the real Objects/Items/Jobs/Colonists/Conditions/Environment/Combat over a World double that paths round water and objects, ignites what steps into fire, and lets DEUS_Environment burn the units): **10/10, exit 0** (2026-09-24). A hauler with five logs (35 lb) stepping onto a burning band beside the pond loses the haul `emergency: lethal hazard` in 4 updates, leaves the five logs on the square, gets a routed `move` to the square beside the water, douses itself there (`water`, 30 ticks), ends unburnt at 10/10 hit points and never steps into fire again. A worker with fire on every square within two but a bent corridor drops its chop `emergency: fire nearby` and walks the five-square corridor to a calm square with no step in fire (the straight way north is shorter and burns). A paralyzed friend aflame pulls a colonist off its chop `emergency: aid`; with a water skin it douses it in 35 updates (`water`), beside the pond another douses with pond water, with nothing a third smothers the flames in 123; all alive. An armed settler struck by a wolf drops its chop `emergency: threat`, holds (no job), faces west onto the wolf, is engaged on it through UF_Combat, and raises an alarm that reaches six; an armed friend six squares off stands where it is; the unarmed one struck runs for that friend (a square within two of it, farther from the wolf). A grapple holds at one square and breaks at two in all 128 grappler-and-victim runs. Eight idle founders read the job list 5 times in 300 updates and take a posted chop after 2. A corridor flight saved at a waypoint comes back identical and finishes unburnt. 0 console errors.
- **Rule 4 mutants** (each exit 1): `panics_into_fire_when_choked` (a step through fire costs 1: 2 fail, the route goes north through the flames), `unarmed_charges_threat` (1 fails: the refuge is a square beside the foe), `ignores_burning_ally` (2 fail: nobody douses, the friend burns).
- **What changed**: `UF.Jobs.safeCellNear` is a cost search (a step through a hazard costs 25, so the way round is taken within reach and the least fire crossed otherwise; a calm goal from safe ground; `preferWater` for the burning; returns the route). `move` jobs walk `params.route` waypoint by waypoint; reflex moves skip the encumbrance rule. `fireNear`, `carriesWater`, `isAflame`, the `douse` job (water 30 ticks with a carried skin or water beside, else smother 120; the patient reserved as in `stabilize`). Colonists: rung 2 also fires for fire or lava beside the colonist when a calm square is reachable (`emergency: fire nearby`); rung 3's armed hold faces the attacker, engages it through `UF.Combat.engage` within one square and raises the alarm (`colony.alarm`, `colonists:alarm`, `data.threat { alarm: true }` within `ALARM_RADIUS` 24: the armed stand to it, the unarmed run only from a foe within `THREAT_RADIUS`); the unarmed run for a refuge (`refugeFor`: the hearth or the nearest armed friend, farther from the attacker); rung 4 douses before it stabilizes, and the sweep pulls the nearest helper for a burning incapacitated colonist first. Idle scans: `openJobs()` reads the list once per update and filters by live state, a colonist that found nothing waits `IDLE_SCAN_INTERVAL` 60, an empty list is empty for everyone for that long, and a job posted (or back in the pool) clears every wait. `docs/systems/DEUS_Colonists.md` rewritten for all of it.
- **Storage accounting aligned** (GEMINI-07 physical occupancy: one slot per stockpile cell plus a container's slots, `slotsPerColonist` needed each): `tools/test_settlement_projects.js` **14/14** (was 13/14), `tools/test_multi_deficit_settlement.js` **10/10** (was 8/10); expectations derive from `UF.Projects.config()`.
- **Regressions** (2026-09-24, all exit 0): `test_hazard_reflex.js` 10/10 with its three mutants still failing (3/2/2), `test_autonomous_project_dispatch.js` **14/14** (`idle_scan_bounded` 2 open() calls in 120 updates, was 39 and failing), `test_survival_needs_loop.js` 19/19, `test_autonomous_settlement_closure.js` 22/22, `test_autonomous_work_recovery.js` 13/13, `test_stabilization.js` 12/12, `test_conditions_native_closure.js` 18/18, `test_stockpiles_designation.js` 18/18, `test_project_construction_loop.js` 14/14. **Not green, not this task's**: `test_starter_kit_and_stockpile.js` crashes at HEAD `5132e61` and at the baseline `28b0e5b` alike (`DEUS_History.js:381` now demands HistoricalDemographics, Callings and Dnd5e before New Game since Gemini's `e0f4e25`; that harness loads none of them), verified in clean worktrees. No `Math.random` in the plugin edits; engine core untouched.
- **Not done / known problems**: headless only, no F5 playtest or screenshot of these scenes. No water container item exists in the catalog (the suite adds a harness-only `TEST_waterskin`; `liquid: "water"` or the tags water + container/drink will count once one is added, and a skin is not emptied by a douse). An armed colonist engages only a foe within one square. An alarm holds the armed within 24 of the attacker off work for `THREAT_TICKS` (300). A colonist beside a fire with no calm square within 8 keeps working. The suite's walker is not the game's pathfinder, and DEUS_Environment's weather can rain a fire out in the sandbox after a few beats (the ally scenes finish well before).
- **Flag for review** (CLAUDE.md, Gemini edits to code): `e0f4e25` touched `DEUS_Core.js`, `DEUS_Levels.js`, `DEUS_Objects.js`, `DEUS_History.js` (Astra's claim) and `tools/test_stockpiles_designation.js` beside its own new files; `28b0e5b` rewrote the survival harness's walker and expectations. Not reviewed line by line here.

## DEUS-TSK-FABLE-11 Closed — Autonomous Survival Competence and Immediate Hazard Reflex Layer (2026-09-24)
- **Status**: `DELIVERED — PASS on every acceptance gate; one side effect on Gemini's survival harness reported below`.
- **Commit**: `[fable] DEUS-TSK-FABLE-11 autonomous survival competence and immediate hazard reflex layer`.
- **Ownership check**: written in `game/js/plugins/DEUS_Colonists.js`, `game/js/plugins/DEUS_Jobs.js`, `game/js/plugins/DEUS_World.js` (the `world:unitMoved` emit only), `tools/test_hazard_reflex.js` (new), `tools/test_stabilization.js` (result capture), `docs/systems/DEUS_Colonists.md` (new) and this section. `DEUS_Stockpiles.js`, `DEUS_Levels.js`, `DEUS_WorldGen.js`, `DEUS_HistoricalDemographics.js`, the catalog and `plugins.js` untouched. `tools/test_survival_needs_loop.js` was instrumented twice for diagnosis and restored to HEAD each time (`git checkout`).
- **The order** (`decide` for the idle, `scan` for the busy, `assess(u)` to read it): 1 unable (unconscious, `UF.Conditions.canAct` false) · 2 lethal hazard (a burning square, lava, deep water, or aflame: `UF.Jobs.inLethalHazard`) · 3 hostile threat (`combat:hit` from a non-colonist within `THREAT_TICKS` 300 / `THREAT_RADIUS` 12) · 4 aid · 5 critical survival (`urgent`, a meal hour with the day's food or water short, starvation past the SRD grace, hypothermia) · 6 a draft or manual combat mode · 7 work · 8 routine needs · 9 idle.
- **Reflexes**: on a hazard square the colonist gets a `move` to `UF.Jobs.safeCellNear` (rings by distance, standable, no hazard, nobody on it; `params.reflex "hazard"`); aflame on safe ground it takes the new `extinguish` job (`roll` 120 ticks, or `water` 30 ticks beside water; `UF.Environment.extinguishUnit`). `scan` cancels any non-reflex job in a hazard `emergency: lethal hazard` every sweep; `UF_Jobs.step` fails such a job on its own before planning. A threatened armed colonist (not set to flee) holds for `UF_Combat` and takes no work; an unarmed one runs to the square farthest from the attacker within 8 (`params.reflex "threat"`); work is cancelled `emergency: threat` once per `PREEMPT_EVERY`. A meal hour with food or water short interrupts labour `survival: meal` once per `PREEMPT_EVERY`. Hazard objects count only when one can stand in them (`passable`): a hearth or kiln is never stood in, and the sandboxes' straight-line walking crossed the hearth and cancelled work until that rule was narrowed.
- **Grapple stepping**: `DEUS_World` emits `world:unitMoved(u, from, to)` from `stepAlongPath`, the off-screen step, `moveUnitToArea` and the on-screen sync (only on a real square change); `UF.Colonists.onUnitMoved` hands it to `UF.Conditions.onUnitMoved` for units that grapple or are grappled.
- **Stabilization repair**: `medicine_check_stabilizes` read the stabilize job's `result` through `UF.Jobs.get` after the fact; `UF_Jobs` prunes finished jobs (40 kept) and the idle fallback's strolls finish by the dozen, so the record was gone. The result is now captured at `jobs:done`: **12/12**.
- **Evidence** (2026-09-24, working tree at `2f22f9e` plus this task):
  - `node tools/test_hazard_reflex.js`: **10 passed, 0 failed (exit 0)**: a hauler stepping onto a burning row loses the haul `emergency: lethal hazard` and gets a `move` to a safe square one update later, safe after one more; with fire closing in again its first ordinary job begins on safe ground; nine burning squares around it → flight to distance 2, never a burning square; aflame: `roll` on open ground, `water` beside the pond, both out, hit points kept; a grappler's second step releases the victim with no `Conditions.tick`; an unarmed colonist runs 15 squares from the wolf, an armed one loses its chop `emergency: threat` and holds (`assess` 3 "holds ground"); `assess` reads 1..9 for a state per rung; a flight saved mid-way comes back (units and the reflex job identical, threat kept) and ends safe; 0 console errors.
  - Rule 4 mutants (each exit 1): `ignores_fire_while_hauling` 2 fail, `flees_into_fire` 1 fails, `grapple_persists_on_step` 1 fails.
  - Required suites: `test_stockpiles_designation.js` **18/18**, `test_starter_kit_and_stockpile.js` **ALL CHECKS PASSED**, `test_conditions_native_closure.js` **18/18**, `test_autonomous_settlement_closure.js` **22/22**, `test_stabilization.js` **12/12** (all exit 0). Also `test_project_construction_loop.js` 14/14.
  - Pre-existing, identical with this task's plugins stashed: `test_settlement_projects.js` 13/14 and `test_multi_deficit_settlement.js` 8/10 (`Storage 0/8` and `10/8`: GEMINI-07's storage accounting changed under Fable's FABLE-02/06 expectations, not in this task's paths), `test_autonomous_project_dispatch.js` 13/14 (`idle_scan_bounded`, since `5f67bee`).
- **Side effect to decide on**: `tools/test_survival_needs_loop.js` (Gemini's) goes from 18/19 to **14/19** with the new order, deterministically (probed 2026-09-24 on throwaway copies of the harness, the current tree against a worktree at `2f22f9e`): `drinks_a_gallon_a_day` and `eats_a_pound_a_day` (its calendar stays at 10:00 for the whole scene, so no meal hour ever comes; founders #4 and #5 work the open shelter project instead of drinking first, as rung 8 says, and drink at 19:00 in a later scene); `labor_resumes_after_supper` (at 19:00 the project has no open job in either tree, `projectJob` null; the baseline hauler had taken the last gather 11 updates after supper, the current one meets none and holds the idle fallback's fire-gathering `move` for the whole 900-update wait, so the check's "no job and nothing open" clause never fires); `long_rest_rules` (the shifted timeline leaves founder #8 at (47,33) east of the pond when the scene begins; it finishes a drink at (47,32), its bed is at (34,32), and the harness's walker drops the goal at the first water square, so the sleep stalls `can't reach it`; in the baseline #8 began the scene at (32,29) inside the camp). The order is the packet's; the harness encodes the earlier "needs first at any hour" and walks in straight lines. Either the harness advances its clock through a meal hour and paths around the pond, or the coordinator restores needs above work.
- **Not done / known problems**: headless only, no F5; drowning relies on `UF.Levels.isFlooded` reporting `deep`/`depth` (no such flood exists in the current world); `warmthJob` needs `UF.Environment` loaded; a colonist trapped with no safe square within 8 stays and logs a thought.

## DEUS-TSK-FABLE-10 Closed — Native Condition Behavioral Verification (2026-09-24)
- **Status**: `DELIVERED — PASS on the task's gates in a clean checkout of the baseline plus this task's files; the working tree carries Gemini's in-flight GEMINI-07 edits, which currently break the stabilization and closure suites (below)`.
- **Commit**: `[fable] DEUS-TSK-FABLE-10 native condition behavioral verification and closure`.
- **Ownership check**: written only in `game/js/plugins/DEUS_Conditions.js`, `tools/test_conditions_native_closure.js` (new), `docs/systems/DEUS_Conditions.md` (new) and this section. `DEUS_Stockpiles.js`, `DEUS_Projects.js`, `DEUS_Containers.js`, `DEUS_Jobs.js`, `DEUS_World.js`, `DEUS_Levels.js`, `DEUS_WorldGen.js`, `DEUS_HistoricalDemographics.js`, the catalog and `plugins.js` untouched.
- **Contracts sealed (DEUS_Conditions.js)**:
  1. Grip: unconscious and petrified drop what they hold (`DROPS_HELD`) onto their own square through `Items.putDown` (live item ids in `mainHand`/`offHand` and the older aliases) and clear the slot, with `condition:dropped_items`; incapacitated, stunned and paralyzed keep their grip. `combat:downed` drops the items of a colonist at 0 hit points.
  2. Grapples: the grappler indexes its victims (`unit.data.grappling`); an incapacitating condition on it ends every hold at once on infliction (before any tick); `colonists:died`, `combat:kill`, `world:unitRemoved` and `onUnitMoved` (more than one cell away) end them too; `tick` on the victim still catches a dead or distant grappler.
  3. Frightened: `canWillinglyMoveTo` refuses every closer square with or without sight; the attack and check disadvantages apply only while a source is in `lineOfSight` (Bresenham over `wall`/`door` objects and solid rock; `setLineOfSight` for a better test; `opts.fearSourceVisible` forces).
  4. Charmed: `socialCheckModifiers(actor, target, ability, skill)` gives the charmer advantage on persuasion, deception, intimidation and performance against the creature it charmed; `canHarmfullyTarget` keeps refusing the charmer and Combat refuses the attack.
  5. Dying vs stable: `isStable(unit)` and the 0-hit-point reflection (`dying`, `stable`) read `UF_Colonists`' record; a stable patient rolls no death saving throws and wakes when its hours are up (proven on the real plugin).
  6. Instances: one penalty per condition; removing one instance leaves the other to its own expiry.
  7. Save/load: indefinite durations are `null` (Infinity would not survive JSON), instance ids come from a per-unit sequence that rides in the record, and a JSON round trip is byte for byte the same.
- **Evidence** (2026-09-24, in `git worktree add <tmp> HEAD` at `e08aae2` with this task's two files copied in, because the working tree's uncommitted `DEUS_Colonists.js` throws `stockpilesStoring is not defined` from every idle decision):
  - `node tools/test_conditions_native_closure.js`: **18 passed, 0 failed (exit 0)** (axe and club on the sleeper's square, slots cleared; petrified drops, half damage; three grips held; downed drops; five incapacitating conditions end the hold at once, poison keeps it; death, removal, a three-cell move end it; closer refused with and without a wall; disadvantage on in sight, off behind three wall cells, forced by the option, back when the wall goes; charmer advantage on persuasion and deception only against the charmed; Combat refuses the charmer and resolves a stranger; stable patient 0/0 saves after five rounds, woke at 4 h with 1 hp; two poisons one disadvantage, the cloud outlives the removed bite and expires at its tick; 1091-byte record identical after the round trip, expiry at t+300 exact, new id unique).
  - Rule 4 mutants (each exit 1): `stunned_drops_item` 2 fail, `grapple_survives_incapacitated` 1 fails, `frightened_allows_advance` 2 fail, `charmed_allows_attack` 2 fail.
  - Regressions in the same worktree: `test_conditions_system.js` **55/55 (exit 0)**, `test_combat_dying_integration.js` **30 passed, 0 failed**, `test_autonomous_settlement_closure.js` **22/22 (exit 0)**, `test_stabilization.js` **11/12**: `medicine_check_stabilizes` fails identically with HEAD's own `DEUS_Conditions.js` (pre-existing since `5f67bee`, reported under FABLE-09).
  - In the working tree as it stands (Gemini's uncommitted GEMINI-07 edits to Colonists, Jobs, Projects, World, plus the untracked `DEUS_Stockpiles.js`): `test_conditions_system.js` 55/55, but `test_stabilization.js` 10/12 and `test_autonomous_settlement_closure.js` 16/22 (deaths of hunger and thirst, `storage 10/8 slots`), all from `UF_Colonists: decide failed ReferenceError: stockpilesStoring is not defined` (`DEUS_Colonists.js` line 1186, used before its `const` at line 1180 is in scope) and the changed storage model. Not this task's files.
- **Findings**: `DEUS_Conditions.js` is not registered in `game/js/plugins.js` (nothing in the engine runs it; Combat, World and Colonists look it up lazily). The `stack_effects` mutant of `tools/test_conditions_system.js` never changes behaviour (55/55 with and without it, before and after this task).
- **Not done / known problems**: headless only, no F5; `DEUS_World` does not call `onUnitMoved` (the victim's `tick` covers it); every `door` counts as opaque; no "stabilized" instance exists, stability is read from `UF_Colonists`.

## DEUS-TSK-FABLE-09 Closed — General Autonomous Work Recovery (2026-09-23)
- **Status**: `DELIVERED — PASS on the task's own gates; three pre-existing regression failures outside Fable's files reported below`.
- **Commit**: `[fable] DEUS-TSK-FABLE-09 general autonomous work recovery`.
- **Ownership check**: written only in `game/js/plugins/DEUS_Projects.js`, `tools/test_autonomous_work_recovery.js` (new), `docs/systems/DEUS_Projects.md` and this section. `DEUS_Colonists.js`, `DEUS_World.js`, `DEUS_Objects.js`, `DEUS_Jobs.js`, the catalog and Astra's files untouched.
- **What recovers now (DEUS_Projects.js)**:
  - Failure classification: a failed job counts against its square only for a square fault (`can't reach …`, `nowhere to take it`, `the square is taken`); a vanished, carried or moved item, a picked plant, missing materials, a departed worker, a survival interruption or a stale withdrawal never pause a square.
  - Stale withdrawal for every job kind: hauls (item gone / carried / boxed / moved), harvests (plant without the action), builds (square already built or blocked by somebody else, materials gone from the square: the logs are hauled again instead of a build job claiming the square forever), and any haul, harvest or build every colonist found undoable (`UF_Jobs`' dry run) for `staleTicks`; a target withdrawn for its dry-run reason waits `retryTicks` (the reason is read before cancel overwrites it).
  - Refused deliveries: a haul `UF_Jobs` finished without a result (a full larder or container) marks the destination `refused` for `retryTicks` (`domain: "action"` stamps); the food cache hauls to another registered larder meanwhile and offers the first again when the refusal expires; a build square that refuses is reported.
  - Paused, not frozen: when nothing of a phase is workable (blocked squares, materials nothing within reach yields, squares waiting to retry, refused deliveries) the project records `blockedSince`, reports the reason in `describe()` ("paused since tick …"), re-looks every ten rechecks, emits `projects:paused`/`projects:resumed`, and after `giveUpTicks` (default three days, 0 = never) cancels itself with `gave up: <reason>`, cooling its kind; no job or reservation of it remains.
  - Idle strolls yield: for every project job posted and still untaken, the nearest colonist on a low-priority idle job (stroll, exploration, inspection, contemplation, hearth or social idling; never a player's order, `params.ordered`) is taken off it (`work: project posted`); `UF_Jobs`' cancel releases its reservations and `UF_Colonists` decides again at once. (`UF_Colonists`' own `work: preempt idle` branch did not fire in the sandbox: `projectJob(u)` returns nothing for a colonist that already holds a job.)
- **Evidence** (2026-09-23, production clock):
  - `node tools/test_autonomous_work_recovery.js`: **13 passed, 0 failed (exit 0)**: three hauls lose their stack on the way (`the item is gone` ×3, 0 squares with a failure count, walls 16/16); an open build job's logs vanish → `stale: the materials are gone` after 1 update, hauled again, wall standing; a gather's plant picked before arrival → slot freed, no failure count, and a walled-in tree withdrawn `stale: can't reach it` once per `retryTicks` with the failure record; a straw floor on a wall square → 15/16 built around it, "Straw bed is in the way; paused since tick 854", cleared → done; a refusing larder → refused record, 0 hauls to it while refused, food at the second larder, hauls to it again after expiry; 5/8 idling founders → 5 idle jobs cancelled `work: project posted`, 4 founders on project jobs within 1 update, 0 leaked reservations, 0 orphans; no straw anywhere → beds phase paused "no straw within reach", a player designation still done meanwhile, straw dropped → resumed → done; a square blocked for good → `cancelled (gave up: Straw bed is in the way)` after 12.1 h, kind cooled, 0 live jobs, 0 reservations; every timer stamp `domain: "action"`; identical record on a second run from the same seed; 0 console errors.
  - Rule 4 mutants (each exit 1): `no_withdrawal` 2 fail, `count_world_failures` 1 fail (4 squares with a failure count), `no_give_up` 2 fail, `no_wake_idle` 1 fail.
  - `node tools/test_autonomous_settlement_closure.js`: **22/22 (exit 0)** (mutants 5/1/13 fail); `test_multi_deficit_settlement.js` 10/10 (mutants 1/7/1 fail); `test_settlement_projects.js` 14/14; `test_project_construction_loop.js` 14/14.
- **Findings (not caused by this task; same result with HEAD's `DEUS_Projects.js` restored via `git stash`, 2026-09-23)**:
  - `tools/test_autonomous_project_dispatch.js` 13/14: `idle_scan_bounded` sees 39 `UF.Jobs.open()` calls in 120 updates with 8 idle founders (bound 20): since `5f67bee` every idle colonist takes a stroll and the sweep's `isLowPriorityJob(job, u) && (projectJob(u) || …)` scans the open jobs for each of them every 30 ticks. The bounded-idle rule (AGENTS 14: no global scans per frame) is broken in `DEUS_Colonists.js`; the guard is left as it is.
  - `tools/test_survival_needs_loop.js` 16/19 with HEAD's plugin (`nutrition_and_water_are_not_weight`, `supper_preempts_labor`, `labor_resumes_after_supper`), 18/19 with this task's plugin (idle strolls are called back to the hauls); `tools/test_stabilization.js` 11/12 (`medicine_check_stabilizes`). Both harnesses and `DEUS_Colonists.js` are Gemini's; the failures appeared with the idle-stroll fallback.
- **Not done / known problems**:
  - Headless only; no native F5 run of the recovery paths.
  - Only project jobs are withdrawn when undoable; player designations that become undoable stay open (`UF_Jobs`).
  - A refused build square is reported and paused, not re-sited.
  - Sandbox founders walk a cell per update; the give-up window in the harness is 6000 ticks (config `giveUpTicks`), the default is three days.

## DEUS-TSK-FABLE-08 Closed — Production-Clock Autonomous Colony Closure (2026-09-23)
- **Classification**: `PASS` — **FIRST SELF-MAINTAINING DEUS COLONY — PASS.** `node tools/test_autonomous_settlement_closure.js` on the default production clock (600 updates an hour, no `--hourTicks` override): **22 passed, 0 failed (exit 0)**; all eight founders and both newcomers alive after 4.5 unattended calendar days.
- **Commit**: `[fable] DEUS-TSK-FABLE-08 production-clock autonomous colony closure` (this entry staged as a hunk on HEAD; other agents' uncommitted STATUS lines stay out).
- **Ownership check**: written only in `game/js/plugins/DEUS_Projects.js`, `tools/test_autonomous_settlement_closure.js`, `docs/systems/DEUS_Projects.md` and this section. `tools/test_multi_deficit_settlement.js` unchanged. `DEUS_Core.js`, `DEUS_TimeSpeed.js`, `DEUS_Colonists.js`, `DEUS_Combat.js`, Astra's history files untouched; the timebase repair `f1e74e5` was consumed as frozen.
- **Root cause found** (not the one in the packet's diagnosis alone): the food cache's four harvest slots and its haul slots filled with open jobs nobody could take: hauls whose stack a colonist had already eaten or carried (`the item is gone`), gathers whose bush somebody had picked (`nothing to gather there`). `UF_Jobs.take`'s dry run marks such jobs with a reason but leaves them open, and the project never withdrew them, so foraging stalled while 45–62 regrown plants stood and the reserve fell 2.76 → 2.01 days over three days; the in-flight cache then never finished (both packet failures). Evidence: the harness day lines now print untaken project jobs by reason (`untaken {"haul (the item is gone)":7,"gather (nothing to gather there)":1,"haul":3}` on days 8–10 before the fix).
- **Fix (DEUS_Projects.js)**: `reconcile` withdraws stale jobs before its sweep (`staleReason`: item gone / carried / in a container / moved; plant without the action; a haul or harvest with a dry-run reason older than `staleTicks`, default 600 = one game hour), cancelling with `stale: …`, which like `survival: …` never counts against a cell; a harvest target withdrawn for its reason waits `retryTicks`; `forageStep` and `postHarvests` skip retrying cells; posted jobs carry `params.postedAt`. Config `staleTicks` added.
- **Harness (tools/test_autonomous_settlement_closure.js)**: Section H recognises a food cache already in flight at the wipe as the recovery vehicle (the brain's food candidate shows it in flight and ineligible for a second) or a new cache when none is; the wipe time is recorded when it happens; Section K allows one food cache in flight at the end (the daily foraging rhythm) while every build must be done; day lines list untaken project jobs by reason.
- **Evidence** (2026-09-23, production clock unless stated):
  - Closure: 22/22 (exit 0). Shelter done after 0.54 days (16 walls and door, 8 beds, hearth; 140 project jobs, 0 assign calls, 0 orders); stockpile 9/9 registered, storage 80/64; food cache done, 40 gathers, reserve 3.581/3; save/load identical (2167 bytes), work resumed within 0.99 days; maintenance: at most 1 project active, 15 live jobs, 1 food cache opened in 2 days, 95 meals and 32 long rests; food wiped at day 5 21:00 → critical → one new cache → 3.513 days with 8 alive after 0.11 days; stockpile lost → replaced after 0.28 days, storage 80; two newcomers → one bedding project of capacity 2, beds 10/10; 14 project jobs cancelled `survival: rest`/`thirst`, 0 counted against a cell, 0 builds active, 0 caches in flight at the end; 37 long rests, longest 8.1 h; nobody died; ledger balanced at founding (logs 19, stone 3, straw 16 built) and at the end.
  - Closure `--hourTicks=3600`: 22/22 (exit 0).
  - Closure mutants (exit 1 each): `carried_not_counted` 5 fail, `survival_counts_as_failure` 1 fail (5 survival cancellations seen as cell failures), `no_autonomy` 6+ fail (0 project jobs done).
  - Regressions (exit 0): `test_multi_deficit_settlement.js` 10/10 (mutants 1/7/1 fail, exit 1), `test_settlement_projects.js` 14/14, `test_autonomous_project_dispatch.js` 14/14, `test_project_construction_loop.js` 14/14, `test_survival_needs_loop.js` 19/19, `test_stabilization.js` 12/12.
- **Invariants kept**: `totalAccessibleNutrition` = larders + containers + accessible ground food + packs of living colonists (each stack once); `communalStoredNutrition` = larders + containers (checks `carried_nutrition_counted`, `deposit_moves_not_adds`, `no_double_count` unchanged and green). The +72-slot virtual stockpile model is untouched (no physical stockpile migration).
- **Not done / known problems**:
  - Headless proof only in this task; the in-game `settlement` suite (8/8 on 2026-09-23 under FABLE-07) was not rerun, and no native F5 run.
  - A food cache opens about once a day in a quiet settlement (eight suppers take a colonist-day off a three-day reserve); bounded, never two at once, but a standing "keep the larder full" project would be quieter.
  - Hunting and fishing are not forage sources; the settlement lives on regrowing wild plants (`DEUS_Objects` regrowth) within 40 cells.
  - `UF_Jobs.take` still leaves undoable open jobs marked but open; only project jobs are withdrawn (by this planner). Player designations that become undoable stay open.

## DEUS-TSK-FABLE-07 Closed — Self-Maintaining Settlement Closure (2026-09-23)
- **Status**: `DELIVERED — headless closure PASS on the clock the colonists assume; FAILS on the engine's own calendar because of a Colonists constant (BLOCKER, AUDIT_LOG A8)`. The brain, the projects and the founders' autonomy close the loop; the survival timing in `DEUS_Colonists.js` does not survive the real calendar.
- **Commit**: `[fable] DEUS-TSK-FABLE-07 self-maintaining settlement closure` (this entry staged as a hunk on HEAD so other agents' uncommitted claims stay out).
- **Ownership check**: written only in `game/js/plugins/DEUS_Projects.js`, `tools/test_autonomous_settlement_closure.js` (new), `tools/test_multi_deficit_settlement.js` (one fixture option), `docs/systems/DEUS_Projects.md`, plus two files outside the packet's list, both small and stated here: `game/js/plugins/DEUS_Test.js` (the 23-line test clock, because `UF_Time.js` is not registered in `plugins.js` and the live `UF.Time` is DEUS_TimeSpeed's) and `docs/systems/UF_Test.md` (one table row). `DEUS_Colonists.js`, `DEUS_Core.js`, `DEUS_TimeSpeed.js` untouched; `UF_Time.js` was edited and restored to HEAD once the registration gap was found.
- **Task A, carried food**: `evaluateDeficits` counts every food stack once (a `seen` set over containers, larders, ground and packs) by `food.nutrition × count`, never weight. `food.totalAccessibleNutrition` (larders + containers + ground + the packs of living colonists) drives `current`, `deficit` and `critical`; `food.communalStoredNutrition` (`storedLb`, `communalDays`: larders and containers) is reported apart for logistics. Population and holders are living colonists only (`!u.data.dead`). Removing carried food raises the deficit by exactly its nutrition; depositing it in the larder moves it (total unchanged). The founders' shipped starting kit (clothes, pouch, 15 gp) carries no food, so in the real world the count comes from what the world drops; the in-game suite adds and removes two rations to prove the path.
- **Task B, clock and preemption**: preemption stays `UF_Colonists`' (untouched). `DEUS_Projects.reconcile` no longer counts a `survival: …` cancellation as a cell failure (the job is reposted; nothing pauses). `UF.Time.setForTest(hour, minute?, day?)` is installed by `DEUS_Test` only in a test run, sets `$ufTime` through `Game_UFTime.setTime`, resets its minute timer and leaves the calendar running; `null` outside a test. The brain's `eligible` now needs a positive utility (no second project of a kind beside one in flight) and a food cache forages to the target plus `reserveMarginDays` (0.5) so one supper does not reopen a cache at once.
- **Task C, evidence** (2026-09-23):
  - `node tools/test_autonomous_settlement_closure.js --hourTicks=3600`: **22 passed, 0 failed (exit 0)**. Founders arrive 19:30 with 3 rations each: brain `communal_shelter:5.00 bedding_expansion:1.50 communal_stockpile:0.88 food_cache:-`; shelter done after 0.08 days (16 walls and door, 8 beds, hearth; 109 project jobs, 0 assign calls, 0 orders); 3×3 stockpile done, 9 cells registered, storage 80/64; food cache done, 25 gathers, reserve 3.588/3 days; `no_unneeded_bedding` beds 8/8, candidate unmet 0; save/load round trip identical (2180 bytes) and 8 more jobs within 0.33 days; maintenance 2 days: at most 1 active, 8 live jobs, 2 food caches opened; food removed at 21:33 → critical → new cache → 3.519 days with 8 alive; stockpile removed → storage 80 → 8 → one replacement built, storage 80; two newcomers → bed deficit 2 → one bedding project of capacity 2, 2 beds laid, beds 10/10; 4 project jobs cancelled `survival: thirst`/`survival: rest`, 0 counted against a cell; 26 long rests, all 8.0 h; nobody died; ledger balanced at founding (logs 19, stone 3, straw 16 built; 8 lb eaten) and at the end.
  - Mutants (`--hourTicks=3600`, each exit 1): `carried_not_counted` 5 fail (packs read 0 lb, the brain opens a food cache first); `survival_counts_as_failure` 1 fail (3 survival cancellations seen as cell failures); `no_autonomy` 11 fail (0 project jobs done, nothing built).
  - `node tools/test_autonomous_settlement_closure.js` (default `--hourTicks=600`, DEUS_Core's calendar): **16 passed, 6 failed (exit 1)**: `long_rest_one_night` (every long rest 48.0 calendar hours, day 1 20:01 to day 3 20:02), `nobody_died` (all eight of thirst on days 7–8) and the four phases after the deaths; the founding itself passed (shelter after 2.16 days, stockpile, food cache, 6 survival interruptions recovered). Cause and fix in `docs/AUDIT_LOG.md` A8 (`DEUS_Colonists.js` `TICKS_PER_HOUR = 3600` vs 600 updates an hour).
  - Regressions (exit 0): `test_multi_deficit_settlement.js` 10/10 (mutants 1/2/1 fail, exit 1), `test_settlement_projects.js` 14/14, `test_autonomous_project_dispatch.js` 14/14, `test_project_construction_loop.js` 14/14, `test_survival_needs_loop.js` 19/19, `test_stabilization.js` 12/12.
  - In-game `settlement` suite on snapshot copies (`node tools/test_snapshot.js --name <n> --suite settlement`, other agents' uncommitted plugin edits included): run 6 (speed step ×8): **8 passed, 0 failed (exit 0)**. `clock_set_for_test` 08:00 → 08:03 after 30 frames; `carried_food_counted` total 24 → 26 → 24 lb; `autonomous_work_done` 38 project jobs done in 72 s without an order (gather 22, build 19, haul 11, chop 7, hunt 5; 51 meals, 8 drinks), `communal_shelter` and `communal_stockpile` opened by the brain, `Shelter 1/1 · Food 3.031/3 days · Beds 0/8 · Storage 320/64`; `project_completed_unattended` Communal stockpile #2 at (131,119)-(133,121) done at 14:32 of calendar day 1; no survival interruption by then; `save_reload_equivalent` saved to slot 19, loaded, project record identical (1596 bytes), work resumed. Earlier runs: run 1 6/8 (no `UF.Time.setForTest`: the hook was in the unregistered `UF_Time.js`; the reload threw), run 2 7/8 (reload threw `AudioManager.playBgm` of null because the suite skipped `$gameSystem.onBeforeSave`, fixed), runs 3–5 at ×32 ended the game process without a trace after 38–71 s (`game_runtime.log` stops mid-run, no error, no exit hook), so the suite runs at ×8 and does not re-create the map scene after loading.
  - Screenshots opened: `settlement.site.png` of runs 1 and 2 show the wooden stockpile cells laid in a block near the hearth, colonists with health bars, stone piles, bushes, a hare and rain; nothing of the shelter yet (its site phase was still clearing).
- **Not done / known problems**:
  - Not a full process restart: the in-game save/reload is `DataManager.saveGame` → `loadGame` → `Scene_Map` in the same process; the headless round trip is `makeSaveContents` → JSON → `extractSaveContents`. A cold start from the save file was not run.
  - Native F5 in the editor: not run; in-game evidence is the snapshot runner (NW.js, `--uf-test`). Runs can be cut short by other agents' `run_tests.js` timeouts, which kill every `nw.exe` (run 3 died at 38 s with code 0).
  - The autonomous-rescue closure (`test_stabilization.js`) was run as a regression only; no stabilisation scenario is in the closure harness.
  - Hunting and fishing are not forage sources; wild food must regrow (`DEUS_Objects` does) for a settlement to hold.
  - The in-game runs only reached calendar day 1 (70 s of ×32 is six hours: the game does about 95 map updates a second at that step), so no long rest was observed in the engine; the 48-hour rest is a headless finding until a longer native run confirms it.

## DEUS-TSK-FABLE-06 Closed — Multi-Deficit Settlement Brain (2026-09-23)
- **Status**: `COMPLETED — PASS` headless (10 checks, 3 mutants); in-game `projects` suite 9/9 on a snapshot copy, after one clock-dependent failure on the first run (below). The food cache, bedding and stockpile kinds are proven headless only; not yet watched in a native F5 run.
- **Commit**: `[fable] DEUS-TSK-FABLE-06 multi-deficit settlement brain` (this entry is staged as a hunk on top of HEAD so other agents' uncommitted claims in this file stay out of the commit).
- **Scope** (Fable sole writer): `game/js/plugins/DEUS_Projects.js`, `tools/test_multi_deficit_settlement.js` (new), the four regression harnesses (`test_settlement_projects.js`, `test_autonomous_project_dispatch.js`, `test_project_construction_loop.js`, `test_survival_needs_loop.js`: they switch the three new blueprints off in their in-memory catalog so the shelter-only fixtures stay deterministic; `deficit_year1` reads the storage and food rows), `docs/systems/DEUS_Projects.md`. The competing hybrid stays archived under `archive/preserved/` and was not merged.
- **Design**:
  - `evaluateDeficits` measures four rows `{ needed, current, deficit, unit }`: `shelter` (doors set into walls), `food` in colonist-days of nutrition (`food.nutrition × count` of every food stack in larders, containers and on the ground within `scanRadius`, divided by the population; never weight; `critical` under one day), `bed` (beds on sheltered cells only: the camp's interior and finished communal shelters), `storage` in slots (container `maxSlots` plus stockpile cells × 8; needed 8 per colonist).
  - Four blueprints: `communal_shelter` (5×5, capacity shelter 1 + bed 8), `communal_stockpile` (3×3 stockpile cells storing wood/stone/metal/material, capacity 72 slots), `bedding_expansion` (task: straw beds on free sheltered cells, capacity = cells), `food_cache` (task: a `stockpile` larder storing food, then a forage phase that hauls loose food into it and keeps up to 4 gather jobs alive on catalog food sources until the reserve reaches `targetReserveDays`; three empty cycles cancel it and cool the kind).
  - Brain: `unmet = max(0, deficit − Σ capacity of active projects for that deficit)`; `utility = severity × min(1, unmet / needed) + survivalBonus − activeOfKind × 10`, `−∞` when nothing is unmet; severity shelter 3, food 2 (4 critical), bed 1.5, storage 1; bonus 5 for critical food, 2 for an unmet shelter; one project per cycle, kinds that fail to open cool for 6000 ticks. `UF.Projects.brain()` and `explain()` show the ranking for `UF_Look`/`UF_Sheet`.
- **Evidence** (all run 2026-09-23 from the working tree):
  - `node tools/test_multi_deficit_settlement.js`: **10 passed, 0 failed (exit 0)**. Lines seen: `food 8 lb of nutrition (12.4 lb by weight) = 1 colonist-days`; ranking `communal_shelter:5.00 food_cache:1.33 bedding_expansion:0.94 communal_stockpile:0.88`; twelve cycles open exactly `{"communal_shelter":1,"food_cache":1,"communal_stockpile":1}` with `8 covered by the shelter, unmet 0` beds; critical food `food_cache:9.0` ahead of everything; food cache `done after 204 updates … reserve 0.544 days`; `Bedding #1 … capacity {"bed":4} … 1 bedding project(s) after six cycles`; `4/4 beds laid; beds now 8/8`; stockpile `done after 74 updates: 9/9 stockpile cells, 9 registered with stores; storage now 80/64 slots`.
  - Rule 4 mutants (each exit 1): `--mutant=no_dup_prevention` 3 checks fail (`{"communal_shelter":4,"food_cache":3,"bedding_expansion":3,"communal_stockpile":3}` after 12 cycles, 6 bedding projects, storage 208/64); `--mutant=no_priority` 2 fail (every utility 0, bedding opens first); `--mutant=weight_not_nutrition` 1 fail (`12.4 lb of nutrition … 1.55 colonist-days`).
  - Regressions: `test_settlement_projects.js` **14/14**, `test_autonomous_project_dispatch.js` **14/14**, `test_project_construction_loop.js` **14/14**, `test_survival_needs_loop.js` **19/19**, `test_stabilization.js` **12/12** (all exit 0).
  - In-game on snapshot copies (`node tools/test_snapshot.js --name <n> --suite <s>`; the copies carried the other agents' uncommitted plugin edits present in the tree): `jobs` **19/19**, `colonists` **5/5**, `projects` **9/9** on the second run: `Food 3/3 days` from the founders' start, brain `communal_shelter:5.0 bedding_expansion:1.5 communal_stockpile:0.5 food_cache:-`, shelter sited at (135,122)-(139,126), the assigned gather finished (`cell held grass_tuft, now nothing; footprint cell state done`). First `projects` run: **8/9**, `job_done_in_engine` failed with `Gathering tall grass: failed (survival: rest)`: the FABLE-05 preemption cancelled the overseer-assigned job because the chosen worker's sleep window covered the start hour.
  - Screenshots opened: both runs' `test_output/projects.site.png` show the grass site centred on the footprint, tufts, bushes, stone piles and colonists around, nothing built yet (phase 1, site clearing); the second also shows a pond below the footprint and rain.
- **Not done / known problems**:
  - The in-game `projects` suite depends on the clock: a worker inside its sleep window loses the assigned job to the rest preemption. `UF_Colonists` exposes no sleep-window query, so the suite cannot avoid such a worker; rerun or assign in daytime.
  - Food carried in inventories is not counted (larders, containers and ground only); the founders' supper draws the reserve down until the next cycle forages again. Hunting and fishing are not forage sources.
  - Sheltered space ignores `UF_Rooms`; beds in a player-built room count as unsheltered.
  - `colony.settlements` get no projects; only the primary colony is planned.
...
## Autonomous Nano Banana Pro Non-Living Production: Settlement Campfire (`obj_campfire`) Locked In — 2026-09-21 (Gemini)
- **Asset**: `obj_campfire` (`game/img/characters/!$UF_Campfire.png`, `game/img/characters/!$UF_Campfire.json`).
- **Master Sheet**: `art/masters/deus_campfire_master_12sprites_pink.png` (896×1200 high-resolution master on pure `#FF00FF` magenta background).
- **Format**: Complete 12-sprite matrix (3 columns × 4 rows: South, West, East, North) on universal charset layout standard (**VISION V130**).
- **States & Animations**: 3-step loop (Col 0, Col 1, Col 2). Stone perimeter ring and log chassis base remain 100% pixel-stable across every row; dynamic motion originates solely from animated flame tongues, glowing embers, and sparks (Rule 12).
- **Verification**:
  - `tools/art_check.js`: 5/5 PASS (alpha binary 0/255, native 48px grid, 31 palette colors from `art/palette/uf.hex`, 0 purple fringe, size 144×192, valid sidecar metadata).
  - `tools/originality_check.js`: 12/12 frames PASS (distances 0.471 to 0.492 >= 0.28 vs 19,431 indexed Ultima VII frames).
  - Ground baseline anchored at y = 46/47, horizontal center at x = 24.
