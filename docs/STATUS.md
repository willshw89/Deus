...
- Claude Code | **World build integration** (docs/design/WORLD_ARCHITECTURE.md): cross-suite fixes, the beat and per-unit turns, one-command full run, review. The stock-art swap is withdrawn (V9 revised 21:30: U7 reference squares are the stand-ins again). **Files: `UF_TimeSpeed.js`, `UF_World.js`, `UF_Jobs.js`, `UF_WorldGen.js` (suite), `UF_Wildlife.js`, `UF_Fog.js`, `UF_Interact.js`, `UF_Look.js`, `tools/run_all_suites.js`, `docs/systems/`.** Not touching `UF_Objects.js`, `UF_Colonists.js`, `UF_ColonyOverseer.js`, `UF_Visuals.js`, `UF_Stance.js` while Gemini works in them on the user's instruction (walls, rooms and beds) | since 2026-09-18 18:00, restarted 21:35
- Claude Code | **Asset briefs for every asset, in segments, in Gemini's brief schema, for the U7 2.5D 48×48 square style** | `docs/asset_briefs/` (new), `tools/check_briefs.js`, pointers in `docs/ASSET_REQUESTS.md` and `GEMINI.md` | since 2026-09-18 21:35 (the 16×16 FF6-style run of 21:15 was stopped when the style changed)
