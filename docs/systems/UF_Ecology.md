...
**Registration:** Registered in `game/js/plugins.js` right after `UF_Wildlife`, before `UF_Stance`. Catalog configuration and `sapling` object applied to `game/data/UF_WorldCatalog.json` via `tools/add_ecology_catalog.js`. Multi-layer support active for ground and subterranean cavern levels.
