# UF_ColonyOverseer
The overseer's view of the colony: a free camera (WASD / arrows), left-click to select a colonist and open its card, left-click on the ground to order the selected colonist there, right-click to deselect; the card reads `UF.Colonists.describe` (name, gender, mood, faction · site, job, hunger/thirst/sleep/social, tool and clothes, latest thought, plan progress). Rewritten 2026-09-18: the old `Colonist` class, needs ticker, glade setup, `colony` suite and the `Window_UFLookLabel` block are gone (UF_Colonists and UF_Look own them). Status: built 2026-09-18, checks: `overseer` (6 checks, on request; all PASS on a snapshot 2026-09-18).
...
- `UF.Overseer`: `card()` → the card window, `colonistAt(mapX, mapY)` → the adapter under a map cell or null, `CARD_W`, `CARD_H`.
