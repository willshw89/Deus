...
| `cellInfo(gx, gy)` → `{ biomeId, biome, ground, water, walkable, region: { savagery, alignment }, fields, lake, peak }` | Cheap, no area build. `ground` = ground kind id (after mountain snow/peak and cursed/blessed swaps), `water` = key in `catalog.water.surface` or `null`, `walkable` = land, not peak, not water, and (DEUS-TSK-FABLE-19B) not an open ground cell: when the ground has its column, a ground cell whose shape is `open` (a natural cut down to -1 or -2, or a dig) has nothing to stand on and isn't walkable land. So faction homes and history camps, placed with it, keep off the cuts. Hills (solid ground cells) are unchanged: see Known limits |
...
| `groundTilesAt(ax, ay, x, y)` → `{ layer0, layer2, region, solid, open, carved, ramp }` or `null` | The ground tiles of one cell as a build paints them now (UF_Levels' shapes with their saved changes). UF_Levels repaints a ground cell whose shape changed, and its 8 neighbours, with it (2026-09-24) |
| `volumeStats["ax,ay"]` → `{ columns, solid, open, carved, ramps, groundReplaced, waterSuppressed, sitePiecesSkipped }` | The column counts of the last ground build of each area: `columns` false = painted without the column (no UF_Levels, a save whose ground is generator < 4); `open` = open ground cells (natural cuts, 19B) painted as the rock face; `carved` = non-solid cells with S >= 1 (cave mouths, and since 19B cut floors inside hills); `groundReplaced`/`waterSuppressed` = natural ground/water cells that became rock face, rock floor or dry ramp |
...
`cellInfo(gx, gy, z = 0)`, `cellInfoLocal(ax, ay, x, y, z = 0)`, `biomeAt(gx, gy, z = 0)` and `isWaterAt(gx, gy, z = 0)` accept a strict integer level of the world's Z range (`UF.World.isLevel`; WG.00.17, `docs/systems/DEUS_ZRange.md`: -16..+15 for new worlds, -2..+2 before). Ground retains the existing classifier. Other levels read `UF.Levels.cellAt/biomeAt/standableShape`, including the pocket freshwater cells; they never infer underground terrain from the surface climate. Invalid levels return null/false. `waterModel()` remains the legacy surface model; level-aware callers must use the cell APIs.

`kitCentres(ax, ay, z = 0)` (a level of the Z range, else `[]`) returns every bare settlement on the requested map, including two separate settlements of the same dwarven faction. Each centre carries `z` and its own `camp` ID. A missing level defaults to Ground for old saves; a dwarf's underground home is not a Ground kit centre.
...
**The column invariant (2026-09-24):** Node harness `tools/test_volumetric_terrain_column.js` (real World, WorldGen, Tiles, Objects, Levels in a vm; boots the tilesets and `Scene_Boot.start` hooks, so UF_Tiles' ground shades are on the built map as in the game). Per seed (20260923, 20260924): column profiles of every cell of the start area against S; every solid ground cell is `peak_rock` on layers 0 and 2 with region 250, tile-impassable and not walkable, and holds no object; ramps and carved cave cells walkable; `volumeStats` against the grid; the cliff classification, the frame spec and frame, ground connectors, the real sprite layer on a ground view; a path and a unit across a hill never on a solid cell; a unit sent into a hill never gets in; a dug tunnel repaints rock floor, matches a fresh build and fills back to the exact rock face; a synthetic history site's piece on a solid cell is skipped and counted; generator 5 puts no ramp under a cap and keeps a generator-4 valley as it was; no object of the real history stands on a solid cell. Results and mutants: `docs/systems/UF_Levels.md` → "The ground's column and its cliffs". Run on 2026-09-24 after DEUS-TSK-FABLE-17: `RESULT: 40 passed, 0 failed (exit 0)` (20 checks per seed).
...
- **Natural cuts (DEUS-TSK-FABLE-19B, 2026-09-25):** `surfaceElevationAt` and `surfaceGrid` stay the *natural* surface height S (the cut's floor is in the strata: `UF.Levels.worldStrataElevationAt`); the painter and the placement read the shapes, which follow the cuts. An open ground cell is drawn as the rock face until 19C. `cellInfo(z = 0).walkable` asks the ground's shape (an open cell isn't walkable); in a multi-area world that builds the other area's baselines when a cell there is asked about. The underground pass (`uf_underground_resources`) treats a sky-open cut floor at -1/-2 as any dry underground floor (cave flora and minerals can stand there). Checks: `tools/test_strata_cuts_and_caves.js` (`ground_holes`), docs/systems/UF_Levels.md, section "Natural cuts and caves".
