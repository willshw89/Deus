...
**Date:** 2026-09-25  
**Writer:** Grok (Lane E). This pass folds N5 and N6 from the Section 6 diff review.  
**Reviewer:** Claude CLI session. Section 6 diff-reviewed `5964f772` (CHANGES REQUESTED, two minors, `tasks/DEUS-TSK-FABLE-19C/claude_review.md`). This pass answers Section 6. The verdict on `5964f772` stays CHANGES REQUESTED until a later diff review.  
**Branch:** `task/lane-e`  
...
This revision folds N5 and N6 from the diff review of `5964f772` (`tasks/DEUS-TSK-FABLE-19C/claude_review.md` Section 6). That review's verdict was CHANGES REQUESTED. This pass is the text answer to those two minors. The verdict on `5964f772` stays CHANGES REQUESTED until a later diff review. N1–N4 stay as the previous revision resolved them, except where N5 replaces the `C-LIVE-ALLOC` window floor and the §8.5 flush comparison. Owner ruling CR-19C-R1 is option D, recorded as DEC-006 in `docs/OWNER_DECISIONS.md` (commit `eefd1f2c`). R2–R11 stay as the earlier revision resolved them. CR-19C-B1 and M2–M8 stay as resolved. The decisions are in the sections named there and are logged in section 9. `DEUS_Levels.js` and `DEUS_WorldGen.js` stay read-only. The compositor in `DEUS_Depth` owns the live-layer cutout and reads strata through public bytes.
...
A still camera still repaints. Stock `Tilemap.update` advances `animationFrame` every 30 frames (`rmmz_core.js:2327-2329`), and stock `updateTransform` rebuilds every spot when that frame changes (`:2376-2386`) and then sorts the children (`:2388`). The wrapper therefore runs 285 times (19×15) every 30 frames with the camera still, and again on every live start-tile change. The sort is `this.children.sort(this._compareChildOrder.bind(this))` (`:2650-2652`). The review measured that bind and sort at 31 bytes per frame (Section 6.5.1). It is outside the depth budget, as stock `addRect` is. `C-LIVE-ALLOC` runs the same rebuilds through the stub frame driver (section 8.1). Inside a judged window that driver does not call stock `update` or stock `updateTransform`. Stock `Tilemap.Layer.addRect` (`rmmz_core.js:2926-2928`) pushes a new 7-element array per rect. That allocation is outside the depth budget and is not on the stub path. The nw.js pan benchmark reports garbage collections during a live rebuild and does not fail on them (section 8.5). `C-LIVE-ALLOC` is the wrapper's proof. `C-REPAINT-ALLOC` is the proof that a plane repaint allocates nothing.
...
Tiles bake ownership into the canvas paint. For plane `d`, when that plane's repaint bit is set (section 6.4), `clearRect` the existing canvas, then `drawImage` only cells with `dHit = d`. The current plugin's "open cells are transparent" is not that rule: on `z ≤ 0` an `open` cell is an opaque tile and would hide a deeper plane. Cells the plane does not own are left clear. The roof substitute of section 3.5 covers an owned cell whose map tile is transparent.

Entities are not masked. Section 7 assigns each one to a plane, crops with `setFrame`, and re-expresses the anchor so the authored anchor pixel stays on the foot point.
...
Each plane carries a repaint bit, a boolean field on the plane object created with the scene. The depth update sets it when a summary rebuild changes the cells that plane owns, and when A1 water on that bound plane advances a frame. A check may set it to force a repaint. The painter runs only when the bit is set: `clearRect` on the existing canvas, then `drawImage` of the cells that plane owns, then the bit is cleared. A clear bit means that plane makes no canvas call. On the still frame above, every bit is clear. `C-REPAINT-ALLOC` and the §8.5 "one dirty plane" row set one bit and leave the other three clear. That is the force both of them use.

The only legal per-frame writes for a visible plane on that still frame are `x`, `y`, and `scale` on objects that already exist, plus `x` and `y` on entity sprites that already exist. The repaint bits stay clear and are not written. `exposed = 0` and `voids = 0` skips even the coordinate writes. A frame that paints may set and clear the one bit, and it may `clearRect` and `drawImage` the existing canvas. `C-REPAINT-ALLOC` is the proof that those calls allocate nothing.
...
`UF.Depth.stats()` keeps `frameAllocs`, `summaryRebuilds`, `canvasUploads`, `peeks`, `filterConstructs`, `maskConstructs`, `offPaletteTexels`. There is no mask upload counter: there is no mask. `frameAllocs` resets at the start of the depth update. The counter is not the proof of zero allocation. It only sees paths the implementer instrumented. The proof that the wrapper allocates nothing is `C-LIVE-ALLOC` (section 8.2). The proof that a plane repaint allocates nothing is `C-REPAINT-ALLOC`. Both are measured the way `tools/test_strata_foundation.js` `no_allocation_queries` measures (`:768-787`): `--expose-gc`, three windows, each after `global.gc()`, a `PerformanceObserver` on `gc`, zero collections inside the windows, and the least window judged. Each window is at least 1,000,000 frames, and long enough that a budget of 1 byte per frame is also at least 100 times the empty-window growth measured in the same run. The stub frame driver does not call stock `update` or `updateTransform` inside those windows, so the stock sort is not on the path. The stub's live tilemap uses recording layers, so stock `addRect` is not on the path. Both of those stock allocations are outside the depth budget. The mutant `alloc_unwired` leaves `frameAllocs` at 0 while the update allocates, and `C-LIVE-ALLOC` still fails. The mutant `alloc_in_repaint` allocates inside the painter, and `C-REPAINT-ALLOC` fails.
...
No sprite mask and no masked container. Tall and wide frames are cropped with `setFrame` against the map cells they overlap. The foot rule above decides whether the sprite exists. The crop decides which texels of that sprite are drawn.

**Placement.** The foot point is the centre of the foot cell in x and the bottom edge of the foot cell in y (`DEUS_Depth.js:609`). Map cell `(cx, cy)` covers plane-local pixels `x ∈ [cx·48, (cx+1)·48)` and `y ∈ [cy·48, (cy+1)·48)`, y downward, before `plane.scale`. The foot cell is the cell whose bottom edge is the foot point. The pixel just above that point belongs to the foot cell. The foot point itself is the shared edge with the cell to the south.

The sidecar anchor is a pixel pair. The plugin stores it as a fraction of the uncropped frame (`DEUS_Depth.js:374`, `:386`), and the default with no anchor is `(0.5, 1)`. Place the uncropped frame so that authored pixel `(ax, ay)` sits on the foot point. The frame's top-left is `(footX − ax, footY − ay)`.

A map cell is a candidate when its intersection with that frame has positive area. A 1 px sliver counts. The review's sidecar survey (Section 6.5.2) is why the crop is not a 48 px grid of the authored frame. `game/img/characters/!$UF_Birch.json` is 96×96 with anchor `[48, 95]`: the frame extends 24 px into the column on either side of the foot, 47 px into the row north of the foot, and 1 px into the row south of the foot. None of its 48 px frame cells lines up with a map cell, so none of them is the foot cell. That survey counts thirty-six sheets at 96 px wide with anchor x 48, and four at 192 px wide with anchor x 96. A 192 px frame with anchor x 96 is also 24 px out of register with the map, and it overlaps more than the two neighbouring columns. The placement rule is the same. A 144 px frame with anchor x 72 does line up with map columns. The one-sided and two-sided frames below are that shape.

**The rectangle.** Candidates are the overlapped cells whose `dHit` equals the plane's `d`. The crop is the largest axis-aligned rectangle of those cells that includes the foot cell. Area is the pixel area of that rectangle's intersection with the frame, not the area of the full cells. A rectangle cannot express L-shaped ownership. Any overlapped cell outside the chosen rectangle is dropped, including an L-shaped remainder. The crop is the rule that wins. A GPU mask is not a second opinion.

`setFrame(cropX, cropY, cropW, cropH)` is that intersection, in uncropped-frame pixels, origin at the frame's top-left. RMMZ `Sprite._refresh` (`rmmz_core.js:2132-2158`) makes the texture frame that crop, and PIXI then offsets the sprite by `-anchor · cropSize` (`game/js/libs/pixi.js:30853`). The anchor is a fraction of the cropped frame.

**Anchor after the crop.** Re-express it so the authored anchor pixel stays on the foot point:

```
anchor.x = (ax − cropX) / cropW
anchor.y = (ay − cropY) / cropH
```

`ax` and `ay` are the sidecar pixels. The same formula with the fractional anchor is `(axFraction · frameW − cropX) / cropW`, and the same for y. Leaving the uncropped fraction in place moves the kept texels. On the one-sided 144×48 frame below, an anchor left at 0.5 puts authored x 48 on the foot centre and shifts the crop 24 px east, into the column the crop was meant to drop. The anchor after the crop is 72 / 96 = 0.75.

When two or more rectangles share that maximum intersection area, break the tie on the intersected rectangle, in this order:

1. Greater height.
2. Greater width.
3. The rectangle whose bottom edge lies on the foot row, ahead of one that extends below the foot.
4. The rectangle whose left edge is closer to the foot column. If those distances are equal, the one that starts further west.

Worked frames. `entity_clip` expects the `setFrame`, the anchor, and the drawn rectangle (section 8.2). Drawn edges are plane-local pixels relative to the foot point, before `plane.scale`.

- Wall. A 48×96 frame, anchor pixel `(24, 96)`, so the fraction is `(0.5, 1)`. The frame covers the foot cell and the cell to the north. The north cell's `dHit` is not `d`. The only rectangle is the foot cell: `setFrame(0, 48, 48, 48)`, anchor stays `(0.5, 1)`. The drawn rectangle is x `[−24, +24)`, y `[−48, 0)`. If both cells match, the full frame is the rectangle.
- One-sided. A 144×48 frame, anchor pixel `(72, 48)`, fraction `(0.5, 1)`. It covers the west cell, the foot cell, and the east cell. Only the east cell has a different `dHit`. The rectangle keeps the west cell and the foot: `setFrame(0, 0, 96, 48)`, anchor `(0.75, 1)`. Drawn: x `[−72, +24)`, y `[−48, 0)`. No pixel of the east cell.
- Two-sided. The same 144×48 frame, east and west both different. The only rectangle is the foot cell: `setFrame(48, 0, 48, 48)`, anchor `(0.5, 1)`. Drawn: x `[−24, +24)`, y `[−48, 0)`.
- Tie. A 96×96 frame whose uncropped anchor is the bottom centre of its south-west cell, pixel `(24, 96)`, fraction `(0.25, 1)`. No sheet in `game/img/characters` uses that anchor. The case pins the tie. The north-west cell matches `d`, the south-east cell matches `d`, and the north-east cell does not. The west column and the foot row are both 4,608 px² of intersection. Greater height wins: the west column, `setFrame(0, 0, 48, 96)`, anchor `(0.5, 1)`. Drawn: x `[−24, +24)`, y `[−96, 0)`. An anchor left at 0.25 draws x `[−12, +36)`, 12 px east of that column.
- Centred 96. `!$UF_Birch.json`, frame 96×96, anchor pixel `(48, 95)`, fraction `(0.5, 95/96)`. The east column (north, foot row, and the 1 px south sliver) has a different `dHit`. The other six overlapped cells match. Their rectangle intersects the frame in 72×96 px: `setFrame(0, 0, 72, 96)`, anchor `(2/3, 95/96)`. Drawn: x `[−48, +24)`, y `[−95, +1)`. Drawn x covers 24 px of the west column and all 48 px of the foot column. Drawn y covers 47 px of the north row, all 48 px of the foot row, and 1 px of the south row. A partition of this frame into 48 px cells has no cell whose centre is the foot point, and it fails this case.

The products `anchor.x · cropW` and `anchor.y · cropH` equal `ax − cropX` and `ay − cropY`. The centred 96 frame is the one where `cropW` does not divide the pixel evenly: `48 / 72 = 2/3`. The check compares those products within `1e-6` of a pixel, and compares drawn edges the same way.
...
Two-grid walls, doors, and cliff faces on a lower plane use the same crop. A 48 px wide frame anchored at its bottom centre keeps the authored upper 48 px when the north cell's `dHit` matches, because that upper half is the north cell. A frame that straddles columns uses the rectangle above rather than assuming the upper half is one cell. Those cap texels are already on the void ramp or are non-master and pass through (section 5). The lower 48 px of an aligned wall is the material face. Under R1 = D it is the source texel. After palette migration it is shaded along that material's ramp when the texel is a master hex. The renderer does not draw a second cap in code and does not tint the cap with a shader.
...
| Node vm, PIXI stub, `node --expose-gc`. The stub records `scale`, `mask`, `filters`, `setFrame`, `anchor`, the drawn rectangle, `visible`, construct counts, and canvas draw calls. Its `x` / `y` / `scale` setters allocate nothing. The live tilemap's spot chain stays stock (the list under this table). Its two layers are recording layers: `addRect` writes into a typed array allocated when the layer is constructed, and `clear` resets a count (`C-LIVE-ALLOC`). Its `_sortChildren` is a no-op. Judged windows use the stub frame driver, defined under this table. | Binding, `O-SHAFT-DRAW`, `O-Z0`, filter and mask constructs, entity visibility and crop, `C-LIVE-ALLOC`, `C-REPAINT-ALLOC`, `C-DIG-REPAINT`, `S-RELATIVE`, `S-SCALE-INT`, `S-PAD`, `C-STILL`, `C-HP`, `C-ALLOC`, `C-ONE`. | The composited framebuffer. |
...
**Stub frame driver.** `C-LIVE-ALLOC`, `C-REPAINT-ALLOC`, and `C-DIG-REPAINT` use this driver. Inside a judged window it does not call stock `Tilemap.prototype.update` (`rmmz_core.js:2327-2335`) or stock `Tilemap.prototype.updateTransform` (`:2367-2391`), and it does not call `PIXI.Container.prototype.updateTransform`. Stock `updateTransform` always calls `_sortChildren` (`:2388`, `:2650-2652`), which allocates a bound comparator. The review measured that path at 31 bytes per frame with recording layers as the only children (Section 6.5.1). The stock sort is outside the depth budget, the same way stock `addRect` is. The stub's `_sortChildren` is a no-op, so a stray call inside the window allocates nothing.

Each frame the driver does four things, and its own body allocates nothing:

1. It advances `animationCount` and `animationFrame` with the two assignments at `:2328-2329`. It does not run the child `update` loop.
2. It repeats the repaint test at `:2376-2380`: `_needsRepaint`, or `animationFrame` changed, or the start tile changed.
3. On a hit it calls stock `_addAllSpots` (`:2422`) and then clears `_needsRepaint` (`:2386`).
4. It does not sort.

The stock spot chain stays stock, from `_addAllSpots` through `_readMapData` (`:2422-2648`): `_addAllSpots` (`:2422`), `_addSpot` (`:2436`), `_addSpotTile` (`:2465`), `_addTile` (`:2473`), `_addNormalTile` (`:2483`), `_addAutotile` (`:2500`), `_addTableEdge` (`:2579`), `_addShadow` (`:2604`), `_readMapData` (`:2618`), and the helpers those call, `_isHigherTile` (`:2638`), `_isTableTile` (`:2642`), and `_isOverpassPosition` (`:2646`). The depth plugin wraps `_addSpot` only. Calls below the layers land on the recording layers.
...
| `C-DIG-REPAINT` | Two separate runs. The production order each run stands in for is `Scene_Map.update` then render (`rmmz_scenes.js:819-824`, `Graphics._onTick` at `rmmz_core.js:808-815`). Run A opens a `+1` floor into a hole (`dHit` moves from 0 to at least 1). Run B builds a floor over a hole (`dHit` moves from at least 1 to 0). Frame N of a run: depth update, then the dig on an in-window column, then the stub frame driver. Frame N+1: depth update, then the stub frame driver. The dig emits `cellChanged`. The camera is still. `animationFrame` is the same on frame N and frame N+1. A change exits 2: an animation tick rebuilds spots even when the depth update never sets the flag. | On frame N+1 the wrapper's recorded decision for that cell matches the new `dHit`. Run A skips the spot (section 3.5 case 1). Run B draws the live tile (case 3). Frame N may still show the pre-dig decision, because the dig lands after that frame's summary rebuild. A build that sets `_needsRepaint` only in the `cellChanged` handler, and not in the depth update after the summary rebuild, still shows the old decision on frame N+1 and fails. |

**Allocation (stub, `--expose-gc`).**

| Id | Build | Expect |
|---|---|---|
| `C-LIVE-ALLOC` | Warm the canvases. Five-level opening, `exposed > 0`, plane 4's presence bit set. Camera still. The stub's live tilemap has two recording layers. `addRect` writes seven integers into a typed array allocated when the layer is constructed, sized for every rect a 19×15 rebuild can emit. `addRect` past that capacity exits 2. It does not grow the array. `clear` resets a count. Neither `addRect` nor `clear` allocates. The stock spot chain in section 8.1 stays stock, including `_addSpotTile` (`rmmz_core.js:2465`), `_addNormalTile` (`:2483`), and `_addTableEdge` (`:2579`). Each judged frame is the depth update, then the stub frame driver. The driver advances `animationFrame` every 30 frames, so the wrapper runs. Stock `update` and stock `updateTransform` do not run inside a judged window. The stub's `_sortChildren` is a no-op. Measure the way `no_allocation_queries` does (`tools/test_strata_foundation.js:768-787`). First, three empty windows in the same run: `global.gc()`, then heap statistics, no depth work and no rebuild, then heap statistics again. `emptyLeast` is the least of those three growths. The review measured a no-loop empty window at 616 bytes and, in another run, at 18,120 bytes (Section 6.5.1). `framesPerWindow` is the smallest multiple of 60 that is at least `max(1000000, 100 * max(emptyLeast, 0))`. The multiple of 60 keeps each half of the window a whole number of 30-frame animation periods. The 100× term stays on top of the 1,000,000 frame floor. At the reviewed 616 byte empty floor the window is 1,000,020 frames. Three judged windows follow. Each starts with `global.gc()`, then runs `framesPerWindow` frames (the first half with units still, the second half with one unit walking on a lower plane inside the window), then reads `v8.getHeapStatistics().used_heap_size`. A `PerformanceObserver` on `gc` counts collections whose `startTime` falls inside a window. Judge the least growth. | Zero garbage collections inside the judged windows. The least growth is under 1 byte per frame in that window. A budget of 1 byte per frame on 1,000,020 frames is 1,000,020 bytes, 5.66 times the 176,624 byte growth the review measured on a frame loop that allocates nothing (Section 6.5.1). That growth did not increase when the same loop was lengthened, which is why a 61,620 frame window could finish above its budget. `frameAllocs` is 0. A counter that stays 0 while a `{}` runs still fails the heap side. Stock `Tilemap.Layer.addRect` (`:2926-2928`) pushes a new 7-element array per rect. The review measured 311,544 bytes over eight rebuilds of one rect per spot, about 1,298 bytes per frame across 240 frames, which fails this budget. That allocation is outside the depth budget and is not on this path. A stub that installs stock layers fails. `C-ALLOC` is not this check. |
| `C-REPAINT-ALLOC` | The same opening, recording layers, driver, `framesPerWindow`, and three-window judgement as `C-LIVE-ALLOC`. Each frame forces one plane to repaint. The harness sets that plane's repaint bit and leaves the other three clear, the same force as the §8.5 "one dirty plane" row. The painter runs `clearRect` and `drawImage` on that plane only. | Zero garbage collections inside the judged windows. The least growth is under 1 byte per frame. The other three planes record no canvas call. `frameAllocs` is 0. A mutant that allocates an object inside the painter fails. |
...
| `entity_clip` | Stub. Five frames. The foot column's `dHit` equals the plane's `d` in each. Anchors are the uncropped sidecar fractions from section 7. (1) A 48×96 wall, anchor `(0.5, 1)`, north cell a different `dHit`. (2) A 144×48 sprite, anchor `(0.5, 1)`, only the east cell different. (3) The same 144×48 sprite, east and west both different. (4) A 96×96 sprite, uncropped anchor `(0.25, 1)`, foot at the south-west cell, north-west matching, south-east matching, north-east different. (5) `!$UF_Birch.json`, 96×96, anchor pixel `(48, 95)`, east column a different `dHit`, the other overlapped cells matching. | Section 7. The check reads `setFrame`, the anchor, and the drawn rectangle in plane-local pixels, before `plane.scale`. (1) `setFrame(0, 48, 48, 48)`, anchor `(0.5, 1)`, drawn x `[−24, +24)`, y `[−48, 0)`. (2) `setFrame(0, 0, 96, 48)`, anchor `(0.75, 1)`, drawn x `[−72, +24)`, y `[−48, 0)`. (3) `setFrame(48, 0, 48, 48)`, anchor `(0.5, 1)`, drawn x `[−24, +24)`, y `[−48, 0)`. (4) `setFrame(0, 0, 48, 96)`, anchor `(0.5, 1)`, drawn x `[−24, +24)`, y `[−96, 0)`. (5) `setFrame(0, 0, 72, 96)`, anchor `(2/3, 95/96)`, drawn x `[−48, +24)`, y `[−95, +1)`. Edges and `anchor · cropSize` match section 7 within `1e-6` of a pixel. No `.mask`. An L-shaped remainder is dropped. A crop that always keeps only the foot cell fails (2) and (4). A crop that leaves the uncropped anchor fraction in place fails (2): drawn x is `[−48, +48)`, and 24 px of the east cell remains. |
...
| A centred 96 px sheet is cropped on 48 px cells of the authored frame | The foot point is not the centre of any of those cells | `entity_clip` |
| Case (2) keeps `setFrame(0, 0, 96, 48)` and leaves the anchor at 0.5 | The kept texels move 24 px east | `entity_clip` |
...
| One plane's forced repaint allocates an object | The painter allocates | `C-REPAINT-ALLOC` |
...
| `alloc_in_repaint` | The plane painter allocates an object when its repaint bit is set | `C-REPAINT-ALLOC` |
| `anchor_fraction_kept` | After `setFrame`, the anchor stays the uncropped sidecar fraction | `entity_clip` |
...
`oracle_uses_shapes`, `no_physics_write`, `standing_frame`, `S-SHADE-VIS`, `S-SHADE-ORACLE`, `S-MONO`, and `S-PAL` are checks with the builds in section 8.2. A patch that trips one of them is a failed run under the same exit rule. The named mutants above are the minimum set: dropping one from the driver fails the gate.
...
| One dirty plane, forced window repaint | That plane under 16 ms. The other planes do not repaint on that frame. The force is that plane's repaint bit (section 6.4). Heap is sampled the way `no_allocation_queries` samples (`global.gc()`, then `used_heap_size`): three empty windows, and three windows around that plane's paint and flush. Net growth is the least flush window minus the least empty window. The row reports the net and does not fail on it. The review's Node probe saw identical empty pairs differ by 192 bytes (Section 6.5.1), so a 1-byte threshold fails a flush that allocates nothing. A garbage collection inside a flush window fails. `C-REPAINT-ALLOC` is the allocation proof for this repaint. |
| A1 water in one lower window | That plane may repaint every 30 frames. The others do not. |
| Pan benchmark | Canvases warmed first. Camera steps one cell every 8 frames, four presence bits set, 120 frames. Report garbage-collection count and repaint ms. A depth-canvas flush is sampled as three flush windows against three empty windows in the same run (`global.gc()`, heap before, the flush or the empty gap, heap after). Net growth is the least flush window minus the least empty window. The row reports the net and does not fail on it. A garbage collection inside a flush window fails. Garbage collections during a live tilemap rebuild are reported and do not fail the benchmark. In nw.js the live layers are stock, so a collection can land while the wrapper is on the stack in a correct build (about 19 rebuilds in 120 frames: 15 start-tile changes and 4 animation ticks). `C-LIVE-ALLOC` is the wrapper's proof. |
...
3. Hot-path allocations: forbidden on the depth update, the plane repaint, and the `_addSpot` wrapper. Section 6.4. Sheet build is outside that path. Stock `Tilemap.Layer.addRect` allocates inside the engine's own layer. Stock `Tilemap._sortChildren` allocates a bound function every frame it runs. Both are outside the depth budget. The stub frame driver does not call stock `update` or `updateTransform`, and the stub's `_sortChildren` is a no-op (section 8.1). The stub proves the wrapper with recording layers (`C-LIVE-ALLOC`) and proves the repaint with `C-REPAINT-ALLOC`. The nw.js rows fail a depth-canvas flush only when a garbage collection happens inside it. They report the net of three flush windows against three empty windows and do not fail that net. They report live-rebuild collections and do not charge them to the wrapper.
