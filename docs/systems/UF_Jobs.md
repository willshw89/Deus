...
- Limits: `params.count` defaults to the whole stack so existing stockpile and larder hauls keep moving stacks; a planner that wants a fixed amount names it. Since DEUS-TSK-FABLE-18 (2026-09-24) container items obey the same rule: `pickUpNow` takes `legalLift(job, unit, it)` out of a container through `UF.Containers.takeItem(container, item, unit, count)`; a split makes the carried part the job's `params.itemId` (reserved for the carrier).

## Fire at the work site (2026-09-24, DEUS-TSK-FABLE-18)

- `fireAtWorkSite(job)` → true when `UF_Fire` burns a square on or beside the job's `target`, its `stand`, or a haul's `params.to` (a hearth or campfire in its place is not a fire here). `douse`, `douse_ally`, `extinguish` and any `params.reflex` job are exempt (they go to the fire on purpose). Free when nothing burns (`UF.Fire.count()` is 0).
- `take` does not take such a job: it stays open with `job.reason = "fire at the work site"` and is taken once the fire is out. `step` fails a taken one with `fire at the work site` before planning, so a worker drops a job the fire reached rather than walking on into the flames (the handler's `cancel` puts a carried load down, as for any failure).
- The colonists' douse of a burning friend is the `douse_ally` job (it was `douse`, which `UF_Fire`'s own square-dousing `douse` replaced at load).
- Checks: `node tools/test_fire_survival_behavior.js` (`job_at_fire_not_taken`, `job_taken_once_the_fire_is_out`, `taken_job_dropped_when_fire_reaches_it`; mutant `takes_jobs_at_fire`).

## First aid (2026-09-23, SRD stabilisation)

- **`stabilize` `{ unitId }`**: plan: the patient exists, is not dead, has `hp <= 0` and is not stable, is on the rescuer's level within 40 cells, and is not reserved by another rescuer (the plan reserves it: `ReservationManager` key `entity:<id>`); stand beside it. Work 360 ticks (a 6 s round). Apply: `UF.Colonists.stabilize(patient, unit)`; a failed check returns `"continue"` for another round; a dead or recovered patient fails the job cleanly (`too late`, `no longer dying`). `job.result` holds `{ ok, roll, total, dc }`. Describe: "Stabilizing <name>". Colonists owns the dying state and the check; this job only walks, waits and asks.
