...
## Dwellings from DEUS_Projects (DEUS-TSK-FABLE-16, 2026-09-24)

While `UF.Projects` is enabled and has the `household_cottage` blueprint (`projectsHouse()`), this planner plans no home of its own: `planSteps` returns `[]`, so `DEUS_Projects` is the one source of settlement housing (`docs/systems/DEUS_Projects.md`, "Hearths, homes and phases"). A finished cottage becomes the household's home through `assignHome(household, { buildingId, x, y, w, h, area, z, wall, door, walls, doors, beds, hearth, storage, entrance })`: it sets `homeBuildingId`, `isMovedIn` and a `home` record shaped like this plugin's own plots (walls, doors, beds with `unitId` per member in order, `sleeping`, `spots`, `hearth`, `storage`, `entrance`, `rooms` living + bedroom, `isShared: false`, `source: "projects"`) so `strictEnclosure`, `isSheltered`, `hasCommunalLiving`, `hasBedroom` and `canConceiveChild` read it; emits `households:homeAssigned(household, home)`. `releaseHome(household, reason?)` clears it (`households:homeReleased`). `isHoused(household, capacity?)` → the household has a private (not shared) dwelling of its own, enclosed, with a bed for every member (up to `capacity` when given). New records carry `homeBuildingId: null`. Note: this plugin is not registered in `game/js/plugins.js` (2026-09-24), so the live game groups households from the colonists' own partner data inside `DEUS_Projects`; when it is registered, its records take precedence there.
