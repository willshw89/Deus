...
## Assets used
- `img/characters/!$U7_Item_*.png` + `.json` sidecars, one per `items.types[].image` in the catalog (27 types on 23 sheets; the tools and clothes reuse `RoughStone` / `PlantFiber` / `LeatherHide` with a `tint`). All are U7 stand-ins (AR-200 originals pending). The sprite draws column 1, row 0 of the 3×4 sheet with the sidecar's `frameWidth/frameHeight/anchor` (all current sidecars: bottom-center). Also listed in `docs/ASSET_INVENTORY.md`.
