...
console.log('Rendering 8-facing troll lineup...');
const trollMasterPath = path.join(ROOT, 'art', 'masters', 'troll_idle.png');
const trollMaster = decodePNG(fs.readFileSync(trollMasterPath), 'troll.png');
...
for (let r = 0; r < 8; r++) {
    ...
    for (let y = 0; y < 96; y++) {
        for (let x = 0; x < 96; x++) {
            const so = ((srcY0 + y) * trollMaster.width + x) * 4;
            if (trollMaster.data[so + 3] > 0) {
                const doff = (y * lineupW + (dstX0 + x)) * 4;
                lineupBuf[doff]     = trollMaster.data[so];
                lineupBuf[doff + 1] = trollMaster.data[so + 1];
                lineupBuf[doff + 2] = trollMaster.data[so + 2];
...
const lineupOutPath = path.join(ROOT, 'art', 'review', 'troll_8way_stand_lineup_2x.png');
writePNG(lineupOutPath, lineupW * 2, lineupH * 2, lineup2x);
console.log(`Saved: ${lineupOutPath}`);

// 2. Scale comparison: Human Settler (48x48), Giant Spider (96x96), Troll (96x96, 88 px tall), Wood Wall, Oak Tree
console.log('Rendering scale comparison scene...');
// Canvas: 10 tiles wide (480 px) x 3 tiles tall (144 px)
const compW = 480;
const compH = 144;
...
// Blit Wall Wood (48x96 at x=105, y=24)
...
// Blit Human Male Settler (48x48 at x=160, y=72)
...
// Blit Giant Spider South (96x96 at x=220, y=24)
const spiderPath = path.join(ROOT, 'art', 'masters', 'giant_spider_idle.png');
if (fs.existsSync(spiderPath)) {
    const spider = decodePNG(fs.readFileSync(spiderPath), 'spider.png');
    for (let y = 0; y < 96; y++) {
        for (let x = 0; x < 96; x++) {
            const so = (y * spider.width + x) * 4;
            if (spider.data[so + 3] > 0) {
                const doff = ((24 + y) * compW + (220 + x)) * 4;
                compBuf[doff]     = spider.data[so];
                compBuf[doff + 1] = spider.data[so + 1];
                compBuf[doff + 2] = spider.data[so + 2];
                compBuf[doff + 3] = 255;
            }
        }
    }
...
// Blit Troll South (96x96 at x=340, y=24)
for (let y = 0; y < 96; y++) {
    for (let x = 0; x < 96; x++) {
        const so = (y * trollMaster.width + x) * 4; // Row 0 (South) col 0 (Stand)
        if (trollMaster.data[so + 3] > 0) {
            const doff = ((24 + y) * compW + (340 + x)) * 4;
            compBuf[doff]     = trollMaster.data[so];
            compBuf[doff + 1] = trollMaster.data[so + 1];
            compBuf[doff + 2] = trollMaster.data[so + 2];
            compBuf[doff + 3] = 255;
        }
    }
}
...
const compOutPath = path.join(ROOT, 'art', 'review', 'troll_scale_comparison_on_meadow_2x.png');
...
for (let col = 0; col < 3; col++) {
    ...
    for (let y = 0; y < 96; y++) {
        for (let x = 0; x < 96; x++) {
            const so = (y * trollMaster.width + (col * 96 + x)) * 4;
            if (trollMaster.data[so + 3] > 0) {
                const doff = (y * idleW + (dstX0 + x)) * 4;
                idleBuf[doff]     = trollMaster.data[so];
                idleBuf[doff + 1] = trollMaster.data[so + 1];
                idleBuf[doff + 2] = trollMaster.data[so + 2];
                idleBuf[doff + 3] = 255;
            }
        }
    }
...
const idleOutPath = path.join(ROOT, 'art', 'review', 'troll_idle_cycle_south_2x.png');
writePNG(idleOutPath, idleW * 2, idleH * 2, idle2x);
console.log(`Saved: ${idleOutPath}`);

console.log('All troll review renders successfully generated!');
