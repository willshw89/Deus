...
const MUTANTS = {
    ...
    communal_shelter_counts_as_home: [{ file: "projects", from: "return !!p && (p.bedCount | 0) >= Math.min(size, Math.max(1, cfg.cottageBeds | 0));", to: "return list(q => q.state === \"done\" && q.kind === \"communal_shelter\").length > 0 || (!!p && (p.bedCount | 0) >= Math.min(size, Math.max(1, cfg.cottageBeds | 0)));" }]
};
...
try {
    ...
    check("village_opens_cottage_for_unhoused_household", n2 > 0 && !!dD && dD.phase === "village" && dD.shelter.deficit === 0 && dD.storage.deficit === 0 && dD.food.current >= Math.min(dD.food.needed, cfg.villageFoodDays) && !!first && !!first.household && !!firstHh && !firstHh.housed && first.household.size === 2 && first.bedCount === 2 &&
        first.capacity.housing === 1 && first.capacity.bed === 2 && concurrent <= 1,
        `after ${n2 > 0 ? (n2 / DAY_TICKS).toFixed(2) : ">10"} more days: phase ${dD ? dD.phase : "?"} (shelter ${dD ? dD.shelter.current : "?"}/${dD ? dD.shelter.needed : "?"}, storage ${dD ? dD.storage.current : "?"}/${dD ? dD.storage.needed : "?"}, food ${dD ? dD.food.current : "?"} days); ${first ? `${P.describe(first)} for household ${first.household ? first.household.id : "none"} (size ${first.household ? first.household.size : "?"}, ${first.bedCount} beds, capacity ${JSON.stringify(first.capacity)})` : "no cottage opened"}; at most ${concurrent} cottage(s) active at once`);
    ...
    check("household_moves_into_finished_cottage", n3 > 0 && !!first && first.state === "done" && !!stC && stC.walls === stC.wallsWanted && stC.hearth === hearthType.id && stC.beds === 2 && stC.chest === cottageBp.chest && stC.clearanceFree === 4 &&
    ...
    const housingJson = JSON.stringify(W.state.colony.projects.housing || null), homesJson = JSON.stringify(H.all().map(h => [h.id, h.homeBuildingId])), bedsJson = JSON.stringify(colonists(S).map(u => [u.id, u.data.bed]));
    ...
    check("save_load_keeps_homes", clean && JSON.stringify(W.state.colony.projects.housing || null) === housingJson && JSON.stringify(H.all().map(h => [h.id, h.homeBuildingId])) === homesJson && JSON.stringify(colonists(S).map(u => [u.id, u.data.bed])) === bedsJson && !!dH && dH.housing.current === (dF ? dF.housing.current : -1),
