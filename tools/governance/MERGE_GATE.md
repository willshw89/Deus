...
3. **Tests run in a clone at the checked sha**, one fresh clone per test, with no shell, their own timeout, and real exit codes. A test cannot pass because of a file that exists only in someone's work tree. The clone is checked out so work-tree bytes equal the committed blobs, including when the caller's global config has `core.autocrlf=true` (§5 (c)).
...
3. **Clone.** For each test, `git clone -c core.autocrlf=false -c core.eol=lf -c core.safecrlf=false --shared --no-checkout <git common dir> %TEMP%\deus-merge-gate-XXXXXX\clone-<n>`. The same three keys are then written into the clone's local config, and `git checkout --detach <checked sha>` runs with the same three `-c` overrides plus `advice.detachedHead=false`. `core.autocrlf=false` stops a caller's global or system `core.autocrlf=true` from smudging LF blobs into CRLF. `core.eol=lf` does the same for a `text` or `text=auto` path whose `eol` attribute is unspecified, which would otherwise follow a caller's `core.eol=crlf` or the Windows native default. `core.safecrlf=false` stops a caller's `core.safecrlf` from refusing that checkout. A blob that already contains CRLF stays as stored when `text=auto` does not convert it. An explicit `eol=` attribute in the tree still wins: that is repository content, not caller config. The clone's `HEAD` must equal the checked sha (`CLONE_FAILED`). `--mutant=clone_autocrlf_off` drops all three settings. The case `pass_clone_lf_under_autocrlf_true` runs the gate with `GIT_CONFIG_GLOBAL` pointed at a temporary file (`core.autocrlf=true`, `core.eol=crlf`, `core.safecrlf=true`, never the real user config) whose gate test requires `src/feature.js` to equal its LF blob.
...
The other cases: seven more passing shapes (bold `CLEAN PASS`, FAIL review then fix then PASS review, `[gemini]` commit between writer and review, no designated reviewer, a `[gemini]` scope update, a run from the lane's own worktree, and `pass_clone_lf_under_autocrlf_true`, a byte-for-blob checkout while the gate's `GIT_CONFIG_GLOBAL` has `core.autocrlf=true`). Also: remaining review, test, manifest and main refusals, the usage and mutant guards, `fail_merge_conflict_is_aborted` (no `MERGE_HEAD` left, work tree clean), and `fail_nothing_to_merge`.
...
| `clone_autocrlf_off` | LF-faithful clone checkout (`core.autocrlf=false`, `core.eol=lf`, `core.safecrlf=false`) | `pass_clone_lf_under_autocrlf_true` |
...
OPS.10.04 (Lane AJ): every test clone is checked out LF-faithful (`core.autocrlf=false`, `core.eol=lf`, `core.safecrlf=false` on the clone, in the clone's local config before checkout, and on the checkout command). The suite count and the run under a temporary `core.autocrlf=true` global config are in `tasks/OPS.10.04/lane-aj/REPORT.md`.
