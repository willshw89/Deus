...
/**
 * tools/build_perfect_female_orc.js
 *
 * UNIFORM SCALE BUILDER for Adult Female Orc (Mountain Orc Warrior / Settler)
 ...
 * 3. Attack (12 sprites) - Melee jagged war axe / cleaver cleave strike with slash arc
 * 4. Bow    (12 sprites) - Heavy war bow aim, draw, and pluck recoil (ZERO flying arrows)
 * 5. Magic  (12 sprites) - Shamanic war chant with soft crimson ritual aura (ZERO flying beams)
 * 6. Work   (12 sprites) - Kneeling crafting/leatherworking on ground with tools (~30px height)
...
// Target female orc height: ~43-44px in RMMZ (muscular ~3.0 heads, grounded at y = 47)
let UNIFORM_SCALE = 43.0 / 248.0;
...
function processAction(rawFileName, actionTag, animations) {
    ...
    if (actionTag === 'Walk') {
        ...
        console.log(`Calibrating scale from female walk standing sprite raw height = ${standH}px`);
        UNIFORM_SCALE = 43.0 / standH;
        console.log(`Calibrated UNIFORM_SCALE = ${UNIFORM_SCALE.toFixed(6)} (Target: 43px in RMMZ)`);
    ...
    saveSheetAndSidecar(sheet, `Orc_Female_${actionTag}`, actionTag, animations);
    return sheet;
}

function buildAll() {
    console.log('=== Building Adult Female Orc 7-Action 12-Sprite Action Suite ===\n');

    // 1. Walk
    console.log('1. Walk...');
    const walkSheet = processAction('orc_female_walk_12_raw.png', 'Walk', { walk: [0, 1, 2, 1], stand: [1] });
    fs.writeFileSync(path.join(CHAR_DIR, '$UF_Orc_Female.png'), writePNG(walkSheet, 144, 192));
    console.log('Updated $UF_Orc_Female.png alias');

    // 2. Haul
    console.log('2. Haul...');
    const haulSheet = processAction('orc_female_haul_12_raw.png', 'Haul', { haul: [0, 1, 2, 1], carry: [1], stand: [1] });

    // 3. Attack (War Axe)
    console.log('3. Attack (War Axe)...');
    const attackSheet = processAction('orc_female_attack_12_raw.png', 'Attack', { attack: [0, 1, 2], stand: [0] });

    // 4. Bow (War Bow)
    console.log('4. Bow (War Bow)...');
    const bowSheet = processAction('orc_female_bow_12_raw.png', 'Bow', { bow: [0, 1, 2], stand: [0] });

    // 5. Magic (Ritual)
    console.log('5. Magic (Ritual)...');
    const magicSheet = processAction('orc_female_magic_12_raw.png', 'Magic', { cast: [0, 1, 2], stand: [0] });

    // 6. Work (Leatherworking)
    console.log('6. Work (Crafting)...');
    const workSheet = processAction('orc_female_work_12_raw.png', 'Work', { work: [0, 1, 2], stand: [0] });

    // 7. Downed
    console.log('7. Downed...');
    const downedSheet = processAction('orc_female_downed_12_raw.png', 'Downed', { hurt: [0], collapse: [1], dead: [2] });
    ...
    const reviewPath = path.join(REVIEW_DIR, 'orc_female_all_7_actions_12_sprites.png');
