...
/**
 * tools/compile_var2_suite.js
 *
 * Compiles the complete 7-action 12-sprite suite for Adult Male Human (Variation 2 - Golden Blonde Scout)
 * into game/img/characters/$UF_Human_Male_2_{Action}.png and .json sidecars.
 */
...
function compileVar2Suite() {
    console.log('=== Compiling Adult Male Human Variation 2 (Scout) 7-Action Suite ===');

    // 1. WALK (Already defined and verified)
    const imgWalk = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_walk.png')));
    ...
    saveSheetAndSidecar(walkSheet, 'Human_Male_2_Walk', 'Walk', { walk: [0, 1, 2, 1], stand: [1] });
    saveSheetAndSidecar(walkSheet, 'Human_Male_2', 'Walk', { walk: [0, 1, 2, 1], stand: [1] });

    // 2. HAUL
    const imgHaul = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_haul.png')));
    ...
    saveSheetAndSidecar(haulSheet, 'Human_Male_2_Haul', 'Haul', { haul: [0, 1, 2, 1], carry: [1], stand: [1] });

    // 3. ATTACK (Dual Daggers)
    const imgAtk = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_attack.png')));
    ...
    const aN = [0, 2, 1].map(c => extractFrameFromCell(imgAtk, c * 234, (c + 1) * 234, 511, 767, 43));
    const atkSheet = assemble12SpriteSheet({ S: aS, W: aE.map(mirrorFrame), E: aE, N: aN });
    saveSheetAndSidecar(atkSheet, 'Human_Male_2_Attack', 'Attack', { attack: [0, 1, 2], ready: [0] });

    // 4. BOW (Composite Yew Bow)
    const imgBow = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_bow.png')));
    ...
    const bN = [0, 2, 1].map(c => extractFrameFromCell(imgBow, c * 234, (c + 1) * 234, 511, 767, 43));
    const bowSheet = assemble12SpriteSheet({ S: bS, W: bE.map(mirrorFrame), E: bE, N: bN });
    saveSheetAndSidecar(bowSheet, 'Human_Male_2_Bow', 'Bow', { shoot: [0, 1, 2], aim: [1], ready: [0] });

    // 5. MAGIC (Spell Initiation)
    const imgMag = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_magic.png')));
    ...
    saveSheetAndSidecar(magSheet, 'Human_Male_2_Magic', 'Magic', { cast: [0, 1, 2, 1], chant: [0] });

    // 6. WORK (Forester Woodcarving)
    const imgWrk = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_work.png')));
    const wkS = [
        extractFrameFromCell(imgWrk, 0, 234, 0, 258, 43),    // Standing check
        extractFrameFromCell(imgWrk, 470, 704, 0, 258, 43),  // Kneeling craftsman (~28px)
        extractFrameFromCell(imgWrk, 704, 938, 0, 258, 43)   // Ground axe strike
    ];
    const wkE = [
        extractFrameFromCell(imgWrk, 0, 234, 258, 511, 43),
        extractFrameFromCell(imgWrk, 470, 704, 258, 511, 43),
    ...
    const wkN = [
        ...
        extractFrameFromCell(imgWrk, 470, 704, 511, 767, 43)
    ];
    const wrkSheet = assemble12SpriteSheet({ S: wkS, W: wkE.map(mirrorFrame), E: wkE, N: wkN });
    saveSheetAndSidecar(wrkSheet, 'Human_Male_2_Work', 'Work', { work: [0, 1, 2, 1], craft: [1] });

    // 7. DOWNED (Defeat Sequence)
    const imgDwn = decodePNG(fs.readFileSync(path.join(RAW_DIR, 'human_male_var2_pro_4d_downed.png')));
    const dS = [
        extractFrameFromCell(imgDwn, 0, 234, 0, 258, 43),     // Flinch
        extractFrameFromCell(imgDwn, 704, 938, 0, 258, 43),   // Kneeling collapse (~28px)
    ...
    saveSheetAndSidecar(dwnSheet, 'Human_Male_2_Downed', 'Downed', { downed: [0, 1, 2], corpse: [2] });

    // Save Var 2 Master 7-Action Review Board (7 sheets wide: 7 * 144 = 1008, height 192)
    ...
    const montagePath = path.join(REVIEW_DIR, 'human_male_var2_all_7_actions_12_sprites.png');
    writePNG(montagePath, 7 * 144, 192, montage);
    console.log(`Saved Var 2 master action montage to: ${montagePath}`);
}
