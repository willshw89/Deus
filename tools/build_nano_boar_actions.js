...
// Invariant uniform scale factor matching $UF_Boar.png
const targetScale = 0.1250;
...
function cleanAndQuantizeSheet(sheetBuf, sheetW, sheetH) {
    // Clean stray disconnected isolated specks (< 8 connected pixels)
...
// 1. Compile Boar Eat Sheet
console.log('--- Compiling Boar Eat ---');
const rawEat = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_boar_eat.png')), 'boar_eat');

const eatSouthL = extractSprite(rawEat, 90, 0, 260, 190);
const eatSouthC = extractSprite(rawEat, 400, 0, 650, 190);
const eatSouthR = extractSprite(rawEat, 730, 0, 1020, 190);

const eatWestL  = extractSprite(rawEat, 30,  190, 335, 380);
const eatWestC  = extractSprite(rawEat, 370, 190, 685, 380);
const eatWestR  = extractSprite(rawEat, 720, 190, 1040, 380);

const eatNorthL = extractSprite(rawEat, 50,  575, 310, 767);
const eatNorthC = extractSprite(rawEat, 460, 575, 600, 767);
const eatNorthR = extractSprite(rawEat, 790, 575, 950, 767);
...
assembleAndSave(eatGrid, 'Boar_Eat', 'eat');

// 2. Compile Boar Attack Sheet
console.log('--- Compiling Boar Attack ---');
const rawAttack = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_boar_attack.png')), 'boar_attack');

const atkSouthL = extractSprite(rawAttack, 20,  0, 335, 190);
const atkSouthC = extractSprite(rawAttack, 375, 0, 680, 190);
const atkSouthR = extractSprite(rawAttack, 730, 0, 1040, 190);

const atkWestL  = extractSprite(rawAttack, 15,  190, 345, 380);
const atkWestC  = extractSprite(rawAttack, 360, 190, 690, 380);
const atkWestR  = extractSprite(rawAttack, 720, 190, 1050, 380);

const atkNorthL = extractSprite(rawAttack, 50,  575, 310, 767);
const atkNorthC = extractSprite(rawAttack, 460, 575, 600, 767);
const atkNorthR = extractSprite(rawAttack, 810, 575, 945, 767);
...
assembleAndSave(atkGrid, 'Boar_Attack', 'attack');

// 3. Compile Boar Sleep Sheet
console.log('--- Compiling Boar Sleep ---');
const rawSleep = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_boar_sleep.png')), 'boar_sleep');

const slpSouthL = extractSprite(rawSleep, 25,  0, 335, 185);
const slpSouthC = extractSprite(rawSleep, 405, 0, 640, 185);
const slpSouthR = extractSprite(rawSleep, 740, 0, 1030, 185);

const slpWestL  = extractSprite(rawSleep, 15,  190, 335, 380);
const slpWestC  = extractSprite(rawSleep, 370, 190, 685, 380);
const slpWestR  = extractSprite(rawSleep, 720, 190, 1040, 380);

const slpNorthL = extractSprite(rawSleep, 50,  575, 340, 767);
const slpNorthC = extractSprite(rawSleep, 450, 575, 610, 767);
const slpNorthR = extractSprite(rawSleep, 800, 575, 955, 767);
...
assembleAndSave(slpGrid, 'Boar_Sleep', 'sleep');

console.log('All boar action suites successfully compiled!');
