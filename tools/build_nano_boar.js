...
const raw = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_boar_walk.png')), 'boar');
...
// South (Row 0):
// C0 (angled step L), C1 (front stand), C2 (angled step R)
const southStepLBox = extractSprite(25, 0, 335, 185);
const southStandBox = extractSprite(405, 0, 640, 190);
const southStepRBox = extractSprite(730, 0, 1035, 185);

// West (Row 1):
// C0 (step L), C1 (stand), C3 (step R)
const westStepLBox  = extractSprite(15, 190, 335, 380);
const westStandBox  = extractSprite(370, 190, 685, 380);
const westStepRBox  = extractSprite(1070, 190, 1390, 380);

// North (Row 3):
// C1 (back stand), C2 (back step)
const northStandBox = extractSprite(460, 575, 600, 765);
const northStepBox  = extractSprite(805, 575, 950, 765);
...
// Invariant uniform scale factor for wild boar:
// Boar body is ~38 px long, ~27 px tall in 48x48 cell, grounded at y = 47.
// westStandBox is 304 px wide -> scale factor ~38 / 304 = 0.125
const targetScale = 38.0 / Math.max(westStandBox.w, 290);
...
const gamePng = path.join(ROOT, 'game', 'img', 'characters', '$UF_Boar.png');
const artPng = path.join(ROOT, 'art', 'masters', '$UF_Boar.png');
writePNG(gamePng, sheetW, sheetH, sheetBuf);
writePNG(artPng, sheetW, sheetH, sheetBuf);
console.log('Saved boar sheet:', gamePng);
...
const gameJson = path.join(ROOT, 'game', 'img', 'characters', '$UF_Boar.json');
const artJson = path.join(ROOT, 'art', 'masters', '$UF_Boar.json');
fs.writeFileSync(gameJson, JSON.stringify(sidecar, null, 2), 'utf8');
fs.writeFileSync(artJson, JSON.stringify(sidecar, null, 2), 'utf8');
console.log('Saved boar sidecar:', gameJson);
