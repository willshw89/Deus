...
// Filter palette for Sand Stalker: warm desert sand, chitin tan, sandstone yellow/ochre, dark claws, amber eyes (no purple/magenta/cyan)
const validSandPalette = palette.filter(c => {
    const [r, g, b] = c;
    if (b > g + 20 && b > 60) return false;
    if (b > r + 30 && b > 80) return false;
    if (r > 160 && g < 70 && b < 70) return false; // avoid pure red bodies
    return true;
});

const sandPaletteLab = validSandPalette.map(c => rgbToLab(c[0], c[1], c[2]));

function snapColor(r, g, b) {
    const lab = rgbToLab(r, g, b);
    let bestDist = Infinity;
    let bestIdx = 0;
    for (let i = 0; i < sandPaletteLab.length; i++) {
        const pl = sandPaletteLab[i];
        const dL = lab[0] - pl[0];
        const da = lab[1] - pl[1];
        const db = lab[2] - pl[2];
    ...
    return validSandPalette[bestIdx];
}
...
// Target width for Sand Stalker: 64 px wide (low desert predator per ART_STANDARD §2)
function processFacing(img, name, targetW = 64) {
    ...
    if (targetH > 72) targetH = 72; // Low profile predator
...
function createIdleFrames(baseFrame) {
    ...
    // Frame 1: Carapace/body heave up 1 px (rows 40..88), legs/claws grounded on 89..95
    ...
    // Keep feet/claws on rows 89..95 anchored
    for (let y = 89; y < 96; y++) {
        for (let x = 0; x < 96; x++) {
            const idx = (y * 96 + x) * 4;
    ...
    // Frame 2: Mandibles / claw pincer twitch
    const f2 = Buffer.alloc(96 * 96 * 4);
    baseFrame.copy(f2);
    for (let y = 75; y < 94; y++) {
        for (let x = 16; x <= 80; x++) {
            if ((x >= 18 && x <= 32) || (x >= 64 && x <= 78)) {
                const idx = (y * 96 + x) * 4;
                if (f2[idx + 3] > 0) {
