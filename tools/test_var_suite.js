...
const varNum = parseInt(process.argv[2] || '3', 10);
console.log(`=== Checking Adult Male Human Variation ${varNum} Suite ===\n`);

let allPass = true;
for (const a of actions) {
    const file = `$UF_Human_Male_${varNum}_${a}.png`;
    const sidecar = `$UF_Human_Male_${varNum}_${a}.json`;
...
console.log(`\nResult: ${allPass ? `ALL PASS (7/7) for Var ${varNum}` : `FAIL for Var ${varNum}`}`);
