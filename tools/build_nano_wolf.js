...
function isPurpleOrMagenta(r, g, b) {
    // Magenta and purple background and edge fringe
    if (r > 40 && b > 40 && (r + b) > (g * 2 + 15)) return true;
    if (r > 100 && b > 100 && g < 100) return true;
    return false;
}

// Extract sprite within clean inner bounds
...
const southStandBox = extractSprite(575, 10, 830, 185);
const southStepBox  = extractSprite(365, 585, 550, 760);

const westStepLBox  = extractSprite(10,   395, 340, 572);
const westStandBox  = extractSprite(575,  395, 830, 572);
const westStepRBox  = extractSprite(1070, 395, 1395, 572);

const northStandBox = extractSprite(575, 585, 830, 760);
const northStepBox  = extractSprite(855, 585, 1045, 760);
...
// Invariant uniform scale factor for wild wolf:
// Wolf body is ~38 px long, ~26 px tall in 48x48 cell, grounded at y = 47.
// westStandBox is 238 px wide -> scale factor ~38 / 238 = 0.1596
const targetScale = 38.0 / Math.max(westStandBox.w, 230);
...
// Row 2: East  (mirror of West)
// Row 3: North (stepL, stand, stepR)
const southL = renderCell(southStepBox, false, -1, 0);
const southC = renderCell(southStandBox, false, 0, 0);
const southR = renderCell(southStepBox, true, 1, 0);
...
const eastL = renderCell(westStepLBox, true, 0, 0);
const eastC = renderCell(westStandBox, true, 0, 0);
const eastR = renderCell(westStepRBox, true, 0, 0);

const northL = renderCell(northStepBox, false, -1, 0);
const northC = renderCell(northStandBox, false, 0, 0);
const northR = renderCell(northStepBox, true, 1, 0);
...
const gamePng = path.join(ROOT, 'game', 'img', 'characters', '$UF_Wolf.png');
const artPng = path.join(ROOT, 'art', 'masters', '$UF_Wolf.png');
writePNG(gamePng, sheetW, sheetH, sheetBuf);
writePNG(artPng, sheetW, sheetH, sheetBuf);
console.log('Saved wolf sheet:', gamePng);
...
const gameJson = path.join(ROOT, 'game', 'img', 'characters', '$UF_Wolf.json');
const artJson = path.join(ROOT, 'art', 'masters', '$UF_Wolf.json');
fs.writeFileSync(gameJson, JSON.stringify(sidecar, null, 2), 'utf8');
fs.writeFileSync(artJson, JSON.stringify(sidecar, null, 2), 'utf8');
console.log('Saved wolf sidecar:', gameJson);
