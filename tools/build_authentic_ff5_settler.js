...
// ----------------------------------------------------------------------------
// Authentic FF5 Villager Architecture mapped to Settler (Dynamic Stance)
// Centered in 16x16: col 0 and 15 empty, row 0 empty, boots on row 15.
// ----------------------------------------------------------------------------
const settler_v1 = [
    "................", // 0: empty
    ".....OOOOOO.....", // 1: top hair outline
    "....OhHHHhhO....", // 2: hair crown (highlight left, midtone right)
    "...OhHHHHHHhO...", // 3: full hair volume
    "...OHHhHHHHHHO..", // 4: bangs layer
    "...OHhshhHHHO...", // 5: forehead skin peeking
    "...OHhssshhHHO..", // 6: brow bridge (sss)
    "...OhWOssOWHO...", // 7: FF5 signature eyes (W=white, O=pupil, ss=nose bridge)
    "...OOWOSSOWOO...", // 8: eyes lower / chin start
    "..OTTOOSSSSOttO.", // 9: chin (SSSS) & linen tunic shoulders
    ".OTTTTOOOOTTuuO.", // 10: tunic chest with open neck (OOOO), arms at sides
    ".OsTTOTTTTTTuTsO", // 11: hands (s), tunic body to waist
    ".OSsOOBbbBOOOO..", // 12: rope belt (OBbbBO) & lower tunic hem
    "..OOOPPPqOPPqO..", // 13: trousers emerging below tunic
    "...OPPPqOLO.....", // 14: dark grey-brown trousers & leather boot
    "....OLLlOOOO...."  // 15: boot soles touching bottom row 16
];

// ----------------------------------------------------------------------------
// Settler Variant 2: Grounded Symmetrical Stance
// Both boots firmly planted on bottom row 16.
// ----------------------------------------------------------------------------
const settler_v2 = [
    "................", // 0: empty
    ".....OOOOOO.....", // 1: top hair outline
    "....OhHHHhhO....", // 2: hair crown
    "...OhHHHHHHhO...", // 3: hair volume
    "...OHHhHHHHHHO..", // 4: bangs
    "...OHhshhHHHO...", // 5: forehead skin peeking
    "...OHhssshhHHO..", // 6: brow bridge
    "...OhWOssOWHO...", // 7: FF5 eyes (W white, O pupil)
    "...OOWOSSOWOO...", // 8: eyes lower / chin
    "..OTTOOSSSSOttO.", // 9: chin & shoulders
    ".OTTTTOOOOTTuuO.", // 10: tunic chest with V-neck
    ".OsTTOTTTTTTuTsO", // 11: hands at sides
    ".OSsOOBbbBOssO..", // 12: brown rope belt & tunic hem
    "..OOPPPq..pqqO..", // 13: trousers below tunic
    "..OLLll....LLlO.", // 14: leather boots
    ".OOLLll....LLlOO"  // 15: both boots touching bottom row 16
];

// ----------------------------------------------------------------------------
// Settler Variant 3: Classic FF5 Bartz/Villager Stance with Longer Tunic Skirt
// Tunic reaches mid-thigh on row 12, trousers on row 13-14, boots on row 15.
// ----------------------------------------------------------------------------
const settler_v3 = [
    "................", // 0: empty
    ".....OOOOOO.....", // 1: hair top
    ...
    "...OHhssshhHHO..", // 6: brow bridge
    "...OhWOssOWHO...", // 7: eyes
    "...OOWOSSOWOO...", // 8: eyes / chin
    "..OTTOOSSSSOttO.", // 9: chin (SSSS) & shoulders
    ".OTTTTOOOOTTuuO.", // 10: chest
    ".OsTTOttttTuuTsO", // 11: linen midtone folds, hands
    "..OOBBbbbBOO....", // 12: rope belt at waist
    "..OTTTTTtttttuO.", // 13: mid-thigh tunic skirt
    "...OPPpq..OppqO.", // 14: trousers & boot tops
    "...OLLll..OLLlO."  // 15: boots touching bottom row 16
];
...
function render(grid, name) {
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            const ch = grid[y][x];
            const c = pal[ch];
            if (!c) throw new Error(`Missing char ${ch} at ${x},${y} in ${name}`);
            const idx = (y * w + x) * 4;
            buf[idx] = c[0];
            buf[idx + 1] = c[1];
            buf[idx + 2] = c[2];
...
render(settler_v1, 'settler_ff5_v1');
render(settler_v2, 'settler_ff5_v2');
render(settler_v3, 'settler_ff5_v3');
console.log('Rendered settler_ff5_v1, v2, v3.');
