...
function isPurpleOrMagenta(r, g, b) {
    if (r > 40 && b > 40 && (r + b) > (g * 2 + 15)) return true;
...
const targetScale = 0.1597; // exact invariant scale factor for wolf
...
// 1. Compile Wolf Eat Sheet
console.log('--- Compiling Wolf Eat ---');
const rawEat = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_wolf_eat.png')), 'wolf_eat');

const eatSouthL = extractSprite(rawEat, 290, 10, 550, 185);
const eatSouthC = extractSprite(rawEat, 575, 10, 835, 185);
const eatSouthR = extractSprite(rawEat, 855, 10, 1050, 185);

const eatWestL  = extractSprite(rawEat, 10,   195, 275, 380);
const eatWestC  = extractSprite(rawEat, 575,  195, 830, 380);
const eatWestR  = extractSprite(rawEat, 1070, 195, 1395, 380);

const eatNorthL = extractSprite(rawEat, 575, 580, 830, 765);
const eatNorthC = extractSprite(rawEat, 850, 580, 1050, 765);
const eatNorthR = extractSprite(rawEat, 575, 580, 830, 765);
...
const eatGrid = [
    ...
    [renderCell(eatNorthL), renderCell(eatNorthC), renderCell(eatNorthR, true)]
];
assembleAndSave(eatGrid, 'Wolf_Eat', 'eat');

// 2. Compile Wolf Attack Sheet
console.log('--- Compiling Wolf Attack ---');
const rawAttack = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_wolf_attack.png')), 'wolf_attack');

const atkSouthL = extractSprite(rawAttack, 290, 10, 550, 185);
const atkSouthC = extractSprite(rawAttack, 575, 10, 835, 185);
const atkSouthR = extractSprite(rawAttack, 855, 10, 1050, 185);

const atkWestL  = extractSprite(rawAttack, 360, 195, 560, 380);
const atkWestC  = extractSprite(rawAttack, 575, 195, 835, 380);
const atkWestR  = extractSprite(rawAttack, 10,  195, 275, 380);

const atkNorthL = extractSprite(rawAttack, 575, 580, 830, 765);
const atkNorthC = extractSprite(rawAttack, 850, 580, 1050, 765);
const atkNorthR = extractSprite(rawAttack, 575, 580, 830, 765);
...
const atkGrid = [
    ...
    [renderCell(atkNorthL), renderCell(atkNorthC), renderCell(atkNorthR, true)]
];
assembleAndSave(atkGrid, 'Wolf_Attack', 'attack');

// 3. Compile Wolf Sleep Sheet
console.log('--- Compiling Wolf Sleep ---');
const rawSleep = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_wolf_sleep.png')), 'wolf_sleep');

const slpSouthL = extractSprite(rawSleep, 290, 10, 550, 185);
const slpSouthC = extractSprite(rawSleep, 855, 10, 1050, 185);
const slpSouthR = extractSprite(rawSleep, 290, 10, 550, 185);

const slpWestL  = extractSprite(rawSleep, 10,  195, 275, 380);
const slpWestC  = extractSprite(rawSleep, 575, 195, 835, 380);
const slpWestR  = extractSprite(rawSleep, 1070, 195, 1395, 380);

const slpNorthL = extractSprite(rawSleep, 360, 580, 560, 765);
const slpNorthC = extractSprite(rawSleep, 850, 580, 1050, 765);
const slpNorthR = extractSprite(rawSleep, 360, 580, 560, 765);

const slpGrid = [
    [renderCell(slpSouthL), renderCell(slpSouthC), renderCell(slpSouthR, true)],
    ...
    [renderCell(slpNorthL), renderCell(slpNorthC), renderCell(slpNorthR, true)]
];
assembleAndSave(slpGrid, 'Wolf_Sleep', 'sleep');

console.log('All wolf action suites successfully compiled!');
