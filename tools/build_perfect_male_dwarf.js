...
/**
 * tools/build_perfect_male_dwarf.js
 *
 * UNIFORM SCALE BUILDER for Adult Male Dwarf (Mountain Folk Settler / Miner)
...
// Target dwarf height: ~36-37px in RMMZ (stocky ~2.6 heads, grounded at y = 47)
let UNIFORM_SCALE = 36.0 / 234.0;
...
function extractGridBBoxes(rawImg) {
    const cellW = rawImg.width / 6;
...
function processAction(rawFileName, actionTag, animations) {
    ...
    const grid = extractGridBBoxes(rawImg);

    // If walk sheet, calibrate UNIFORM_SCALE from row0 standing sprite height
    if (actionTag === 'Walk') {
    ...
    let S, W, E, N;
    if (actionTag === 'Downed') {
        // Downed has flinch (col 0), collapse (col 4), prone corpse (col 5)
        S = [grid[0][0], grid[0][4], grid[0][5]].map(b => extractUniformSprite(rawImg, b));
        W = [grid[1][0], grid[1][4], grid[1][5]].map(b => extractUniformSprite(rawImg, b));
        E = W.map(mirrorFrame);
        N = [grid[2][0], grid[2][4], grid[2][5]].map(b => extractUniformSprite(rawImg, b));
    } else if (actionTag === 'Attack') {
        // Attack: row 0 cols 0..2 (South), row 1 cols 0..2 (West), row 1 cols 3..5 (East), row 2 cols 3..5 (North)
        ...
        E = [grid[1][3], grid[1][4], grid[1][5]].map(b => extractUniformSprite(rawImg, b));
        N = [grid[2][3], grid[2][4], grid[2][5]].map(b => extractUniformSprite(rawImg, b));
    } else if (actionTag === 'Bow') {
        // Crossbow: aim, tension, pluck
        S = [grid[0][0], grid[0][1], grid[0][2]].map(b => extractUniformSprite(rawImg, b));
        W = [grid[1][0], grid[1][1], grid[1][2]].map(b => extractUniformSprite(rawImg, b));
        E = [grid[1][3], grid[1][4], grid[1][5]].map(b => extractUniformSprite(rawImg, b));
        N = [grid[2][3], grid[2][4], grid[2][5]].map(b => extractUniformSprite(rawImg, b));
    } else if (actionTag === 'Work') {
        // Checking work, kneeling craftsman, hammer strike
        S = [grid[0][0], grid[0][1], grid[0][2]].map(b => extractUniformSprite(rawImg, b));
        W = [grid[1][0], grid[1][1], grid[1][2]].map(b => extractUniformSprite(rawImg, b));
        E = [grid[1][3], grid[1][4], grid[1][5]].map(b => extractUniformSprite(rawImg, b));
        N = [grid[2][3], grid[2][4], grid[2][5]].map(b => extractUniformSprite(rawImg, b));
        ...
        // Walk, Haul, Magic
        S = [grid[0][0], grid[0][1], grid[0][2]].map(b => extractUniformSprite(rawImg, b));
        W = [grid[1][0], grid[1][3] || grid[1][0], grid[1][0]].map(b => extractUniformSprite(rawImg, b));
        E = [grid[1][2], grid[1][5] || grid[1][2], grid[1][2]].map(b => extractUniformSprite(rawImg, b));
        N = [grid[2][3], grid[2][4], grid[2][5]].map(b => extractUniformSprite(rawImg, b));
    }

    const sheet = assemble12Charset({ S, W, E, N });
    saveSheetAndSidecar(sheet, `Dwarf_Male_${actionTag}`, actionTag, animations);
    return sheet;
}

function buildAll() {
    console.log('=== Building Adult Male Dwarf 12-Sprite Action Suite (Serious Chibi) ===\n');

    // 1. Walk (Master Sheet)
    console.log('1. Walk...');
    const walkSheet = processAction('dwarf_male_walk_12_raw.png', 'Walk', { walk: [0, 1, 2, 1], stand: [1] });
    fs.writeFileSync(path.join(CHAR_DIR, '$UF_Dwarf_Male.png'), writePNG(walkSheet, 144, 192));
    fs.writeFileSync(path.join(CHAR_DIR, '$UF_Dwarf.png'), writePNG(walkSheet, 144, 192));
    console.log('Updated $UF_Dwarf_Male.png and $UF_Dwarf.png aliases');

    // 2. Haul
    console.log('2. Haul...');
    const haulSheet = processAction('dwarf_male_haul_12_raw.png', 'Haul', { haul: [0, 1, 2, 1], carry: [1], stand: [1] });

    // 3. Attack (Axe)
    console.log('3. Attack (Battleaxe)...');
    const attackSheet = processAction('dwarf_male_attack_12_raw.png', 'Attack', { attack: [0, 1, 2], stand: [0] });
    fs.writeFileSync(path.join(CHAR_DIR, '$UF_Dwarf_Attack_Axe.png'), writePNG(attackSheet, 144, 192));

    // 4. Bow (Crossbow)
    console.log('4. Bow (Crossbow)...');
    const bowSheet = processAction('dwarf_male_bow_12_raw.png', 'Bow', { bow: [0, 1, 2], stand: [0] });
    fs.writeFileSync(path.join(CHAR_DIR, '$UF_Dwarf_Attack_Crossbow.png'), writePNG(bowSheet, 144, 192));

    // 5. Magic (Rune Hammer)
    console.log('5. Magic (Rune Hammer)...');
    const magicSheet = processAction('dwarf_male_magic_12_raw.png', 'Magic', { cast: [0, 1, 2], stand: [0] });
    fs.writeFileSync(path.join(CHAR_DIR, '$UF_Dwarf_Cast_Hammer.png'), writePNG(magicSheet, 144, 192));

    // 6. Work
    console.log('6. Work (Blacksmith/Miner)...');
    const workSheet = processAction('dwarf_male_work_12_raw.png', 'Work', { work: [0, 1, 2], stand: [0] });

    // 7. Downed
    console.log('7. Downed...');
    const downedSheet = processAction('dwarf_male_downed_12_raw.png', 'Downed', { hurt: [0], collapse: [1], dead: [2] });
    ...
    const reviewPath = path.join(REVIEW_DIR, 'dwarf_male_all_7_actions_12_sprites.png');
