...
async function runNativeSurvivalSoak() {
    ...
    
    {
        ...
        const LIMIT = Number(process.env.PROBE_LIMIT || 5000);
        let n = 0;
        const log = s => { if (n++ < LIMIT) console.log("  PROBE " + hm() + " " + s); };
        const itemAt = id => { const it = I.get(id); return it ? `${it.type}x${it.count} holder ${it.holder} cont ${it.container} at ${it.x},${it.y}` : "gone"; };
        const seen = new Map();
        env.UF.Events.on("jobs:failed", j => {
            if (!j || !["eat", "fetch", "haul"].includes(j.type)) return;
            const k = j.type + ":" + j.reason;
            seen.set(k, (seen.get(k) || 0) + 1);
            if (seen.get(k) <= 6) log(`FAILED ${j.type}#${j.id} owner ${j.owner} assigned ${j.assigned} reason "${j.reason}" item ${j.params && j.params.itemId !== undefined ? itemAt(j.params.itemId) : "-"} tgt ${j.target ? j.target.x + "," + j.target.y : "-"} params ${JSON.stringify(j.params || {}).slice(0, 160)}`);
        });
        env.UF.__probeHourly = () => {
            if (env.$ufTime.minute !== 0 || env.$ufTime.hour % 6 !== 0) return;
            for (const u of W.units().filter(u => u && u.data && (u.data.founder || u.data.kind === "colonist") && !u.data.dead)) {
                const inv = I.inventoryOf(u.id).map(it => it.type + "x" + it.count).join(" ");
                const enc = typeof I.encumbrance === "function" ? I.encumbrance(u.id) : null;
                const w = typeof I.carriedWeight === "function" ? I.carriedWeight(u.id) : null;
                const nd = u.data.needs || {};
                log(`PACK ${u.name}#${u.id} food ${nd.foodLb} load ${w === null ? "?" : w.toFixed(1)} lb enc ${enc ? JSON.stringify(enc) : "-"} inv [${inv}]`);
            }
            if (env.$ufTime.hour === 0) log("FAILCOUNTS " + JSON.stringify([...seen.entries()]));
        };
    }
    ...
    for (let day = 1; day <= SOAK_DAYS; day++) {
        ...
        for (let hour = 0; hour < 24; hour++) {
            ...
            for (let minute = 0; minute < 60; minute++) {
                env.$ufTime.minute = minute; if (env.UF.__probeHourly) env.UF.__probeHourly();
