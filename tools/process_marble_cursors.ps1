...
# 1. Process Diagonal Mouse Cursor (Tip at top-left 4, 4)
$mouseJpg = "$brainDir\deus_marble_lightning_cursor_1789929184670.jpg"
Write-Host "Processing Marble Mouse Cursor: $mouseJpg"
...
for ($y = 0; $y -lt $targetSize; $y++) {
    for ($x = 0; $x -lt $targetSize; $x++) {
        ...
        if ($px.R -gt 32 -or $px.G -gt 32 -or $px.B -gt 32) {
            $snap = FindNearestPaletteColor($px)
            $canvas.SetPixel($x + 4, $y + 4, $snap)
        }
    }
}
$src.Dispose()
...
$pointerJpg = "$brainDir\deus_marble_lightning_pointer_1789929126268.jpg"
Write-Host "Processing Marble Menu Pointer: $pointerJpg"
...
# Target size 32x20 inside 48x48
$targetW = 32
$targetH = 20
...
# Place so right tip touches x = 46 and vertically centered at y = 24
$offsetX = 46 - $targetW
$offsetY = [Math]::Max(0, [int]((48 - $targetH) / 2))
