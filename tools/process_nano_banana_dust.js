...
// Target colors from Prompt 13 for dust puff
const COLOR_CREAM   = [239, 235, 231]; // #EFEBE7
const COLOR_LIGHT   = [206, 198, 190]; // #CEC6BE
const COLOR_MID     = [174, 166, 158]; // #AEA69E
const COLOR_SHADOW  = [142, 134, 126]; // #8E867E
...
function loadJpgOrPng(filePath) {
    ...
    const tmpPng = path.join(os.tmpdir(), `tmp_dust_${Date.now()}.png`);
...
/**
 * Process raw Nano Banana image into master, raw 4x, and RMMZ single character sheet
 */
function processDust(srcPath) {
    ...
    console.log(`Loaded Nano Banana dust image: ${rawImg.width}x${rawImg.height}`);

    // Bounding detection for 4 frames arranged horizontally
    // 4 frames divide roughly into 4 quarters along width
    ...
    for (let f = 0; f < 4; f++) {
        ...
        if (count > 0) {
            ...
            console.log(`Frame ${f}: bounds [${minX}..${maxX}] x [${minY}..${maxY}], count=${count}`);
        } else {
    ...
    // Target 48x48 cell per frame, dust puff grounded at row 46-47 (anchor [24, 47])
    const TARGET_CX = 24;
    const TARGET_BOTTOM_Y = 46;
    ...
    for (let f = 0; f < 4; f++) {
        ...
        for (let gy = 0; gy < 48; gy++) {
            for (let gx = 0; gx < 48; gx++) {
                ...
                const relGy = gy - TARGET_BOTTOM_Y;

                const srcX = Math.round(box.cx + relGx * blockSize);
                const srcY = Math.round(box.maxY + relGy * blockSize);
                ...
                if (count > totalWinPixels * 0.25) {
                    ...
                    let chosen = COLOR_LIGHT;
                    if (lum > 220) chosen = COLOR_CREAM;
                    else if (lum > 185) chosen = COLOR_LIGHT;
                    else if (lum > 150) chosen = COLOR_MID;
                    else chosen = COLOR_SHADOW;
    ...
    const masterPngPath = path.join(ROOT, 'art', 'masters', 'fx_dust.png');
    ...
    fs.writeFileSync(path.join(ROOT, 'art', 'masters', 'fx_dust.json'), JSON.stringify(masterJson, null, 2));
    ...
    const rawPngPath = path.join(ROOT, 'art', 'raw', 'fx_dust.png');
    writePNG(rawPngPath, 768, 192, raw4xBuf);

    // 3. Export RMMZ single character sheet: 144x192 (3 columns x 4 rows) with transparent background
    // Key frames for 3-frame sequence: Col 0: Frame 0 (initial), Col 1: Frame 2 (peak), Col 2: Frame 3 (dissipate)
    const rmmzFrameIndices = [0, 2, 3];
    ...
    const gameSheetPath = path.join(ROOT, 'game', 'img', 'characters', '!$UF_fx_dust.png');
    ...
    fs.writeFileSync(path.join(ROOT, 'game', 'img', 'characters', '!$UF_fx_dust.json'), JSON.stringify(gameJson, null, 2));

    console.log('Successfully processed and exported fx_dust (master, raw, and RMMZ character sheet)!');
    return true;
}

if (require.main === module) {
    const srcArg = process.argv[2] || path.join(ROOT, 'art', 'raw', 'fx_dust_raw.jpg');
    processDust(srcArg);
}

module.exports = { processDust };
