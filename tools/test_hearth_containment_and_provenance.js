 ...
 * 5. Rule 4 negative control mutants: --mutant=hearth_escapes_freely (a normal hearth escapes at 0.5 a beat),
 *    --mutant=spread_forgets_origin (a spread cell gets a fresh fire id) and --mutant=fire_ids_restart_on_load (fire
 *    ids from a counter in the plugin instance, not the save) -> each must FAIL (exit 1).
 *
...
function makeSandbox() {
    ...
    const sandbox = {
        ...
        UF: {
            ...
            Jobs: { douseJobs: () => [], list: () => [], handler: () => null },
            Time: { ticks: () => clock },
            Events: {
                on(name, fn) {
