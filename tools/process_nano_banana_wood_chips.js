...
// Target colors from Prompt 13 / SEG-18 for wood chips
const COLOR_PALE    = [202, 178, 146]; // #CAB292 (sapwood / splinter glint)
const COLOR_TIMBER  = [170, 134, 89];  // #AA8659 (fresh cut heartwood)
const COLOR_BARK    = [125, 77, 24];   // #7D4D18 (outer bark)
const COLOR_DARK    = [93, 53, 12];    // #5D350C (bark shadow)
const COLOR_DEEP    = [61, 36, 12];    // #3D240C (deep notch)
...
function loadJpgOrPng(filePath) {
    ...
    const tmpPng = path.join(os.tmpdir(), `tmp_wood_${Date.now()}.png`);
...
function processWoodChips(srcPath) {
    ...
    console.log(`Loaded Nano Banana wood chips image: ${rawImg.width}x${rawImg.height}`);
    ...
    for (let f = 0; f < 4; f++) {
        ...
        if (count > 0) {
            ...
            console.log(`Wood Frame ${f}: bounds [${minX}..${maxX}] x [${minY}..${maxY}], count=${count}`);
        } else {
    ...
    // Target 48x48 cell per frame, impact centered at (24, 30)
    const TARGET_CX = 24;
    const TARGET_CY = 30;
        ...
        for (let gy = 0; gy < 48; gy++) {
            for (let gx = 0; gx < 48; gx++) {
                ...
                if (count > totalWinPixels * 0.25) {
                    ...
                    let chosen = COLOR_TIMBER;
                    if (lum > 180) chosen = COLOR_PALE;
                    else if (lum > 130) chosen = COLOR_TIMBER;
                    else if (lum > 85) chosen = COLOR_BARK;
                    else if (lum > 50) chosen = COLOR_DARK;
                    else chosen = COLOR_DEEP;
    ...
    const masterPngPath = path.join(ROOT, 'art', 'masters', 'fx_wood_chips.png');
    ...
    fs.writeFileSync(path.join(ROOT, 'art', 'masters', 'fx_wood_chips.json'), JSON.stringify(masterJson, null, 2));
    ...
    const rawPngPath = path.join(ROOT, 'art', 'raw', 'fx_wood_chips.png');
    ...
    const gameSheetPath = path.join(ROOT, 'game', 'img', 'characters', '$UF_fx_wood_chips.png');
    ...
    fs.writeFileSync(path.join(ROOT, 'game', 'img', 'characters', '$UF_fx_wood_chips.json'), JSON.stringify(gameJson, null, 2));

    console.log('Successfully processed and exported fx_wood_chips (master, raw, and RMMZ sheet)!');
    return true;
}

if (require.main === module) {
    const srcArg = process.argv[2] || path.join(ROOT, 'art', 'raw', 'fx_wood_chips_raw.jpg');
    processWoodChips(srcArg);
}

module.exports = { processWoodChips };
