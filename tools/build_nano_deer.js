...
const raw = decodePNG(fs.readFileSync(path.join(ROOT, 'art', 'raw', 'pro_deer_walk.png')), 'deer');
...
// South:
const southStandBox = extractSprite(280, 10, 430, 250);
const southStepBox  = extractSprite(290, 520, 420, 765);

// West:
const westStepLBox  = extractSprite(15, 270, 215, 510);
const westStandBox  = extractSprite(250, 270, 440, 510);
const westStepRBox  = extractSprite(480, 270, 685, 510);

// East:
const eastStepLBox  = extractSprite(720, 270, 930, 510);
const eastStandBox  = extractSprite(965, 270, 1160, 510);
const eastStepRBox  = extractSprite(1180, 270, 1390, 510);

// North:
const northStandBox = extractSprite(760, 520, 890, 765);
const northStepBox  = extractSprite(990, 520, 1120, 765);
...
// Invariant uniform scale factor for wild red deer stag:
// Height with antlers is ~46 px tall in 48x48 cell, grounded at y = 47.
// southStandBox height is 235 px -> scale factor ~46 / 235 = 0.1957
const targetScale = 46.0 / Math.max(southStandBox.h, 230);
...
const gamePng = path.join(ROOT, 'game', 'img', 'characters', '$UF_Deer.png');
const artPng = path.join(ROOT, 'art', 'masters', '$UF_Deer.png');
writePNG(gamePng, sheetW, sheetH, sheetBuf);
writePNG(artPng, sheetW, sheetH, sheetBuf);
console.log('Saved deer sheet:', gamePng);
...
const gameJson = path.join(ROOT, 'game', 'img', 'characters', '$UF_Deer.json');
const artJson = path.join(ROOT, 'art', 'masters', '$UF_Deer.json');
fs.writeFileSync(gameJson, JSON.stringify(sidecar, null, 2), 'utf8');
fs.writeFileSync(artJson, JSON.stringify(sidecar, null, 2), 'utf8');
console.log('Saved deer sidecar:', gameJson);
