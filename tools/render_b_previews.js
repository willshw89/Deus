...
const targets = ['Outside_B.png', 'Dungeon_B.png'];

for (const name of targets) {
    const p = path.join(__dirname, '..', 'game', 'img', 'tilesets', name);
    const img = decodePNG(fs.readFileSync(p), name);
    const sw = 512, sh = 512;
    ...
    const outPath = path.join(__dirname, '..', 'art', 'review', 'nano_' + name);
