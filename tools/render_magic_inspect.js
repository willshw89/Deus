...
const magicRaw = loadJpg(path.join(RAW_DIR, 'elf_magic_nano_banana_raw.jpg'));
const bowRaw = loadJpg(path.join(RAW_DIR, 'elf_bow_nano_banana_raw.jpg'));

// Create preview image of magicRaw and bowRaw scaled down
const outW = 1024, outH = 1024;
const outBuf = Buffer.alloc(outW * outH * 4);
for (let i = 0; i < outBuf.length; i += 4) {
    outBuf[i] = magicRaw.data[i];
    outBuf[i+1] = magicRaw.data[i+1];
    outBuf[i+2] = magicRaw.data[i+2];
    outBuf[i+3] = 255;
}

fs.writeFileSync(path.join(ROOT, 'art', 'review', 'magic_raw_inspect.png'), writePNG(outBuf, outW, outH));
console.log('Saved magic_raw_inspect.png');
