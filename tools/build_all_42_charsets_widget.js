...
/**
 * tools/build_all_42_charsets_widget.js
 *
 * Builds the comprehensive master showcase widget for Adult Male Human
 * featuring all 42 charsets (6 variations x 7 actions).
 * Outputs to artifact directory: human_male_master_showcase.html
 */
...
const VARIATIONS = [
    {
        num: 1,
        name: 'Var 1 — Master Settler / Militia',
        role: 'Faction Iron Arming Sword & Round Wooden Buckler',
        desc: 'Chestnut brown hair, clean-shaven, rustic brown doublet & linen shirt, dark breeches, leather boots.',
        prefix: 'Human_Male'
    },
    {
        num: 2,
        name: 'Var 2 — Frontier Scout / Forester',
        role: 'Dual Frontier Hunting Daggers & Light Composite Yew Bow',
        desc: 'Golden blonde swept-back hair, light stubble, forest olive-green woolen tunic with dark vest, breeches, boots.',
        prefix: 'Human_Male_2'
    },
    {
        num: 3,
        name: 'Var 3 — Heavy Town Guard / Veteran',
        role: 'Heavy Iron Broadsword & Garrison Longbow',
        desc: 'Raven black shaggy hair, full dark beard and mustache, charcoal grey wool tunic, iron-riveted cross-belt.',
        prefix: 'Human_Male_3'
    },
    {
        num: 4,
        name: 'Var 4 — Artisan Woodsman / Carpenter',
        role: 'Broad Bearded Woodcutter Axe & Carpentry Mallet',
        desc: 'Fiery auburn hair, trimmed red goatee, terracotta/ochre artisan tunic with rolled sleeves, work breeches.',
        prefix: 'Human_Male_4'
    },
    {
        num: 5,
        name: 'Var 5 — Blacksmith / Quarryman',
        role: 'Heavy Smithing Hammer & Miner Crossbow / Sling',
        desc: 'Shaved bald head, rugged dark full beard and mustache, rawhide leather work vest, heavy apron trousers.',
        prefix: 'Human_Male_5'
    },
    {
        num: 6,
        name: 'Var 6 — Seasoned Veteran Ranger',
        role: 'Arthurian Yew Longbow & Steel Sidearm Hunting Sword',
        desc: 'Salt-and-pepper steel grey hair, trimmed grey beard, deep indigo-blue woolen tunic with leather cross-strap.',
        prefix: 'Human_Male_6'
    }
];
...
const ACTIONS = [
    ...
    { key: 'Attack', name: 'Melee Combat', desc: 'Faction-specific melee combat strike with curved luminous steel slash arcs' },
    { key: 'Bow', name: 'Archery / Ranged', desc: 'Faction-specific bow/crossbow aim, string tension draw, and pluck recoil. ZERO flying projectiles (VISION V111)' },
...
const closeupB64 = toBase64(path.join(REVIEW_DIR, 'human_male_6_variations_live_closeup.png'));
const normalB64  = toBase64(path.join(REVIEW_DIR, 'human_male_6_variations_live_normal.png'));
...
<head>
  <meta charset="UTF-8">
  <title>Adult Male Human — Master 42-Charset Showcase</title>
  ...
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="bg-[#141b27] border border-slate-700/80 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        <div class="flex flex-wrap items-center gap-2">
          <span class="px-2.5 py-0.5 rounded text-xs font-bold uppercase tracking-wider bg-emerald-950/90 text-emerald-400 border border-emerald-800/60">Demographic 1: Adult Male</span>
    ...
    <div class="bg-[#141b27] border border-slate-700/80 rounded-2xl p-6 shadow-xl">
      <div class="flex justify-between items-center mb-4">
        ...
        <span class="px-3 py-1 rounded bg-emerald-950/80 border border-emerald-700/60 text-emerald-400 text-xs font-bold">13/13 Smoke Tests PASS</span>
      </div>
    ...
    <div class="bg-[#141b27] border border-slate-700/80 rounded-2xl p-6 shadow-xl space-y-5">
      ...
      <div id="infoCard" class="bg-slate-800/40 border border-slate-700/60 rounded-xl p-4 text-xs space-y-1">
        <div class="flex items-center gap-2">
          <span class="font-bold text-amber-300 text-sm" id="infoName">Variation 1 — Master Settler / Militia</span>
          <span class="text-slate-400">•</span>
          <span class="text-emerald-400 font-semibold" id="infoAction">Walk Cycle</span>
        </div>
        <p class="text-slate-300" id="infoRole">Role: Faction Iron Arming Sword & Round Wooden Buckler</p>
        <p class="text-slate-400" id="infoDesc">Chestnut brown hair, clean-shaven, rustic brown doublet & linen shirt, dark breeches, leather boots.</p>
        <p class="text-amber-400/90 font-mono text-[11px] mt-1" id="infoFile">File: $UF_Human_Male_Walk.png (144×192 px, 31 colors, binary alpha, grounded y=47)</p>
      </div>
    ...
    <div class="bg-[#141b27] border border-slate-700/80 rounded-2xl p-6 shadow-xl space-y-3">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-xs text-slate-400">864×192 px master review montage showing complete demographic variety, hair, beards, and tunics.</p>
        </div>
  ...
  <script>
    ...
    function renderActiveFrame() {
        getSheetImage(currentVar, currentAct, (img) => {
            ...
            // Standard walk/haul/work: [0, 1, 2, 1]
            // Attack/Bow/Downed: [0, 1, 2]
...
const outPath = path.join(BRAIN_DIR, 'human_male_master_showcase.html');
