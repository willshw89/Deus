# Lane Q Brief: Structure, material and support/collapse design, docs only (SIM.40.01)
...
**Lane:** lane-q | **Task ID:** SIM.40.01 (support model and blast propagation; also drafts the missing material-and-structure model that the audit §6 step 5 names as SIM.40.01's unlisted prerequisite, and the hooks SIM.40.02-.04 need) | **Branch:** task/lane-q | **Worktree:** `C:\Users\snewt\.deus_worktrees\lane-q` | **Writer:** claude | **Reviewer:** grok (independent, launched later by the PM; the task is not DONE until that review passes) | **Base:** origin/main `84ee3b55f3f8c1b8a16710ccf3878795d98d1683` | **Source:** Directive 0028-AC A2 item 6 (PM design queue), re-scoped by the PM on 2026-09-26 to align with the SIM.50.01 living-world gap audit.

**allowedPaths** (exact):
- `tasks/SIM.40.01/lane-q/**` (the launcher also writes its saved prompt under `tasks/SIM.40.01/lane-q/launches/`; leave it alone)

**FORBIDDEN:** everything else. In particular `game/**` and `tools/**` (read only; commit no code, scripts or tests; ad-hoc `node -e` one-liners and `git grep` runs for counting are fine), `docs/**` (read only, including `docs/STATUS.md`, `docs/VISION.md`, `docs/OWNER_DECISIONS.md`, every `*WBS*.md`, `docs/audits/**`, `docs/design/**`, `docs/telemetry/**`), `art/**`. You do NOT mint WBS IDs, change WBS statuses or answer Owner questions. Proposed new work items are written `PROPOSED-Q-NN`, never as real IDs. (0028-AC named `docs/design/sim/SIM.40.01_STRUCTURAL_SUPPORT.md` as the output path; the PM moved the deliverable into this lane's task folder to keep the write set tight. Do not write under `docs/`.)

## Goal
Design how structures exist, bear load, fail and conserve matter in DEUS's 32-layer strata world, so later code lanes (the material model, SIM.40.01-.04, SIM.50.10, SIM.60.03, GP.07.02) can be written against one reviewed spec. The audit found no support propagation or collapse (SUP-1), buildings stored as objects rather than strata so no support model can see them (LAND-5, SUP-2), no rubble or debris material (SUP-3), no falling damage (SUP-4), no cross-layer blast attenuation and no gameplay caller of `applyVolumeDamage` (SUP-5), and no mass ledger (F-04). Buildings, natural rock and rubble must all be matter in the same strata model.
...
- Audit sections for this lane: §2.2 vertical model, §2.3 storage, §2.4 mutation API and events, §2.6 conservation, §3.7 land reshaping (LAND-1, LAND-5), §3.9 geological events, §4.1 support and collapse (SUP-1..5), §5, §6, Appendix A.
- `docs/design/DURABILITY.md`, `docs/design/VERTICAL_BUILD_PLAN.md`, `game/data/DEUS_WorldCatalog.json` (materials, constructions, `weatherResistance`/`corrosionResistance`), `game/js/plugins/DEUS_Jobs.js` (build jobs ~L721), `DEUS_Walls.js`, `DEUS_Doors.js`, `DEUS_Floors.js`, `DEUS_Interact.js` (dig ~L167).
- SRD 5.1 object rules in `rules.json` (object AC and hit points by material and size, damage thresholds) and blast spells in `spells.json` (for example Fireball, Shatter, Earthquake, Disintegrate, Wall of Stone, Move Earth). SRD object numbers are the baseline for material durability where they exist; say so where the design extrapolates.
- Unreviewed sibling input if present: Lane P's SRD spell-effect audit (`origin/task/lane-p`: `docs/audits/SRD_SPELL_EFFECT_AUDIT.md`, under Grok review).
...
| ID | Required content |
|---|---|
| Q-01 | **Material model gap (audit §6 step 5):** today's materials (`DEUS_Levels.js` ~L1006: solid, support, maxHP, debris) versus what support, collapse, decay and fire need. Propose a material property table: mass/density per 2-ft slice of a 5-ft cell, load capacity, span capacity, porosity, flammability, weathering and corrosion class, SRD object AC/HP link. Propose the new materials (rubble, loose fill, sediment, ash, ice, scrap, rust/mineral trace) and their free ids. Write it as the input to a PROPOSED material-and-structure model package that precedes SIM.40.01. |
| Q-02 | **Buildings as strata (LAND-5, SUP-2):** walls, floors, roofs, doors, stairs, ramps, columns and furniture: which become constructed strata (using the existing constructed flag ~L1602), which stay objects, how a build job writes them, how thickness maps to 2-ft slices, and the migration of existing saves/objects. Foundations must be stratum matter that later ruins and re-founding can reuse (SET-4). |
| Q-03 | **Support model:** load paths from constructed and natural strata down to bedrock or the area edge; natural rock and ledges (V128, DEC-010); span limits in 5-ft cells for floors, bridges, roofs and cave ceilings; vertical propagation through 2-ft slices and 10-ft layers across -16..+15. |
| Q-04 | **Localized support recheck (V133):** triggered by the `writeCell` / `levels:strataChanged` event (~L1558), bounded neighbourhood and worst-case bound, queue and budget per tick, determinism, no full-world scan; what happens at area/region edges and under LOD. |
| Q-05 | **Sudden collapse:** dig, blast, overload or removed support causing immediate failure: the cascade down through layers, conversion of the failed strata into rubble strata/loose items of equal mass, talus/angle-of-repose settling, fall damage to units and objects below (V95, SUP-4), and the support recheck the rubble itself triggers. |
| Q-06 | **Decay-driven collapse:** the interface through which decay (SIM.40.05, Lane R) lowers member strength over time and hands off to the same collapse path; roofs before walls; localized rechecks only; how an abandoned building ends as rubble (interface contract with Lane R stated explicitly). |
| Q-07 | **Blasts breaking floors across layers:** cross-layer blast propagation with 10 ft per layer, floor thickness in 2-ft slices, material attenuation (DEC-013 §4), `applyVolumeDamage`, SRD damage kept as the baseline (DEC-018), breaching into the layer below, effects on units on both layers, and the coupling to GP.07.02 / DEC-022 volume damage and SIM.60 spell primitives. |
| Q-08 | **Mass conservation via the ledger (LIFE-001, WG.65.15):** the ledger classes this design touches and the exact entries for build, dig, quarry, collapse, blast, rubble clearing and salvage; how the known leaks (LAND-1: dig creating stone, quarry 2-in-4-out, roof decks from nothing) are closed; conjured matter (Wall of Stone) as a logged source/sink. |
| Q-09 | **Sparse storage and cost at 32 layers:** representation of constructed strata, support state and pending-recheck sets (memory and saves, D-3); memory per building, per region and per layer; per-tick CPU class; summary/coarse representation for LOD regions (SIM.30, DEC-012) and deep-history collapse. |
| Q-10 | **Hooks and event contracts:** events and APIs for SIM.40.02 collapse, SIM.40.03 colonist behaviour (props, pillars, unsafe digging), SIM.40.04 QA, SIM.50.10 earthquakes and sinkholes, SIM.50.02 water breaching through floors, SIM.50.05 fire weakening wood, SIM.60 spells; event names, payloads and ordering. |
| Q-11 | **Acceptance tests and fixtures for the later code lanes:** automatable tests (deterministic cave-in, tower collapse across several upper layers, blast breaching one and two floors, exact ledger totals before/after, zero full-world scans) each with a mutant that must fail. |
| Q-12 | **WBS impact and Owner questions:** PROPOSED packages with real dependency IDs (including the material-and-structure model prerequisite and the dependency changes the audit §6 asks for), and Owner questions with options (for example material strength tables, how realistic spans are, whether furniture carries load). |
...
## Deliverables (all under `tasks/SIM.40.01/lane-q/`)
1. `SIM.40.01_STRUCTURAL_SUPPORT.md`, the design document. Header: task, base commit (paste `git rev-parse HEAD`), inputs read, method, limits, dependency on PROPOSED ADR-003. It must contain these exact headings (the gate checks them):
- `## Material model`
- `## Buildings as strata`
- `## Support model`
...
2. `trace.json`: `{ "task": "SIM.40.01", "baseCommit": "<sha>", "requirements": [ { "id": "Q-01", "section": "<exact heading text or a phrase that appears verbatim in the doc>", "status": "COVERED|PARTIAL|OWNER_QUESTION", "notes": "..." }, ... ], "proposedPackages": [ { "id": "PROPOSED-Q-01", "title": "...", "deps": ["<real WBS ids>"], "acceptanceTests": ["..."] } ], "ownerQuestions": [ { "id": "OQ-Q-01", "question": "...", "options": ["..."] } ] }` with exactly 12 requirement records `Q-01`..`Q-12` in the order above.
...
## Gate check (in `tasks/SIM.40.01/lane-q/lane.json`)
A node one-liner run from the repo root: `trace.json` parses, `task` is `SIM.40.01`, it has exactly the 12 requirement IDs, every status is one of the three words, every `section` string appears verbatim in `SIM.40.01_STRUCTURAL_SUPPORT.md`, every required heading is present, there is at least one Owner question, and the doc is at least 24,000 bytes. Run it yourself before your final commit and paste its output in REPORT.md.

## Acceptance criteria (the independent Grok reviewer will check these)
- Buildings are designed as constructed strata (LAND-5) and the material-model gap is closed with a concrete property table and new materials that later lanes can implement.
- Support rechecks are local to mutations with a stated worst-case bound; sudden and decay-driven collapse both end as rubble of equal mass with localized rechecks.
- Blasts break floors across layers using 10-ft layers and 2-ft slices with material attenuation, SRD damage unchanged.
- Every build/dig/collapse/blast/salvage path has ledger entries; LAND-1 leaks closed in the design.
...
- Proposed packages use `PROPOSED-Q-NN`, name real dependency IDs, and each has automatable acceptance tests including a mutant that must fail.
- No file outside allowedPaths changed; no code or art committed; nothing self-certified.

Commit messages start `[claude] SIM.40.01`.
...
4. Commit and push to your own branch only: `git push origin task/lane-q` after your final commit (never any other branch, never `main`, never force). If the push is refused, do not work around it (never set `DEUS_INTEGRATOR`); write the refusal in your REPORT and stop; the PM pushes.
...
6. Stop and write `tasks/SIM.40.01/lane-q/escalation.md` (then commit and push it) if you need a file outside your allowedPaths, find a bug in a read-only shared file, or find two sources that disagree in a way the brief does not settle. Never resolve an Owner question yourself.
