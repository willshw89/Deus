...
`tools/run_tests.js` is the NW.js harness (`--deus-test=<suite>`). `DEUS_Combat` registers the `combat` suite and `DEUS_Anim` registers `anim`. Those runs are in the FIX1 section below. `tools/validate_srd_catalog.js` was not run; the brief records that it exits 1 at base (37/1) and is out of this lane.
...
`combat:hit` still carries `attacker`, `target`, `damage`, `hit`, `style`, `attackType`, `critical`, `fumble`, `advantage`, `disadvantage`, `roll`. `combat:kill` still carries `attacker` and `target`. `tools/bench_render_layers.js` calls `resolveAttack` with no `opts.hit` and no `opts.damage`, and it does not read the result keys (that file was not edited). Its units carry `combatLevels` only, so they resolve as the SRD Commoner. At `1057a448` an unmapped unit neither dealt nor took damage, and the combat loop logged a console error on every swing.
...
- `game/js/sim/rules/species_map.js` maps all 23 catalog species onto an SRD 5.1 stat block. Runtime hit points are `UF.Rules.hitPoints` of that block, the printed average. The catalog `combat` block is not a combat number. The FIX1 section lists every row. A proxy row is `PM_DEFAULT`.
- A humanoid with no ability scores uses the SRD Commoner (AC 10, 4 hit points, Club when unarmed). That is `species: "human"` and a unit that only has `combatLevels`. A unit with `data.stats` is a character and keeps `data.maxHp` or `data.dnd.hpMax`.
- `data.stats` / `abilities` / `scores` with no creature match is a character. Unarmored AC is 10 + Dexterity. Armor and shields come from `equipment.json`. `mail_iron`, `plate_iron`, `shield_iron`, `shield_wood`, and `armor_leather` name Chain Mail, Plate, Shield, and Leather. `fiber_wrap`, `hide_cloak`, and `common_clothes` are listed non-armor.
...
1. Class Unarmored Defense is not applied unless `data.unarmoredDefense` is `"barbarian"` or `"monk"`. The monk feature does not apply while a shield is wielded (`srd:class:monk`). The barbarian feature allows a shield. The baseline otherwise is 10 + Dexterity.
...
4. When a creature has several weapon actions and the weapon key matches none of their names, the attack throws `UNKNOWN_WEAPON`. When more than one action matches, it throws `AMBIGUOUS_ATTACK`.
...
8. Closed in FIX1. `srd:rule:combat-damage-and-healing` says "Resistance and then vulnerability are applied after all other modifiers to damage." 25 fire with both is 12, then 24.
...
- PROPOSED-AB-03: decide whether the catalog’s 14 slot names or the sheet’s 12 slots are the paper doll. `clothes` → `torso` does not appear on the sheet. The species map is done; proxy rows are `PM_DEFAULT`.
- PROPOSED-AB-04: `DEUS_Test.js` around line 490 still passes `legacy: true` into `resolveAttack`. The option is ignored. Removing it is a follow-up outside this lane.
- PROPOSED-AB-05: `DEUS_Wildlife.js` around lines 1117-1124 subtracts the catalog `combat.attack` from prey hit points beside `C.engage`. That is a second damage law. It belongs to a follow-up after Lane AA. This lane did not edit that file.
...
- `game/js/sim/rules/species_map.js`
...
## FIX1 (after review_claude_1057a448)

SRD 5.1 is the only source of combat numbers. `DEUS_Combat.maxHp` calls `UF.Rules.hitPoints` and uses the printed average. The hit-dice formula is not rolled. The catalog `combat` block is not read for hit points, attack, defence, level, or bonus.

At `1057a448`, 14 catalog species had no stat block. An attack by or against one of them threw, the combat loop logged a console error, and the swing did no damage. The `anim` suite aborted on the first `resolveAttack` (0 passed, 1 failed) against a base of 5 passed, 5 failed. After this fix every catalog species resolves, and a humanoid with no ability scores uses the SRD Commoner, so those callers keep working. The `anim` suite on this tree is 5 passed, 5 failed, the same five passes as base `819ef62d`.

### Findings

| Finding | What changed | Where | Proof |
|---|---|---|---|
| MAJOR-1 | Runtime `maxHp` is `UF.Rules.hitPoints` printed average | `DEUS_Combat.js` `maxHp`, `UF_Combat.md`, `DEUS_Rules.md` | `catalog_species_maxhp_is_srd_average`; mutant that reads catalog `hitpoints` returns wolf 24 and is killed. NW.js `combat.srd_creatures` PASS (wolf 11, deer 4, jackal 3, boar 11, giant spider 26, troll 84) |
| MAJOR-2 | 23 species mapped; humanoids with no scores use Commoner | `species_map.js`, `rules.js` `creatureOf` | `species_map_covers_23_and_commoner_default`. Bench-shaped `combatLevels` unit resolves. `anim` no longer aborts |
| MAJOR-3 | NW.js `combat` and `anim` run in a throwaway clone; `srd_creatures` expects the SRD block | `DEUS_Combat.js` suite | `combat` 18 passed, 1 failed (`perf`, below). `anim` tip 5 passed, 5 failed. `anim` base `819ef62d` 5 passed, 5 failed. The five base passes still pass |
| MAJOR-4 | `armor_leather` aliases SRD Leather. Torso/clothes/shield items resolve or are listed non-armor | `srd_index.js` `ITEM_ALIASES`, `NON_ARMOR` | `catalog_armor_slots_resolve_or_are_listed_non_armor`. Mutant that drops the alias throws `UNKNOWN_ARMOR` |
| MAJOR-5 | `savingThrow` uses the printed bonus | `rules.js` `rollTotal` | `printed_saves_315_equal_10_plus_bonus` (Adult Red Dragon Con at roll 10 is 23). Mutant that ignores the printed bonus is killed |
| MINOR-1 | Resistance, then vulnerability | `rules.js` `adjustForTraits` | 25 fire becomes 24. Mutant that skips vulnerability when both apply is killed. Open question 8 closed |
| MINOR-2 | Unconditional `plus N (XdY)` riders and save-gated damage riders | `srd_index.js` `parseAttackExtra`, `rules.js` `damage` | 63 unconditional and 19 save riders applied. Dragon bite max 40 (28 piercing + 12 fire). Giant spider bite adds 2 poison on a failed DC 11 Con save, 1 on a success (half). Mutant that drops the rider loop is killed. 119 gaps listed in the rules log |
| MINOR-3 | Removed `combatLevel`, `level()`, `bonusesOf`, `levelOffset`, `magicDefence`, and the style accuracy/strength/defence fields. Strongest target uses challenge rating or character level. Chronicle names that rank | `DEUS_Combat.js` | `zero_retired_formula_in_deus_combat` |
| MINOR-4 | A named `opts.hit` / `opts.damage` still goes through `UF.Rules.damage` | `DEUS_Combat.js` `resolveAttack` | Petrified half-damage check still passes (`combat_applies_half_damage_to_petrified`). `DEUS_Test.js` `legacy: true` is PROPOSED-AB-04 |
| MINOR-5 | The plugin publish is what the loader check reads. `rulesApi` throws `RULES_NOT_PUBLISHED` if `attach` did not run | `DEUS_Combat.js`, `test_srd_rules.js` | `plugin_publishes_window_uf_rules`. Mutant that removes `mod.attach(window, rules)` is killed by that check |
| MINOR-6 | Default rng is mulberry32 from the world seed. `opts.magical` skips nonmagical resistance | `DEUS_Combat.js` `attackRng`, `rules.js` `traitApplies` | `plugin_default_rng_is_seeded`, `plugin_default_rng_repeats_for_the_same_seed`, `magical_bypasses_nonmagical_resistance`. Both mutants are killed |
| MINOR-7 | The clothes/torso sheet gap is a `KNOWN_GAP` line and is not a pass | `test_d20_equipment_slots.js` | Log line `KNOWN_GAP sheet_clothes_alias_has_no_torso_slot PROPOSED-AB-03`. Results 20 passed, 0 failed |
| MINOR-8 | Text says `data.unarmoredDefense`. Death-save thresholds and the long-jump distance are parsed from srd51. Monk Unarmored Defense rejects a shield. `AMBIGUOUS_ATTACK` is named in open question 4 | `rules.js`, `srd_index.js`, `DEUS_Rules.md` | `monk_unarmored_defense_forbids_shield` (bare AC 16, with shield AC 15, barbarian with shield AC 17) |
| MINOR-9 | Wildlife catalog damage left in place | not edited | PROPOSED-AB-05 |

### Species map

Direct rows use the creature of the same name. Proxy rows are `PM_DEFAULT (Owner may re-map)`.

| Species | SRD id | Kind | Reason |
|---|---|---|---|
| deer | `srd:creature:deer` | direct | The catalog deer is the SRD Deer. HP 4 |
| boar | `srd:creature:boar` | direct | The catalog boar is the SRD Boar. HP 11 |
| rat | `srd:creature:rat` | direct | The catalog rat is the SRD Rat |
| wolf | `srd:creature:wolf` | direct | The catalog wolf is the SRD Wolf. HP 11 |
| jackal | `srd:creature:jackal` | direct | The catalog jackal is the SRD Jackal. HP 3 |
| hawk | `srd:creature:hawk` | direct | The catalog hawk is the SRD Hawk |
| bat | `srd:creature:bat` | direct | The catalog bat is the SRD Bat |
| giant_spider | `srd:creature:giant-spider` | direct | The catalog giant spider is the SRD Giant Spider. HP 26 |
| troll | `srd:creature:troll` | direct | The catalog troll is the SRD Troll. HP 84 |
| hare | `srd:creature:weasel` | proxy `PM_DEFAULT (Owner may re-map)` | No hare. The Weasel is the nearest tiny beast with a bite. HP 1 |
| fowl | `srd:creature:blood-hawk` | proxy `PM_DEFAULT (Owner may re-map)` | No fowl. The Blood Hawk is the nearest small bird with a beak |
| fox | `srd:creature:jackal` | proxy `PM_DEFAULT (Owner may re-map)` | No fox. The Jackal is the nearest small canid |
| arctic_fox | `srd:creature:jackal` | proxy `PM_DEFAULT (Owner may re-map)` | No arctic fox. The Jackal is the nearest small canid |
| wildcat | `srd:creature:panther` | proxy `PM_DEFAULT (Owner may re-map)` | No wildcat. The Panther is the nearest medium feline with a claw |
| serpent | `srd:creature:poisonous-snake` | proxy `PM_DEFAULT (Owner may re-map)` | No serpent. The Poisonous Snake is the nearest venomous bite |
| songbird | `srd:creature:raven` | proxy `PM_DEFAULT (Owner may re-map)` | No songbird. The Raven is the nearest tiny bird with a beak |
| bog_horror | `srd:creature:shambling-mound` | proxy `PM_DEFAULT (Owner may re-map)` | No bog horror. The Shambling Mound is the marsh plant horror |
| sand_stalker | `srd:creature:ankheg` | proxy `PM_DEFAULT (Owner may re-map)` | No sand stalker. The Ankheg is the nearest burrowing ambush predator |
| restless_dead | `srd:creature:zombie` | proxy `PM_DEFAULT (Owner may re-map)` | No restless dead. The Zombie is the risen corpse |
| aurochs | `srd:creature:elk` | proxy `PM_DEFAULT (Owner may re-map)` | No aurochs. The Elk is the nearest large horned grazer |
| wild_horse | `srd:creature:riding-horse` | proxy `PM_DEFAULT (Owner may re-map)` | No wild horse. The Riding Horse is the horse stat block |
| wild_sheep | `srd:creature:goat` | proxy `PM_DEFAULT (Owner may re-map)` | No sheep. The Goat is the nearest small horned grazer |
| ice_wraith | `srd:creature:wraith` | proxy `PM_DEFAULT (Owner may re-map)` | No ice wraith. The Wraith is the incorporeal undead of that name |

A catalog natural attack uses that creature's own printed action, so a boar's stab is Tusk and a hare's stab is the Weasel's Bite.

### Rider gaps

176 attack actions have an `extra` string. 63 unconditional `plus N (XdY) type damage` riders are applied, each with its own type, after resistance and then vulnerability. 19 save-gated damage riders are applied through `savingThrow` when the DC, the ability, and the dice parse (full on a failed save, half when the text says half on a success). The review counted 60 and 52; the counts above are the strings this parser accepts in `attack.extra`.

119 extras still have a part that is not applied. The rules log prints one `RIDER_GAP` line each. Grouped:

- 56 condition, grapple, or forced movement (this lane does not apply conditions)
- 27 alternate primary damage (two-handed, shillelagh, enlarged, swarm at half hit points), not an extra rider
- 16 ongoing damage plus a condition
- 6 hit point maximum reduction
- 5 prose with no parsed damage rider
- 4 ongoing damage, hit point maximum reduction, and a condition
- 2 ongoing damage only
- 1 instant death (Solar slaying longbow)
- 1 choice of damage type (djinni scimitar, lightning or thunder)
- 1 ability score reduction (shadow Strength Drain)

The giant spider's 2d8 poison is applied. The gap on that bite is the paralyzed-while-poisoned condition.

### NW.js

Throwaway clone `%TEMP%\lane_ab_fix1`, `core.autocrlf=false`, FIX1 tree copied in, then deleted after these runs. `nw.exe` children were the harness's own. Other `nw.exe` processes were already running and were left alone.

Final `combat` on this tree (`RESULT: 18 passed, 1 failed`):

```
PASS combat.srd_d20
PASS combat.srd_damage
PASS combat.srd_styles
PASS combat.srd_equipment
PASS combat.srd_creatures - 23 species; hp match the SRD block; hare hp 1; attack types true; wolf hit true ; resolve ok; veteran AC 18
PASS combat.hitsplats
PASS combat.health_bar
PASS combat.attack_speed
PASS combat.retaliate
PASS combat.no_code_motion
PASS combat.death
PASS combat.regen
PASS combat.saved
PASS combat.faction_aid_called
PASS combat.faction_aid_range
PASS combat.faction_aid_isolated
FAIL combat.perf - 100 units fighting, 360 frames at x1, zoom 1.00: 0.525 ms per frame (loop 0.130, drawing 0.395; worst single update in the window: loop 15.93 ms, drawing 4.66 ms); 250 attacks, 100 health bars and 50 hitsplats drawn at the end
PASS combat.no_math_random
PASS combat.no_errors
RESULT: 18 passed, 1 failed (exit 1)
EXIT=1
```

Two earlier combat runs on the same tree were 0.577 and 0.577 ms/frame, each with one 14–19 ms tick. The loop steady cost is about 0.12 ms/frame. The 0.50 ms budget is the mean, so one hitch puts it over. This was measured while other `nw.exe` processes were already running.

`anim` at this tree:

```
PASS anim.no_carry_pose
PASS anim.missing_frames_plain
PASS anim.object_frames
PASS anim.remains_saved
PASS anim.no_errors
FAIL anim.no_code_motion
FAIL anim.death_frames_and_remains
FAIL anim.state_frames
FAIL anim.layers
FAIL anim.pooled_and_perf
RESULT: 5 passed, 5 failed (exit 1)
EXIT=1
```

`anim` at base `819ef62d93027f221c8e8c24245679d164d500be`:
...
Every check that passed at base still passes.

### Gate log

Commands from `tasks/SIM.60.05/lane-ab/lane.json`, run from the worktree root on this tree.
...
PASS: sheet_layout_columns_spaced (slots spaced horizontally across 6 columns)
UF_World: no walkable cell within 24 of (10,10) in area (0,0) for "TEST_CombatColonist": left on the blocked cell it asked for
PASS: combat_mail_in_armor_slot_is_chain_mail (chain mail AC 16 (SRD chain mail, Commoner Dexterity does not add))
PASS: combat_weaponOf_resolves_mainHand (weapon profile name=Stone Axe, speed=5)
UF_World: no walkable cell within 24 of (12,12) in area (0,0) for "TEST_LegacyColonist": left on the blocked cell it asked for
PASS: combat_clothes_alias_reads_chain_mail (clothes mail_iron AC 16)
PASS: sheet_weapon_alias_maps_to_mainHand (mainHand via weapon type stone_axe)
KNOWN_GAP sheet_clothes_alias_has_no_torso_slot PROPOSED-AB-03 clothes maps to torso; DEUS_Sheet has no torso slot (slots 12, mail_iron shown: false)
PASS: test_must_be_able_to_fail (verified test can detect inequality and throw)
...
`git diff --name-only $(git merge-base origin/main HEAD) HEAD` after this commit adds `game/js/sim/rules/species_map.js` to the branch diff already listed above. Every path is inside allowedPaths. `review_claude_1057a448.md` was not edited.
