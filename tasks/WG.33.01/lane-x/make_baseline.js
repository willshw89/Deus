...
function reason(f) {
    ...
    switch (`${f.rule} ${f.code}`) {
        case 'WSR-01 SYSTEM_NOT_IN_ENUM': {
            const sys = /system (\S+) is not/.exec(d)[1];
            return `Registry row names system ${sys}, which is not in the section 2 system enum (the enum has ECOLOGY_WILDLIFE); the registry is Gemini's file, question X-Q1.`;
        }
        ...
        case 'MANIFEST-TEMPLATE SHEET_NOT_IN_TEMPLATE': {
            ...
            const s = ctx.cat.sheets.get(f.id);
            if (s.kind === 'RMMZ_CHARACTER') return `No template for tree sheet ${f.id} because the generator refuses the sheet (SHEET_INVALID); it has ${n} slots; X-E2, PROPOSED-X-05.`;
            return `No template for ${f.id} because the generator refuses GEOM_STRATUM slots on it, so none of its ${n} slots can be compared; X-E2, PROPOSED-X-05.`;
        }
        default:
            return null;
    }
...
for (const f of ctx.findings) {
    ...
    entries.push({ rule: f.rule, code: f.code, id: f.id, reason: r });
}
if (missing.length) {
...
const head = {
    ...
    about: `Known gaps of tools/verify_world_state_registry.js on main ${BASE} (WG.33.01, Lane X, 2026-09-26). Written by tasks/WG.33.01/lane-x/make_baseline.js; every entry has a one-line reason. The gate fails on any gap not listed here and on any entry that no longer occurs, so this file must shrink as gaps close. Question and proposal ids (X-Qn, X-En, PROPOSED-X-nn) are in tasks/WG.33.01/lane-x/REPORT.md.`,
    base: BASE
};
const lines = ['{'];
